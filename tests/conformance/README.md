...
3. **Drives the REAL dispatch path** — it launches the stub through the
   ...
   (`MERGEMILL_CONF`, `MERGEMILL_CONF_DIR`, `PROJECT_DIR`) is pointed at
   conf-free paths so `load_MergeMill_conf` finds no operator conf on any of its
   ...
   inherited operator env — including a project's real `MergeMill.conf` reachable
   via `MERGEMILL_CONF_DIR` or `PROJECT_DIR` — can't leak extra argv, route
