...
LIB_SMOKE="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-agent-smoke.sh"
...
_classify_fixture() {
  ...
  (
    ...
    # lib-agent.sh::load_MergeMill_conf has THREE discovery branches, ALL read at
    # source time: (1) MERGEMILL_CONF (a file), (2) MERGEMILL_CONF_DIR/MergeMill.conf
    # — called as `load_MergeMill_conf "${MERGEMILL_CONF_DIR:-$_LIB_AGENT_DIR}"`,
    # (3) PROJECT_DIR/scripts/MergeMill.conf. Scrubbing only MERGEMILL_CONF is NOT
    # enough: an inherited MERGEMILL_CONF_DIR pointing at a real project conf (e.g.
    ...
    # same via branch 3. CRUCIALLY, setting MERGEMILL_CONF_DIR="" does NOT help —
    # the `${MERGEMILL_CONF_DIR:-$_LIB_AGENT_DIR}` default treats empty as unset and
    # falls back to $_LIB_AGENT_DIR, which on a self-hosting checkout resolves into a
    # real scripts/ tree that DOES carry a live MergeMill.conf. So point the
    # conf-discovery vars at CONCRETE conf-free paths: MERGEMILL_CONF at a file that
    # cannot exist, MERGEMILL_CONF_DIR + PROJECT_DIR at an empty dir (no
    # MergeMill.conf, no scripts/MergeMill.conf). All three branches then miss and
    # `load_MergeMill_conf` returns 1 — the classification depends ONLY on input.env.
    # This makes the runner self-defending: it no longer relies on the caller passing
    # `env -u PROJECT_DIR`.
    export MERGEMILL_CONF="$no_conf_dir/.nonexistent.conf"
    export MERGEMILL_CONF_DIR="$no_conf_dir"
    ...
    #     FAIL argv-mismatch. The fixtures encode the lib default `MergeMill-dev`.
    ...
    export KIRO_AGENT_NAME="MergeMill-dev"
    ...
    # shellcheck source=../../skills/MergeMill-dispatcher/scripts/lib-agent-smoke.sh
    source "$LIB_SMOKE" 2>/dev/null || { printf '__ERR__:lib-source-failed\n'; exit 0; }

    # AGENT_CMD MUST be set AFTER sourcing: lib-agent.sh's load_MergeMill_conf
    ...
    if [[ "$mode" == "dev-resume" ]]; then
      ...
      # shellcheck source=../../skills/MergeMill-dispatcher/scripts/lib-review-codex.sh
      source "$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-codex.sh" 2>/dev/null \
    ...
    #     production review wrapper (MergeMill-review.sh's drop-loop), so the
    ...
    case "$adapter" in
      ...
      kiro|codex)
        ...
        if [[ "$adapter" == "kiro" ]]; then
          ...
          # (MergeMill-review.sh's drop-loop: `_classify_codex_drop_reason <log>
    ...
    if [[ "$mode" == "review" && -n "$artifact_src" ]]; then
      ...
      source "$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-artifact.sh" 2>/dev/null || true
