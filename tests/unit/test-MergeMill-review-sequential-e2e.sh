#!/bin/bash
# test-MergeMill-review-sequential-e2e.sh — issue #182 / INV-46.
#
...
#   2. source-of-truth greps against MergeMill-review.sh for the structural
...
#
# Run: bash tests/unit/test-MergeMill-review-sequential-e2e.sh
...
E2E_LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-e2e.sh"
...
AGG_LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-aggregate.sh"
REF="$PROJECT_ROOT/skills/MergeMill-review/references/e2e-command-mode.md"
...
if [[ -f "$E2E_LIB" ]]; then
  # shellcheck source=../../skills/MergeMill-dispatcher/scripts/lib-review-e2e.sh
  source "$E2E_LIB"
...
if [[ -f "$E2E_LIB" ]]; then
  ...
  # its own jq over that shape. In the review wrapper, MergeMill-review.sh
...
if [[ -f "$E2E_LIB" ]]; then
  ...
  PROVIDER_ITP="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/providers/itp-github.sh"
