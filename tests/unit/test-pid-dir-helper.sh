...
LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-config.sh"

RED='\033[0;31m'
GREEN='\033[0;32m'
NC='\033[0m'

# shellcheck source=../../skills/MergeMill-dispatcher/scripts/lib-config.sh
source "$LIB"
set +e
...
# TC-PD-001: MERGEMILL_PID_DIR override wins
unset XDG_RUNTIME_DIR
export MERGEMILL_PID_DIR="$TMPROOT/override"
out=$(pid_dir_for_project)
rc=$?
assert_rc "MERGEMILL_PID_DIR override returns 0" 0 "$rc"
assert_eq "MERGEMILL_PID_DIR override echoes the override path" "$TMPROOT/override" "$out"
assert_eq "MERGEMILL_PID_DIR override creates dir" "true" "$([ -d "$TMPROOT/override" ] && echo true || echo false)"
mode=$(stat -c '%a' "$TMPROOT/override" 2>/dev/null || stat -f '%Lp' "$TMPROOT/override" 2>/dev/null)
assert_eq "MERGEMILL_PID_DIR override sets mode 700" "700" "$mode"
unset MERGEMILL_PID_DIR
...
out=$(pid_dir_for_project)
assert_eq "XDG_RUNTIME_DIR honored" "$TMPROOT/xdg/MergeMill-test-piddir" "$out"
...
out=$(pid_dir_for_project)
assert_eq "HOME fallback path" "$TMPROOT/home/.local/state/MergeMill-test-piddir" "$out"
...
out=$(pid_dir_for_project)
assert_eq "XDG_RUNTIME_DIR set but missing → HOME fallback" "$TMPROOT/home/.local/state/MergeMill-test-piddir" "$out"
unset XDG_RUNTIME_DIR
...
mkdir -p "$target"
ln -sfn "$target" "$TMPROOT/home/.local/state/MergeMill-test-piddir-link"
