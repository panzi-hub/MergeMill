...
( timeout 5 bash -c 'source "'"$TMPDIR"'/skills/MergeMill-dispatcher/scripts/lib-agent.sh" 2>/dev/null; acquire_pid_guard "'"$CRASH_PID_FILE"'" test-crash 42' )
CRASH_RC=$?
...
if command -v mkfifo >/dev/null 2>&1; then
  ...
  ( timeout 10 bash -c 'source "'"$TMPDIR"'/skills/MergeMill-dispatcher/scripts/lib-agent.sh" 2>/dev/null; acquire_pid_guard "'"$FIFO_PID_FILE"'" test-fifo-lock 101' 2>/dev/null )
  FIFO_RC=$?
