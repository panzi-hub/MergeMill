...
# + MERGEMILL_PROVIDERS_DIR=<fixture>), exactly like test-provider-dispatch.sh
...
guard_out=$(
  env -u ISSUE_PROVIDER -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
      ISSUE_PROVIDER=degraded MERGEMILL_PROVIDERS_DIR="$guard_dir" \
      REPO="$REPO" REPO_OWNER="$REPO_OWNER" PROJECT_ID="$PROJECT_ID" MAX_RETRIES=3 MAX_CONCURRENT=5 \
  bash -c '
    set -euo pipefail
    source "'"$LIB"'" 2>/dev/null
...
deps_guard_out=$(
  env -u ISSUE_PROVIDER -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
      ISSUE_PROVIDER=noleaf MERGEMILL_PROVIDERS_DIR="$noleaf_dir" \
      REPO="$REPO" REPO_OWNER="$REPO_OWNER" PROJECT_ID="$PROJECT_ID" MAX_RETRIES=3 MAX_CONCURRENT=5 \
  bash -c '
    set -euo pipefail
