...
REVIEW_SCRIPTS="$PROJECT_ROOT/skills/MergeMill-review/scripts"
...
if grep -qE 'readlink -f' "$UPLOAD" && grep -qE 'MergeMill-dispatcher/scripts/lib-code-host.sh' "$UPLOAD"; then
...
guard_out=$(
  env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
      CODE_HOST=emptyprov MERGEMILL_PROVIDERS_DIR="$EMPTY_PROV_DIR" \
  bash -c '
    set -uo pipefail
    source "'"$CHP_LIB"'" 2>/dev/null
