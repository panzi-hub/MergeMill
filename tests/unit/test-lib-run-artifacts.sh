...
#
# Strategy: source the lib, point MERGEMILL_RUN_DIR_BASE at a temp dir, exercise
...
LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-run-artifacts.sh"
ARTIFACT_LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-artifact.sh"
...
# shellcheck source=../../skills/MergeMill-dispatcher/scripts/lib-run-artifacts.sh
source "$LIB"
...
(
  export PROJECT_ID="proj"
  unset MERGEMILL_RUN_DIR_BASE
  export XDG_STATE_HOME="/x"
  assert_eq "TC-010 XDG_STATE_HOME path" "/x/MergeMill-proj/runs/proj-235-dev-T" "$(run_dir_for proj-235-dev-T)"
)
(
  export PROJECT_ID="proj"
  unset MERGEMILL_RUN_DIR_BASE XDG_STATE_HOME
  export HOME="/h"
  assert_eq "TC-011 HOME fallback path" "/h/.local/state/MergeMill-proj/runs/proj-235-dev-T" "$(run_dir_for proj-235-dev-T)"
)
(
  export PROJECT_ID="proj"
  unset MERGEMILL_RUN_DIR_BASE
  ...
  # shellcheck source=../../skills/MergeMill-dispatcher/scripts/lib-review-artifact.sh
...
(
  export PROJECT_ID="proj"
  export MERGEMILL_RUN_DIR_BASE="$TMP_ROOT/state/MergeMill-proj"
...
(
  export PROJECT_ID="proj"
  export MERGEMILL_RUN_DIR_BASE="$TMP_ROOT/tmplog/MergeMill-proj"
...
(
  export PROJECT_ID="proj"
  export MERGEMILL_RUN_DIR_BASE="$TMP_ROOT/tmplog97/MergeMill-proj"
...
(
  export PROJECT_ID="proj"
  export MERGEMILL_RUN_DIR_BASE="$TMP_ROOT/state2/MergeMill-proj"
  unset RUN_ID RUN_DIR
...
(
  export PROJECT_ID="proj"
  export MERGEMILL_RUN_DIR_BASE="$TMP_ROOT/state3/MergeMill-proj"
...
(
  export PROJECT_ID="proj"
  export MERGEMILL_RUN_DIR_BASE="/proc/cannot-write-here/MergeMill-proj"
  unset RUN_ID RUN_DIR
  set -e
  rc_before=0
  run_artifacts_init dev 235 || true
...
echo "== run_footer =="
(
  export RUN_ID="proj-235-dev-T" RUN_DIR="/x/MergeMill-proj/runs/proj-235-dev-T"
  f="$(run_footer)"
  assert_contains "TC-030a footer has run-id" "run-id: proj-235-dev-T" "$f"
  assert_contains "TC-030b footer has artifacts dir" "artifacts: /x/MergeMill-proj/runs/proj-235-dev-T" "$f"
...
(
  export PROJECT_ID="proj"
  export MERGEMILL_RUN_DIR_BASE="$TMP_ROOT/prune/MergeMill-proj"
  parent="$MERGEMILL_RUN_DIR_BASE/runs"
  mkdir -p "$parent"
...
(
  export PROJECT_ID="proj"
  export MERGEMILL_RUN_DIR_BASE="$TMP_ROOT/prune2/MergeMill-proj"
  mkdir -p "$MERGEMILL_RUN_DIR_BASE/runs/proj-235-dev-OLD"
  jq -nc --arg s "$(date -u -d '-99 days' +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -v-99d +%Y-%m-%dT%H:%M:%SZ)" \
    '{started_at:$s}' > "$MERGEMILL_RUN_DIR_BASE/runs/proj-235-dev-OLD/meta.json"
  unset RUN_ID
  run_prune "not-a-number" 235 || true
  assert_false "TC-038 non-numeric retention falls back to 30 (still prunes 99d)" \
    [ -d "$MERGEMILL_RUN_DIR_BASE/runs/proj-235-dev-OLD" ]
)
(
  export PROJECT_ID="proj"
  export MERGEMILL_RUN_DIR_BASE="$TMP_ROOT/prune3/MergeMill-proj"   # runs/ absent
...
(
  export PROJECT_ID="proj"
  export MERGEMILL_RUN_DIR_BASE="$TMP_ROOT/prune-all/MergeMill-proj"
  parent="$MERGEMILL_RUN_DIR_BASE/runs"
  mkdir -p "$parent"
...
(
  export PROJECT_ID="proj"
  export MERGEMILL_RUN_DIR_BASE="$TMP_ROOT/persist/MergeMill-proj"
  unset RUN_ID RUN_DIR
