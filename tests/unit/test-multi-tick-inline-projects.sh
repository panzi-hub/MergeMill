...
# REPO, REPO_OWNER, REPO_NAME, EXECUTION_BACKEND, SSM_*) and MERGEMILL_CONF.
...
WRAPPER_SRC="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/dispatcher-multi-tick.sh"
...
# local projects, we source MERGEMILL_CONF first to mirror what the real
...
#!/bin/bash
# Mirror lib-config.sh::load_MergeMill_conf priority-1 behavior.
if [[ -n "${MERGEMILL_CONF:-}" ]] && [[ -f "$MERGEMILL_CONF" ]]; then
  # shellcheck disable=SC1090
  source "$MERGEMILL_CONF"
fi
{
  printf 'MERGEMILL_CONF=%s\n' "${MERGEMILL_CONF:-<unset>}"
  printf 'REPO=%s\n' "${REPO:-<unset>}"
  printf 'REPO_OWNER=%s\n' "${REPO_OWNER:-<unset>}"
  printf 'REPO_NAME=%s\n' "${REPO_NAME:-<unset>}"
  printf 'PROJECT_ID=%s\n' "${PROJECT_ID:-<unset>}"
...
# MERGEMILL_TRUST_CONF=1 because /tmp is mode 1777 (PR-8 trust gate)
export MERGEMILL_TRUST_CONF=1
...
assert_contains "MERGEMILL_CONF unset for inline" "MERGEMILL_CONF=<unset>" "$record"
...
# Local project tick records MERGEMILL_CONF=$LOCAL_CONF
assert_contains "local project: MERGEMILL_CONF set to MergeMill.conf path" "MERGEMILL_CONF=$LOCAL_CONF" "$record"
# Remote project records inline metadata + MERGEMILL_CONF=<unset>
assert_contains "remote project: REPO=myorg/remote-proj propagated" "REPO=myorg/remote-proj" "$record"
assert_contains "remote project: MERGEMILL_CONF unset" "MERGEMILL_CONF=<unset>" "$record"
...
record=$(cat "$RECORD")
assert_contains "PR-8 path entry: MERGEMILL_CONF set" "MERGEMILL_CONF=$LOCAL_CONF" "$record"
...
# ---------------------------------------------------------------------------
echo ""
echo "=== TC-IFILT-123: path-entry project (MergeMill.conf file) has NO charset restriction ==="
# ---------------------------------------------------------------------------
