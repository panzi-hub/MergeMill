...
#
# Tests the MERGEMILL_CONF env var and ${BASH_SOURCE[0]:-$0} fallback.
...
# ===========================================================================
echo ""
echo "=== TC-BSE-001: MERGEMILL_CONF env var takes highest priority ==="
echo ""
...
# Config in script dir (should be ignored when MERGEMILL_CONF is set)
cat > "$TMPDIR/bse-001/scripts/MergeMill.conf" <<'CONF'
...
#!/bin/bash
PROJECT_ID=""
if [[ -n "${MERGEMILL_CONF:-}" && -f "$MERGEMILL_CONF" ]]; then
  source "$MERGEMILL_CONF"
else
  _SELF="${BASH_SOURCE[0]:-$0}"
  _DIR="$(cd "$(dirname "$(readlink -f "$_SELF")")" && pwd)"
  if [[ -f "${_DIR}/MergeMill.conf" ]]; then
    source "${_DIR}/MergeMill.conf"
  fi
fi
echo "$PROJECT_ID"
SCRIPT
...
RESULT=$(MERGEMILL_CONF="$TMPDIR/bse-001/env-config.conf" bash "$TMPDIR/bse-001/scripts/test-config.sh")
assert_eq "MERGEMILL_CONF env var takes priority" "from-env-var" "$RESULT"

# ===========================================================================
# TC-BSE-002: Config loads normally when MERGEMILL_CONF is not set
# ===========================================================================
echo ""
echo "=== TC-BSE-002: Config loads from local dir without MERGEMILL_CONF ==="
echo ""

RESULT=$(unset MERGEMILL_CONF; bash "$TMPDIR/bse-001/scripts/test-config.sh")
assert_eq "Local config loaded when MERGEMILL_CONF unset" "from-local-dir" "$RESULT"

# ===========================================================================
# TC-BSE-003: bash -c invocation works with MERGEMILL_CONF
# ===========================================================================
echo ""
echo "=== TC-BSE-003: bash -c invocation works with MERGEMILL_CONF ==="
echo ""

RESULT=$(MERGEMILL_CONF="$TMPDIR/bse-001/env-config.conf" bash -c "bash $TMPDIR/bse-001/scripts/test-config.sh")
assert_eq "bash -c with MERGEMILL_CONF works" "from-env-var" "$RESULT"
...
cat > "$TMPDIR/bse-004/scripts/MergeMill.conf" <<'CONF'
...
# ===========================================================================
# TC-BSE-005: Invalid MERGEMILL_CONF path falls through to local config
# ===========================================================================
echo ""
echo "=== TC-BSE-005: Invalid MERGEMILL_CONF falls through ==="
echo ""

RESULT=$(MERGEMILL_CONF="/nonexistent/path.conf" bash "$TMPDIR/bse-001/scripts/test-config.sh")
assert_eq "Invalid MERGEMILL_CONF falls through to local" "from-local-dir" "$RESULT"
...
# PR-4 (#58 fix) consolidated MERGEMILL_CONF / BASH_SOURCE handling into
# lib-config.sh::load_MergeMill_conf. lib-agent.sh and lib-auth.sh now
# delegate via `source lib-config.sh; load_MergeMill_conf "$dir"`.
...
if [[ -f "$LIB_AGENT" ]]; then
  ...
  assert_contains "lib-agent.sh calls load_MergeMill_conf" 'load_MergeMill_conf' "$CONTENT"
...
if [[ -f "$LIB_AUTH" ]]; then
  ...
  assert_contains "lib-auth.sh calls load_MergeMill_conf" 'load_MergeMill_conf' "$CONTENT"
...
if [[ -f "$LIB_CONFIG" ]]; then
  ...
  assert_contains "lib-config.sh defines load_MergeMill_conf" 'load_MergeMill_conf()' "$CONTENT"
  assert_contains "lib-config.sh MERGEMILL_CONF priority 1" 'MERGEMILL_CONF' "$CONTENT"
