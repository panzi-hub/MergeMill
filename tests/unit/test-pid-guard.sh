#!/bin/bash
# test-pid-guard.sh — Unit tests for PID guard logic in MergeMill-dev.sh and MergeMill-review.sh
#
...
echo "TC-SCRIPT-002: MergeMill-dev.sh calls acquire_pid_guard"
if [[ -f "$DEV_SCRIPT" ]]; then
  DEV_CONTENT=$(cat "$DEV_SCRIPT")
  assert_contains "PID guard call in MergeMill-dev.sh" 'acquire_pid_guard "$PID_FILE" "MergeMill-dev"' "$DEV_CONTENT"
else
  echo -e "  ${RED}FAIL${NC}: MergeMill-dev.sh not found at $DEV_SCRIPT"
  ((FAIL++))
fi

echo "TC-SCRIPT-003: MergeMill-review.sh calls acquire_pid_guard"
if [[ -f "$REVIEW_SCRIPT" ]]; then
  REVIEW_CONTENT=$(cat "$REVIEW_SCRIPT")
  assert_contains "PID guard call in MergeMill-review.sh" 'acquire_pid_guard "$PID_FILE" "MergeMill-review"' "$REVIEW_CONTENT"
else
  echo -e "  ${RED}FAIL${NC}: MergeMill-review.sh not found at $REVIEW_SCRIPT"
  ((FAIL++))
fi
...
SKILL_MD=$(mktemp)
cat \
  "$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/dispatcher-tick.sh" \
  "$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-dispatch.sh" \
  > "$SKILL_MD"
