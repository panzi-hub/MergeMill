...
_MOCK_ISSUE_LIST="[$(mkissue 101 MergeMill in-progress approved)]"
...
_MOCK_ISSUE_LIST="[$(mkissue 102 MergeMill reviewing approved)]"
out=$(list_stale_candidates)
...
_MOCK_ISSUE_LIST="[$(mkissue 103 MergeMill in-progress)]"
out=$(list_stale_candidates)
assert_count "TC-003 in-progress alone detected" 1 "$out"

# TC-004: reviewing alone → MUST still be detected
_MOCK_ISSUE_LIST="[$(mkissue 104 MergeMill reviewing)]"
out=$(list_stale_candidates)
assert_count "TC-004 reviewing alone detected" 1 "$out"

# Mixed bag — partial defense + partial detection in one query
_MOCK_ISSUE_LIST="[$(mkissue 201 MergeMill in-progress approved),$(mkissue 202 MergeMill in-progress),$(mkissue 203 MergeMill reviewing approved),$(mkissue 204 MergeMill reviewing)]"
out=$(list_stale_candidates)
