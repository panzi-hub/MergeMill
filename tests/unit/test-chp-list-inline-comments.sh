...
# MergeMill-dev.sh:1086 (the dev-resume prompt builder's PR_REVIEW_COMMENTS read).
#
...
if [[ -f "$REC1/.verbs" ]] && grep -qx "api repos/panzi-hub/MergeMill/pulls/42/comments" "$REC1/.verbs"; then
...
rc=0
out=$(env -u CODE_HOST -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
        REPO="$REPO" \
      bash -c "
        set -euo pipefail
...
rc2=0
env -u CODE_HOST -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR REPO="$REPO" \
  bash -c "
    set -uo pipefail
    source '$CHP_LIB' 2>/dev/null
    unset -f chp_github_list_inline_comments 2>/dev/null || true
...
rc3=0
out3=$(env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR REPO="$REPO" CODE_HOST="fakehost" \
  bash -c "
    set -euo pipefail
    source '$CHP_LIB' 2>/dev/null
...
assert_eq "AC3 MergeMill-dev.sh has ZERO executable raw 'gh api …/pulls/\$PR_NUM/comments'" "0" "$n_pulls"
assert_eq "AC3 MergeMill-dev.sh invokes chp_list_inline_comments \"\$PR_NUM\" (×1)" \
...
assert_eq "AC3 the distinct issues/\$PR_NUM/comments AUTO_MERGE read is also gone (migrated by #334)" \
  "0" "$n_issues"
assert_eq "AC3 MergeMill-dev.sh's AUTO_MERGE marker now routes through itp_list_comments \"\$PR_NUM\"" \
...
if command -v jq >/dev/null 2>&1; then
  has_pulls=$(jq '[.surviving_sites[] | select(.file=="MergeMill-dev.sh" and (.content | test("pulls/\\$\\{PR_NUM\\}/comments")))] | length' "$BASELINE")
  has_issues=$(jq '[.surviving_sites[] | select(.file=="MergeMill-dev.sh" and (.content | test("issues/\\$\\{PR_NUM\\}/comments")))] | length' "$BASELINE")
...
AC4_BULLET='- #296 second-tier (#328): the dev-resume PR inline-review-comment read (`PR_REVIEW_COMMENTS`, MergeMill-dev.sh) migrated from raw `gh api repos/$REPO/pulls/$PR_NUM/comments --jq` to the NEW verb `chp_list_inline_comments` — byte-identical (the `--jq` formatter stays caller-side, #281); baseline shrank by 1 sig.'
