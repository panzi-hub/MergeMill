...
# match to `${PROJECT_DIR}/scripts/MergeMill-<type>.sh --issue <N>` so that:
#   1. dev-resume for issue N never SIGTERMs MergeMill-review.sh for N
...
DISPATCH_LOCAL="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/dispatch-local.sh"
...
assert_contains "TC-REGEX-001 dev-new contains MergeMill-dev.sh" \
  'MergeMill-dev\.sh' "$re_dev_new"
[[ "$re_dev_new" != *MergeMill-review* ]] \
  && { echo -e "  ${GREEN}PASS${NC}: TC-REGEX-001 dev-new excludes review wrapper"; PASS=$((PASS+1)); } \
  || { echo -e "  ${RED}FAIL${NC}: TC-REGEX-001 dev-new should not include 'MergeMill-review'"; FAIL=$((FAIL+1)); }

assert_contains "TC-REGEX-002 dev-resume contains MergeMill-dev.sh" \
  'MergeMill-dev\.sh' "$re_dev_resume"
assert_contains "TC-REGEX-003 review contains MergeMill-review.sh" \
  'MergeMill-review\.sh' "$re_review"
[[ "$re_review" != *MergeMill-dev\\.sh* ]] \
  && { echo -e "  ${GREEN}PASS${NC}: TC-REGEX-003 review excludes dev wrapper"; PASS=$((PASS+1)); } \
  || { echo -e "  ${RED}FAIL${NC}: TC-REGEX-003 review should not include 'MergeMill-dev.sh'"; FAIL=$((FAIL+1)); }
assert_contains "TC-REGEX-004 default catch-all matches both wrappers under project" \
  'MergeMill-(dev|review)\.sh' "$re_default"
...
[[ "/home/xX.local/foo/scripts/MergeMill-dev.sh" =~ ^${re_dotted}$ ]] \
...
# TC-PGREP-001 — dev-resume must NOT kill MergeMill-review.sh decoy of same project.
echo ""
echo "--- TC-PGREP-001 dev-resume MUST NOT kill review wrapper for same issue/project ---"
review_pid=$(spawn_decoy "$PROJ_A" "MergeMill-review.sh" 998877 --some-flag x)
...
# TC-PGREP-002 — review must NOT kill MergeMill-dev.sh decoy of same project.
echo ""
echo "--- TC-PGREP-002 review MUST NOT kill dev wrapper for same issue/project ---"
dev_pid=$(spawn_decoy "$PROJ_A" "MergeMill-dev.sh" 998877 --mode resume)
...
echo ""
echo "--- TC-PGREP-003 dev-resume DOES kill MergeMill-dev.sh orphan for same issue/project ---"
dev_pid=$(spawn_decoy "$PROJ_A" "MergeMill-dev.sh" 998877 --mode resume)
_track_decoy "$dev_pid"
if ! kill -0 "$dev_pid" 2>/dev/null; then
...
dev_pid=$(spawn_decoy "$PROJ_A" "MergeMill-dev.sh" 99 --mode resume)
_track_decoy "$dev_pid"
if ! kill -0 "$dev_pid" 2>/dev/null; then
...
projb_pid=$(spawn_decoy "$PROJ_B" "MergeMill-dev.sh" 998877 --mode resume)
