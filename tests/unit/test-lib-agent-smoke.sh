...
LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-agent-smoke.sh"
...
PIPELINE_DOC="$PROJECT_ROOT/docs/MergeMill-pipeline.md"
...
# project MergeMill.conf via lib-agent.sh, restoring conf-assigned globals like
...
#
# We simulate a configured box by pointing the lib at a temp MergeMill.conf
# (MERGEMILL_CONF env override, honored by lib-config.sh::load_MergeMill_conf)
...
PATH="$STUB_DIR:$ORIG_PATH" MERGEMILL_CONF="$ENVORDER_CONF" SMOKE_CONF="$envorder_conf" \
...
launch_out=$(PATH="$STUB_DIR:$ORIG_PATH" MERGEMILL_CONF="$LAUNCH_CONF" SMOKE_CONF="$launch_conf" \
...
launch_claude_out=$(PATH="$STUB_DIR:$ORIG_PATH" MERGEMILL_CONF="$LAUNCH_CONF" SMOKE_CONF="$launch_conf" \
  bash "$HARNESS" 2>/dev/null)
...
# No conf launcher this time (MERGEMILL_CONF without AGENT_LAUNCHER) so the ONLY
...
GH_AUTH_MODE="token"
CONF
PATH="$STUB_DIR:$ORIG_PATH" MERGEMILL_CONF="$launch_optin_conf" SMOKE_CONF="$launch_conf" \
  bash "$HARNESS" >/dev/null 2>&1
...
printf 'm|claude|MiniMax-M2|export SMOKE_LAUNCHER_MARKER=%s; export ANTHROPIC_BASE_URL=https://api.example.test/anthropic\n' \
  "$LAUNCH_MARKER" > "$launch_conf"
ce_out=$(PATH="$STUB_DIR:$ORIG_PATH" MERGEMILL_CONF="$LAUNCH_CONF" SMOKE_CONF="$launch_conf" \
  bash "$HARNESS" 2>/dev/null)
...
# branch; the operator's MergeMill.conf tunes it for ONE CLI (e.g. kiro's
...
extra_out=$(PATH="$STUB_DIR:$ORIG_PATH" MERGEMILL_CONF="$EXTRA_CONF" SMOKE_CONF="$extra_conf" \
  bash "$HARNESS" 2>/dev/null)
...
extra_optin_out=$(PATH="$STUB_DIR:$ORIG_PATH" MERGEMILL_CONF="$EXTRA_CONF" SMOKE_CONF="$extra_conf" \
  bash "$HARNESS" 2>/dev/null)
...
assert_grep "TC-AGENT-SMOKE-056c MergeMill-pipeline.md references the smoke" 'agent-smoke' "$PIPELINE_DOC"
