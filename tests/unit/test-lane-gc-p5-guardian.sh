...
#   - MergeMill-dev.sh / MergeMill-review.sh: guardian install-order
...
# ===========================================================================
for wf in "MergeMill-dev.sh:$DEV_WRAPPER" "MergeMill-review.sh:$REVIEW_WRAPPER"; do
  name="${wf%%:*}"; file="${wf#*:}"
...
declare -A SITES=(
  ...
  ["MergeMill-review.sh:smoke probe subshell"]="$REVIEW_WRAPPER"
  ["MergeMill-review.sh:fan-out subshell"]="$REVIEW_WRAPPER"
  ...
  ["MergeMill-dev.sh:run.log tee process substitution"]="$DEV_WRAPPER"
  ["MergeMill-review.sh:run.log tee process substitution"]="$REVIEW_WRAPPER"
...
if [[ "$REVIEW_CNT" -ge 2 ]]; then
  assert_pass "TC-LGC5-103: MergeMill-review.sh carries >= 2 distinct ADT_GUARD_FD close guards (smoke probe, fan-out subshell) — found $REVIEW_CNT"
else
  assert_fail "TC-LGC5-103: MergeMill-review.sh carries only $REVIEW_CNT ADT_GUARD_FD close guard(s) — expected >= 2"
fi
...
# reason to mention it) — MergeMill-review.sh legitimately names the real
