...
mapfile -t out_app < <(env -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
...
mapfile -t out32 < <(env -u MERGEMILL_CONF_DIR -u PROJECT_DIR -u GH_TOKEN_FILE -u GH_TOKEN -u GITHUB_PERSONAL_ACCESS_TOKEN -u GH_USER_PAT \
...
# Scrub the live-wrapper env contamination (MERGEMILL_CONF_DIR / inherited PATH
...
env -u MERGEMILL_CONF_DIR -u PROJECT_DIR -u GH_TOKEN_FILE -u GITHUB_PERSONAL_ACCESS_TOKEN -u GH_USER_PAT \
  PATH="/usr/bin:/bin" \
  bash -c "
...
env -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="/usr/bin:/bin" bash -c "
  source '$SBA/lib-auth.sh'
...
# MergeMill-dev.sh's function in isolation is impractical (it has a heavy top),
...
# `gh pr create`.
DEV_SH="$SCRIPTS/MergeMill-dev.sh"
...
mapfile -t out92 < <(env -u MERGEMILL_CONF_DIR -u PROJECT_DIR -u GH_TOKEN_FILE -u GITHUB_PERSONAL_ACCESS_TOKEN -u GH_USER_PAT \
  PATH="/usr/bin:/bin" bash -c "
  source '$SBA/lib-auth.sh'
  wdir=\$(mktemp -d /tmp/agent-auth-XXXXXX)
...
# refreshed token.
mapfile -t out93 < <(env -u MERGEMILL_CONF_DIR -u PROJECT_DIR -u GH_TOKEN_FILE -u GITHUB_PERSONAL_ACCESS_TOKEN -u GH_USER_PAT \
  PATH="/usr/bin:/bin" bash -c "
  source '$SBA/lib-auth.sh'
  wdir=\$(mktemp -d /tmp/agent-auth-XXXXXX)
  export GH_WRAPPER_DIR=\"\$wdir\"; export PATH=\"\$wdir:\$PATH\"
...
out95=$(env -u MERGEMILL_CONF_DIR -u PROJECT_DIR -u GH_TOKEN -u GH_TOKEN_FILE -u GITHUB_PERSONAL_ACCESS_TOKEN -u GH_USER_PAT \
  PATH="/usr/bin:/bin" bash -c "
  source '$SBA/lib-auth.sh'
  wdir=\$(mktemp -d /tmp/agent-auth-XXXXXX); export GH_WRAPPER_DIR=\"\$wdir\"; export PATH=\"\$wdir:\$PATH\"
...
env -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="$GHSB95:/usr/bin:/bin" bash -c "
...
# MergeMill-review.sh must export AGENT_BOT_TRIGGER_FILE + drain it post-run.
LIB_BOTS="$SCRIPTS/lib-review-bots.sh"
REVIEW_SH="$SCRIPTS/MergeMill-review.sh"
...
# (c) source-level: MergeMill-review.sh exports AGENT_BOT_TRIGGER_FILE AND drains it.
if grep -qF 'export AGENT_BOT_TRIGGER_FILE' "$REVIEW_SH" \
   && grep -qF 'drain_agent_bot_triggers "$ISSUE_NUMBER" "$REPO"' "$REVIEW_SH"; then
  assert_pass "source: MergeMill-review.sh exports AGENT_BOT_TRIGGER_FILE and drains it (drain_agent_bot_triggers)"
else
  assert_fail "source: MergeMill-review.sh does not export/drain the bot-trigger broker file"
fi
...
env -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="$DRAIN_GH:/usr/bin:/bin" bash -c "
...
MBR=$(env -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="$MBR_GH:/usr/bin:/bin" \
  MBR_REVIEWS='[]' bash -c "
...
MBR2=$(env -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="$MBR_GH:/usr/bin:/bin" \
...
env -u MERGEMILL_CONF_DIR -u PROJECT_DIR -u CODE_HOST PATH="$MBR_GH:/usr/bin:/bin" bash -c "
  set -euo pipefail
  source '$MBR_SBA/lib-review-bots.sh'
  missing_bot_reviews 'q' 42 owner/repo
" >"$MBR3_OUT" 2>/dev/null
...
env -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="$MBR_GH:/usr/bin:/bin" \
  CODE_HOST=noleaf MERGEMILL_PROVIDERS_DIR="$NOLEAF_DIR" bash -c "
  set -euo pipefail
  source '$MBR_SBA/lib-code-host.sh'
  source '$MBR_SBA/lib-review-bots.sh'
  missing_bot_reviews 'q' 42 owner/repo
...
# (e) source-level: MergeMill-review.sh hard-gates a missing bot review in the PASS
#     branch (calls missing_bot_reviews and re-queues to pending-review) + passes the
#     allow-list to the drain; MergeMill-dev.sh passes the allow-list too.
...
# `git ls-remote --heads origin`, mirroring [INV-45] at MergeMill-dev.sh:397) —
...
PATH="$RSB:$PATH" bash -c "
  ...
  # [#342] Source the CHP seam FIRST, mirroring production order (MergeMill-review.sh
...
out_clean=$(env -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
...
# (CODE_HOST=<name> + MERGEMILL_PROVIDERS_DIR=<fixture dir>):
...
GHSTUB
  ...
  env -u CODE_HOST -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="$GHG1:/usr/bin:/bin" REPO="owner/repo" bash -c "
...
GHSTUB
  ...
  env -u CODE_HOST -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="$GHG2:/usr/bin:/bin" REPO="owner/repo" \
    MERGEMILL_CONF_DIR="$SBG2" bash -c "
...
GHSTUB
  ...
  out_raw=$(env -u CODE_HOST -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="$GHRAW:/usr/bin:/bin" REPO="owner/repo" bash -c "
  ...
  # chp_github_trigger_bot, selected via CODE_HOST=github + MERGEMILL_PROVIDERS_DIR:
...
GHSTUB
  ...
  out_gn=$(env -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="$GHGN:/usr/bin:/bin" REPO="owner/repo" \
    MERGEMILL_CONF_DIR="$SBGN" CODE_HOST=github MERGEMILL_PROVIDERS_DIR="$FBDISP_GH_NOTRIGGER" bash -c "
  ...
  out_n1=$(env -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="$GHN1:/usr/bin:/bin" REPO="owner/repo" \
    CODE_HOST=fbdispnoleaf MERGEMILL_PROVIDERS_DIR="$FBDISP_NOLEAF" bash -c "
  ...
  out_n2=$(env -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="$GHN2:/usr/bin:/bin" REPO="owner/repo" \
    MERGEMILL_CONF_DIR="$SBN2" CHP_FBDISP_PR_BODY="closes #346" \
    CODE_HOST=fbdispnoleaf MERGEMILL_PROVIDERS_DIR="$FBDISP_NOLEAF" bash -c "
  ...
  out_n3=$(env -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="$GHN3:/usr/bin:/bin" REPO="owner/repo" \
    MERGEMILL_CONF_DIR="$SBN3" CHP_FBDISP_PR_BODY="closes #346" \
    CODE_HOST=fbdispnoleaf MERGEMILL_PROVIDERS_DIR="$FBDISP_NOLEAF" bash -c "
  ...
  env -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="$GHL1:/usr/bin:/bin" REPO="owner/repo" \
    CHP_FBDISP_LEAF_LOG="$LEAFLOG1" CODE_HOST=fbdispleaf MERGEMILL_PROVIDERS_DIR="$FBDISP_LEAF" bash -c "
  ...
  env -u MERGEMILL_CONF_DIR -u PROJECT_DIR PATH="$GHL2:/usr/bin:/bin" REPO="owner/repo" \
    MERGEMILL_CONF_DIR="$SBL2" CHP_FBDISP_PR_BODY="closes #346" CHP_FBDISP_LEAF_LOG="$LEAFLOG2" \
    CODE_HOST=fbdispleaf MERGEMILL_PROVIDERS_DIR="$FBDISP_LEAF" bash -c "
