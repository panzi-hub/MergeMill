...
# exit "nothing outstanding". This test extracts that helper from MergeMill-dev.sh and
...
# Extract emit_post_approval_findings_block() from MergeMill-dev.sh.
...
out=$(run_helper 188 50 \
  GH_PR_REVIEWS_JSON="$(write_reviews APPROVED 2026-06-08T07:00:00Z)" \
  GH_ISSUE_COMMENTS_JSON="$(write_comments "2026-06-08T06:00:00Z::Dispatching MergeMill review...")")
...
AM_DOC="$PROJECT_ROOT/skills/MergeMill-dev/references/MergeMill-mode.md"
assert_grep "TC-PAF-D01: MergeMill-mode.md documents approval-vs-findings timestamp ordering" \
  '[Tt]imestamp|newer than the.*approval|after the approval' "$AM_DOC"
assert_grep "TC-PAF-D02: MergeMill-mode.md documents broadened findings recognition" \
  'BLOCKING|\[P1\]' "$AM_DOC"
