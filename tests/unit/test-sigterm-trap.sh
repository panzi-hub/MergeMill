#!/bin/bash
# test-sigterm-trap.sh — Verify INV-15 SIGTERM convergence in MergeMill-dev.sh.
#
...
# MergeMill-dev.sh — any divergence means the test is lying.
# ---------------------------------------------------------------------------
classify_label() {
...
# ---------------------------------------------------------------------------
# Guard against drift: the cleanup() in MergeMill-dev.sh must contain the
...
WRAPPER="$SCRIPT_DIR/../../skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh"
LIB_AGENT="$SCRIPT_DIR/../../skills/MergeMill-dispatcher/scripts/lib-agent.sh"

# The trap can be installed two ways:
#   (A) Inline in MergeMill-dev.sh: `on_sigterm()` + `trap on_sigterm TERM`.
...
if [[ "$trap_inline_ok" -eq 1 || "$trap_helper_ok" -eq 1 ]] \
  ...
  echo -e "  ${GREEN}PASS${NC}: MergeMill-dev.sh contains RECEIVED_SIGTERM trap + cleanup rewrite"
  PASS=$((PASS + 1))
else
  echo -e "  ${RED}FAIL${NC}: MergeMill-dev.sh missing one of:"
