...
DISPATCHER_SCRIPTS="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts"
...
# ===========================================================================
echo ""
echo "=== TC-SYM-004: Config fallback finds MergeMill.conf ==="
echo ""
...
cat > "$TMPDIR/sym-004/project/scripts/MergeMill.conf" <<'CONF'
...
# skills/MergeMill-dispatcher/scripts/ -> ../../scripts/ goes to project root
mkdir -p "$TMPDIR/sym-004b/skills/MergeMill-dispatcher/scripts"
mkdir -p "$TMPDIR/sym-004b/scripts"

cat > "$TMPDIR/sym-004b/scripts/MergeMill.conf" <<'CONF'
PROJECT_ID="test-project-from-fallback"
PROJECT_DIR="/tmp/fake-project"
CONF

cat > "$TMPDIR/sym-004b/skills/MergeMill-dispatcher/scripts/test-config.sh" <<'SCRIPT'
#!/bin/bash
SCRIPT_DIR="$(cd "$(dirname "$(readlink -f "$0")")" && pwd)"
PROJECT_ID=""
if [[ -f "${SCRIPT_DIR}/MergeMill.conf" ]]; then
...
echo "$PROJECT_ID"
SCRIPT
chmod +x "$TMPDIR/sym-004b/skills/MergeMill-dispatcher/scripts/test-config.sh"

RESULT=$(bash "$TMPDIR/sym-004b/skills/MergeMill-dispatcher/scripts/test-config.sh")
assert_eq "Config loaded from fallback ../../scripts/MergeMill.conf" "test-project-from-fallback" "$RESULT"

# ===========================================================================
# TC-SYM-005: Local MergeMill.conf takes precedence over fallback
# ===========================================================================
echo ""
echo "=== TC-SYM-005: Local config takes precedence ==="
echo ""

mkdir -p "$TMPDIR/sym-005/skills/MergeMill-dispatcher/scripts"
mkdir -p "$TMPDIR/sym-005/scripts"

cat > "$TMPDIR/sym-005/skills/MergeMill-dispatcher/scripts/MergeMill.conf" <<'CONF'
PROJECT_ID="local-config"
PROJECT_DIR="/tmp/fake-project"
CONF

cat > "$TMPDIR/sym-005/scripts/MergeMill.conf" <<'CONF'
PROJECT_ID="fallback-config"
PROJECT_DIR="/tmp/fake-project"
CONF

cat > "$TMPDIR/sym-005/skills/MergeMill-dispatcher/scripts/test-config.sh" <<'SCRIPT'
#!/bin/bash
SCRIPT_DIR="$(cd "$(dirname "$(readlink -f "$0")")" && pwd)"
PROJECT_ID=""
if [[ -f "${SCRIPT_DIR}/MergeMill.conf" ]]; then
...
echo "$PROJECT_ID"
SCRIPT
chmod +x "$TMPDIR/sym-005/skills/MergeMill-dispatcher/scripts/test-config.sh"

RESULT=$(bash "$TMPDIR/sym-005/skills/MergeMill-dispatcher/scripts/test-config.sh")
...
DEV_SCRIPT="$DISPATCHER_SCRIPTS/MergeMill-dev.sh"
REVIEW_SCRIPT="$DISPATCHER_SCRIPTS/MergeMill-review.sh"
...
# MUST still hold is narrower: the CONF lookup (load_MergeMill_conf) must
...
if [[ -f "$LIB_AGENT" ]]; then
  ...
  assert_contains "lib-agent.sh calls load_MergeMill_conf" 'load_MergeMill_conf' "$LIB_CONTENT"
  # INV-14 preserved: conf lookup must use the UNRESOLVED dir, not the realpath one.
  if grep -nE 'load_MergeMill_conf[[:space:]]+"\$\{?_LIB_AGENT_REAL_DIR' "$LIB_AGENT" >/dev/null; then
    echo -e "  ${RED}FAIL${NC}: lib-agent.sh passes the realpath dir to load_MergeMill_conf (INV-14 regression)"
    ((FAIL++))
  else
...
if [[ -f "$LIB_AUTH" ]]; then
  ...
  assert_contains "lib-auth.sh calls load_MergeMill_conf" 'load_MergeMill_conf' "$LIB_AUTH_CONTENT"
  ...
  if grep -nE 'load_MergeMill_conf[[:space:]]+"\$\{?_LIB_AUTH_REAL_DIR' "$LIB_AUTH" >/dev/null; then
    echo -e "  ${RED}FAIL${NC}: lib-auth.sh passes the realpath dir to load_MergeMill_conf (INV-14 regression)"
    ((FAIL++))
  else
...
# Simulates: .agents/skills/MergeMill-dispatcher/scripts/lib-agent.sh
# with MergeMill.conf at <project>/scripts/MergeMill.conf
mkdir -p "$TMPDIR/sym-006/skills/MergeMill-dispatcher/scripts"
mkdir -p "$TMPDIR/sym-006/scripts"

cat > "$TMPDIR/sym-006/scripts/MergeMill.conf" <<'CONF'
PROJECT_ID="installed-skill-project"
PROJECT_DIR="/tmp/fake-project"
CONF

cat > "$TMPDIR/sym-006/skills/MergeMill-dispatcher/scripts/test-lib-config.sh" <<'SCRIPT'
#!/bin/bash
_LIB_AGENT_DIR="$(cd "$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")" && pwd)"
PROJECT_ID=""
if [[ -f "${_LIB_AGENT_DIR}/MergeMill.conf" ]]; then
  source "${_LIB_AGENT_DIR}/MergeMill.conf"
elif [[ -f "${_LIB_AGENT_DIR}/../../../scripts/MergeMill.conf" ]]; then
  source "${_LIB_AGENT_DIR}/../../../scripts/MergeMill.conf"
fi
echo "$PROJECT_ID"
SCRIPT
chmod +x "$TMPDIR/sym-006/skills/MergeMill-dispatcher/scripts/test-lib-config.sh"

RESULT=$(bash "$TMPDIR/sym-006/skills/MergeMill-dispatcher/scripts/test-lib-config.sh")
...
# MergeMill-dev.sh in the project's scripts/ to print PROJECT_ID and exit.
...
_inv14_install_lib_chain() {
  ...
  if [[ "$mode" == "with-trap" ]]; then
    cat > "$target/MergeMill.conf" <<'WRONG_CONF'
...
# Helper: build a project layout with scripts/MergeMill.conf and a stub
# MergeMill-dev.sh that prints PROJECT_ID. Caller decides how to wire
...
_inv14_make_project() {
  ...
  cat > "$proj/scripts/MergeMill.conf" <<CONF
PROJECT_ID="$marker"
REPO="test/test"
REPO_OWNER="test"
REPO_NAME="test"
...
CONF
  ...
  cat > "$proj/scripts/MergeMill-dev.sh" <<'STUB'
#!/bin/bash
...
exit 0
STUB
  # Also stub MergeMill-review.sh symmetrically (some test paths exercise it).
  cp "$proj/scripts/MergeMill-dev.sh" "$proj/scripts/MergeMill-review.sh"
  chmod +x "$proj/scripts/MergeMill-dev.sh" "$proj/scripts/MergeMill-review.sh"
}
...
# real MergeMill.conf. Pre-exporting PROJECT_DIR would let
...
VENDOR1="$PROJ1/.agents/skills/MergeMill-dispatcher/scripts"
_inv14_make_project "$PROJ1" "marker-vendored"
mkdir -p "$PROJ1/.agents/skills/MergeMill-common/scripts"
...
SHARE2="$TMPDIR/inv14-2-share/skills/MergeMill-dispatcher/scripts"
...
SHARE3="$TMPDIR/inv14-3-altshare/opt/share/MergeMill-dispatcher/scripts"
...
# Verifies the legacy ${SCRIPT_DIR}/../../../scripts/MergeMill.conf
...
#
#   <proj>/skills/MergeMill-dispatcher/scripts/dispatch-local.sh
#     ↑     ↑      ↑                     ↑
...
LEGACY_VENDOR4="$PROJ4/skills/MergeMill-dispatcher/scripts"
...
# dispatch-local.sh -> MergeMill-dev.sh's PROJECT_DIR resolution. The
# wrapper inherits PROJECT_DIR from dispatch-local.sh's exported env,
# and lib-agent.sh's load_MergeMill_conf falls back to tier-3
# (${PROJECT_DIR}/scripts/MergeMill.conf) if its tier-2 misses.
echo ""
echo "=== TC-INV14-5: end-to-end (dispatch-local→wrapper) under shared install ==="
echo ""

# Reuse TC-INV14-2's setup but stub MergeMill-dev.sh to verify it sees
# the right env propagated from dispatch-local.sh.
PROJ5="$TMPDIR/inv14-5"
SHARE5="$TMPDIR/inv14-5-share/skills/MergeMill-dispatcher/scripts"
_inv14_make_project "$PROJ5" "marker-e2e"
_inv14_install_lib_chain "$SHARE5"
_inv14_link_chain "$PROJ5/scripts" "$SHARE5"
# Override the stubbed MergeMill-dev.sh to also print PROJECT_DIR seen
# inside the spawned wrapper, asserting env inheritance worked.
cat > "$PROJ5/scripts/MergeMill-dev.sh" <<'STUB'
#!/bin/bash
...
exit 0
STUB
chmod +x "$PROJ5/scripts/MergeMill-dev.sh"
