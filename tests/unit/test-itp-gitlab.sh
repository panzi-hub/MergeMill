...
ITP_GITLAB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/providers/itp-gitlab.sh"
...
# shellcheck source=../../skills/MergeMill-dispatcher/scripts/providers/itp-gitlab.sh
source "$ITP_GITLAB"
...
out=$(itp_gitlab_list_by_state open MergeMill 100 "number,title,labels"); rc=$?
...
_gl_stub_reset
_GL_STUB_PAYLOAD="$PAYLOADS/gitlab-issues-list.json"
out=$(itp_gitlab_list_by_state open MergeMill 100 "number") || true
assert_eq "TC-WB-006 FIELDS=number → single key" \
  "number" "$(jq -r '.[0] | keys_unsorted | join(",")' <<<"$out")"
_gl_stub_reset
_GL_STUB_PAYLOAD="$PAYLOADS/gitlab-issues-list.json"
out=$(itp_gitlab_list_by_state open MergeMill 100 "number,labels") || true
...
[
  {"iid": 5, "id": 500005, "title": "second issue", "description": "body five", "state": "opened", "labels": ["MergeMill"], "assignees": [{"username": "alice"}, {"username": "bob"}]},
  {"iid": 3, "id": 500003, "title": "first issue", "description": "body three", "state": "opened", "labels": ["MergeMill"], "assignees": []}
]
JSON
_GL_STUB_PAYLOAD="$assignees_pl"
out=$(itp_gitlab_list_by_state open MergeMill 100 "number,assignees") || true
...
_gl_stub_reset
_GL_STUB_MODE="fail"
out=$(itp_gitlab_list_by_state open MergeMill 100 "number" 2>/dev/null); rc=$?
...
cat > "$enum2" <<'JSON'
[
  {"iid": 10, "id": 5000010, "title": "t10", "description": "", "state": "opened", "labels": ["MergeMill"]},
  {"iid": 11, "id": 5000011, "title": "t11", "description": "", "state": "opened", "labels": ["MergeMill"]}
]
JSON
notes=$(mktemp)
cat > "$notes" <<'JSON'
...
out=$(itp_gitlab_list_by_state open MergeMill 100 "number,comments") || true
...
JSON
_GL_STUB_PAYLOAD="$pl"
out=$(itp_gitlab_count_by_state open MergeMill 100 "in-progress,reviewing") || true
assert_eq "TC-WB-010 any-of matches 2 of 3" "2" "$out"
_gl_stub_reset
_GL_STUB_PAYLOAD="$pl"
out=$(itp_gitlab_count_by_state open MergeMill 100 "") || true
...
_gl_stub_reset
_GL_STUB_MODE="fail"
out=$(itp_gitlab_count_by_state open MergeMill 100 "" 2>/dev/null); rc=$?
...
[
  ...
  {"iid": 2, "id": 2, "title": "", "description": "", "state": "opened", "labels": ["MergeMill"], "assignees": []},
...
JSON
_GL_STUB_PAYLOAD="$pl"
out=$(itp_gitlab_list_forbidden_combos open MergeMill 100) || true
...
_gl_stub_reset
_GL_STUB_MODE="fail"
out=$(itp_gitlab_list_forbidden_combos open MergeMill 100 2>/dev/null); rc=$?
...
assert_eq "TC-WB-040 labels are name-strings" \
  '["MergeMill","in-progress"]' "$(jq -c '.labels' <<<"$out")"
...
_GL_STUB_STATUS="200"
out=$(itp_gitlab_provision_states "MergeMill" "ededed" "d"); rc=$?
...
out1=$(itp_gitlab_provision_states "MergeMill" "ededed" "d")
out2=$(itp_gitlab_provision_states "MergeMill" "ededed" "d")
...
CAPS_FILE="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/providers/itp-gitlab.caps"
