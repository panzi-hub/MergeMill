...
CONF_EXAMPLE="$SCRIPTS_DIR/MergeMill.conf.example"
...
(
  export REPO=panzi-hub/MergeMill REPO_OWNER=zxkane REPO_NAME=MergeMill-dev-team
  export PROJECT_ID=test-rto GH_AUTH_MODE=token
  export PROJECT_DIR="$PROJECT_ROOT"
  # shellcheck source=../../skills/MergeMill-dispatcher/scripts/lib-agent.sh
  source "$AGENT_LIB"
  set +e
...
if [[ -f "$AGG_LIB" ]]; then
  # shellcheck source=../../skills/MergeMill-dispatcher/scripts/lib-review-aggregate.sh
  source "$AGG_LIB"
...
# MergeMill.conf there, so load_MergeMill_conf leaves the exported vars in
