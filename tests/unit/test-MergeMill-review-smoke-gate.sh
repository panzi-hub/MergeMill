#!/bin/bash
# test-MergeMill-review-smoke-gate.sh — issue #224 / INV-64.
#
# Pre-fan-out agent-smoke gate (Phase A.5). Two-pronged (the wrapper is too heavy
# to run end-to-end, mirroring test-MergeMill-review-sequential-e2e.sh):
#
#   1. source-of-truth greps against MergeMill-review.sh: the Phase A.5 block is
...
#
# Run: bash tests/unit/test-MergeMill-review-smoke-gate.sh
...
SMOKE_LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-smoke.sh"
CONF_EXAMPLE="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/MergeMill.conf.example"
...
# The whole-wrapper no-bare-wait pin is owned by test-MergeMill-review-multi-agent.sh
...
# ---------------------------------------------------------------------------
echo "=== TC-REVIEW-SMOKE config knobs (MergeMill.conf.example) ==="
# ---------------------------------------------------------------------------
...
_cascade() {
  ...
  env -i PATH="$PATH" HOME="$TEST_HOME" \
    REPO=panzi-hub/MergeMill REPO_OWNER=zxkane REPO_NAME=MergeMill-dev-team \
