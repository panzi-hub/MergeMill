...
# skills/MergeMill-dispatcher/scripts/ and skills/MergeMill-common/scripts/
...
# ---------------------------------------------------------------------------
assert_eq "MergeMill-dev.sh mode" "100755" \
  "$(git_mode skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh)"
assert_eq "MergeMill-review.sh mode" "100755" \
  "$(git_mode skills/MergeMill-dispatcher/scripts/MergeMill-review.sh)"
...
for lib in \
  ...
  skills/MergeMill-common/scripts/lib-installer.sh \
  skills/MergeMill-common/scripts/lib-installer-translate.sh; do
  mode=$(git_mode "$lib")
  if [[ "$mode" == "100644" ]]; then
...
assert_contains "tick self-heals MergeMill-dev.sh" \
  "MergeMill-dev.sh" "$TICK_CONTENT"
assert_contains "tick self-heals MergeMill-review.sh" \
  "MergeMill-review.sh" "$TICK_CONTENT"
...
# ---------------------------------------------------------------------------
LIB_INST="$PROJECT_ROOT/skills/MergeMill-common/scripts/lib-installer.sh"
...
# ---------------------------------------------------------------------------
for installer in "$PROJECT_ROOT"/skills/MergeMill-common/scripts/install-*-hooks.sh; do
...
# ---------------------------------------------------------------------------
echo ""
echo "=== TC-EXEC-008: MergeMill-dispatcher/SKILL.md has #97 hash-bump note ==="
# ---------------------------------------------------------------------------
DISP_SKILL="$PROJECT_ROOT/skills/MergeMill-dispatcher/SKILL.md"
