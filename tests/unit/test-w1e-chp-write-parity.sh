...
#   2. MergeMill-review.sh PASS path — chp_approve (rc drives manual-merge
#      fallback: notification + reviewing->approved transition + exit 0).
#   3. MergeMill-review.sh merge path — chp_merge (MERGE_OUT/MERGE_RC under
...
# ---------------------------------------------------------------------------
# 2. chp_approve — the PASS-path approve decision (MergeMill-review.sh:3510-3523).
...
_run_approve_case() {
  ...
  env -u PROJECT_DIR REPO="$REPO" REPO_OWNER="$REPO_OWNER" \
    bash -c '
      ...
      # Excerpt the PASS-decision block from MergeMill-review.sh.
      REVIEW_SH="'"$SCRIPTS"'/MergeMill-review.sh"
      PR_NUMBER=4242
      ISSUE_NUMBER=400
      E2E_ACTIVE=false
      # Simulate the exact block behavior (MergeMill-review.sh:3505-3523).
...
# ---------------------------------------------------------------------------
# 3. chp_merge — the merge decision (MergeMill-review.sh:3548-3663).
#    rc 0: metrics_emit result=success, itp_transition_state reviewing,MergeMill
...
_run_merge_case() {
  ...
  env -u PROJECT_DIR REPO="$REPO" _STUB_OUT="$stderr_out" \
    bash -c '
      ...
      ISSUE_NUMBER=400
      RUN_ID=RID-1
      # Simulate the exact merge block behavior (MergeMill-review.sh:3548-3663).
      set +e
      ...
      if [[ $MERGE_RC -eq 0 ]]; then
        ...
        itp_transition_state "$ISSUE_NUMBER" "reviewing,MergeMill" "approved"
...
if grep -qF 'TRANSITION:400 reviewing,MergeMill approved' <<<"$out0"; then
  ok "TC-W1E-120 chp_merge rc=0 performs the reviewing,MergeMill->approved CSV transition"
else
...
WRAPPER="$SCRIPTS/MergeMill-review.sh"
...
_pin "TC-W1E-315 merge: success CSV transition reviewing,MergeMill->approved" \
  'itp_transition_state "$ISSUE_NUMBER" "reviewing,MergeMill" "approved"' "$WRAPPER"
