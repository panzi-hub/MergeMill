...
DEV="$SCRIPTS/MergeMill-dev.sh"
REVIEW="$SCRIPTS/MergeMill-review.sh"
...
itp_github_transition_state 42 "reviewing,MergeMill" "" >/dev/null
assert_eq "TC-ITV-003 CSV multi-remove, empty add omits --add-label" \
  "issue edit 42 --repo $REPO --remove-label reviewing --remove-label MergeMill" "$(recorded_argv)"
...
assert_contains "TC-ITV-021 A2 review migrated to itp_transition_state CSV multi-remove" \
  'itp_transition_state "$ISSUE_NUMBER" "reviewing,MergeMill" "approved"' "$rev_a2"
...
assert_eq "TC-ITV-050 MergeMill-dev.sh has ZERO raw 'gh issue edit'" "0" "$n_dev"
n_rev=$(grep -c 'gh issue edit' "$REVIEW" || true)
assert_eq "TC-ITV-050 MergeMill-review.sh has ZERO raw 'gh issue edit'" "0" "$n_rev"
...
absent_dev=$(jq -r '[.surviving_sites[] | select(.file=="MergeMill-dev.sh" and (.content|test("gh issue edit")))] | length' "$BASELINE")
assert_eq "TC-ITV-051 baseline: MergeMill-dev.sh gh-issue-edit signature removed" "0" "$absent_dev"
absent_rev=$(jq -r '[.surviving_sites[] | select(.file=="MergeMill-review.sh" and (.content|test("gh issue edit")))] | length' "$BASELINE")
assert_eq "TC-ITV-051 baseline: MergeMill-review.sh gh-issue-edit signatures removed (both)" "0" "$absent_rev"
...
assert_not_contains "TC-ITV-041 review-pass-merged no longer anchors on the raw --remove-label literal" \
  '--remove-label "MergeMill"' "$a_merged"
