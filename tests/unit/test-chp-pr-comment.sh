...
#   MergeMill-review.sh:3342,3538  +  lib-review-e2e.sh:344,380,387,402,580
#
...
rc=0
out=$(env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
        REPO="$REPO" CODE_HOST=emptyprov MERGEMILL_PROVIDERS_DIR="$EMPTY_PROV_DIR" \
      bash -c "
        set -euo pipefail
...
warn=$(env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
        REPO="$REPO" CODE_HOST=emptyprov MERGEMILL_PROVIDERS_DIR="$EMPTY_PROV_DIR" \
...
assert_eq "TC-CPC-030 MergeMill-review.sh has ZERO executable raw 'gh pr comment'" \
...
assert_eq "TC-CPC-033 MergeMill-review.sh invokes chp_pr_comment ×2" \
...
# ONLY `chp_pr_comment "$PR_NUMBER" \` in MergeMill-review.sh (3538 is a capture,
