...
if [[ -f "$REC1/.verbs" ]] && grep -qx "api graphql" "$REC1/.verbs"; then
  ...
  assert_contains "S1 -F repo=MergeMill-dev-team present" "repo=MergeMill-dev-team" "$argv_joined"
...
env -u CODE_HOST -u MERGEMILL_CONF -u PROJECT_DIR \
    REPO="$REPO" REC_DIR="$REC2" BTFILE="$BTFILE" \
    MERGEMILL_CONF_DIR="$PROJECT_ROOT/skills/MergeMill-common/scripts" \
  bash -c "
    set -uo pipefail
...
if [[ -f "$REC2/.verbs" ]] && grep -qx "api graphql" "$REC2/.verbs"; then
  ...
  assert_contains "S2 -F repo=MergeMill-dev-team present" "repo=MergeMill-dev-team" "$argv_joined"
...
if grep -qF 'drain_agent_pr_create "$ISSUE_NUMBER" "$REPO"' "$DEV_SH"; then
  pass "TC-308-AC7-DEV-CREATE MergeMill-dev.sh calls drain_agent_pr_create \"\$ISSUE_NUMBER\" \"\$REPO\""
else
  fail "TC-308-AC7-DEV-CREATE drain_agent_pr_create not called with \"\$REPO\" in MergeMill-dev.sh"
fi
if grep -qF 'drain_agent_bot_triggers "$ISSUE_NUMBER" "$REPO"' "$DEV_SH"; then
  pass "TC-308-AC7-DEV-BOT MergeMill-dev.sh calls drain_agent_bot_triggers \"\$ISSUE_NUMBER\" \"\$REPO\""
else
  fail "TC-308-AC7-DEV-BOT drain_agent_bot_triggers not called with \"\$REPO\" in MergeMill-dev.sh"
fi
if grep -qF 'drain_agent_bot_triggers "$ISSUE_NUMBER" "$REPO"' "$REVIEW_SH"; then
  pass "TC-308-AC7-REVIEW-BOT MergeMill-review.sh calls drain_agent_bot_triggers \"\$ISSUE_NUMBER\" \"\$REPO\""
else
  fail "TC-308-AC7-REVIEW-BOT drain_agent_bot_triggers not called with \"\$REPO\" in MergeMill-review.sh"
fi
