...
# MergeMill-dev.sh entry + a project-side MergeMill.conf, and exactly ONE
...
SKILL="$TMPDIR/skill/MergeMill-dispatcher/scripts"
mkdir -p "$SKILL"
...
# real MergeMill.conf. NO lib symlinks at all — that's the whole point.
PROJ="$TMPDIR/proj"
mkdir -p "$PROJ/scripts"
ln -s "$SKILL/MergeMill-dev.sh" "$PROJ/scripts/MergeMill-dev.sh"
...
OUT=$(cd "$PROJ" && GH_TOKEN="dummy-token-for-e2e" \
  bash "$PROJ/scripts/MergeMill-dev.sh" 2>&1); RC=$?
...
if grep -qiE 'Set (REPO|PROJECT_ID|PROJECT_DIR) in MergeMill.conf' <<<"$OUT"; then
