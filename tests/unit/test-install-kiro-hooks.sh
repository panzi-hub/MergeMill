...
INSTALLER="$PROJECT_ROOT/skills/MergeMill-common/scripts/install-kiro-hooks.sh"
...
(cd "$TMPDIR/repo2" && bash "$INSTALLER" --no-git-hook --agent MergeMill-dev >/dev/null 2>&1)

target2="$TMPDIR/repo2/.kiro/agents/MergeMill-dev.json"
if [[ -f "$target2" ]]; then
  echo -e "  ${GREEN}PASS${NC}: --agent MergeMill-dev created the right file"
  PASS=$((PASS + 1))
else
  echo -e "  ${RED}FAIL${NC}: --agent MergeMill-dev did NOT create the expected file"
  FAIL=$((FAIL + 1))
fi
