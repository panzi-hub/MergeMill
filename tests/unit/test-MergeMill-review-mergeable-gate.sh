#!/bin/bash
# test-MergeMill-review-mergeable-gate.sh — issue #176 / INV-44.
#
...
#      wrapper.
#
# Run: bash tests/unit/test-MergeMill-review-mergeable-gate.sh
...
MG_LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-mergeable.sh"
...
if [[ -f "$MG_LIB" ]]; then
  # shellcheck source=../../skills/MergeMill-dispatcher/scripts/lib-review-mergeable.sh
  source "$MG_LIB"
...
# Reuse the dev-resume rebase hook: MergeMill-dev.sh greps PR comments for a
