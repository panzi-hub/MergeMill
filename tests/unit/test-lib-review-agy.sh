...
LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-agy.sh"
...
if bash -n "$WRAPPER" 2>/dev/null; then
  echo -e "  ${GREEN}PASS${NC}: TC-AGYQ-SRC-06b MergeMill-review.sh parses (bash -n)"; PASS=$((PASS + 1))
else
  echo -e "  ${RED}FAIL${NC}: TC-AGYQ-SRC-06b MergeMill-review.sh fails bash -n"; FAIL=$((FAIL + 1))
fi
