...
#   MergeMill-review.sh used to rebind AGENT_CMD="$AGENT_REVIEW_CMD"
#   AFTER `source lib-agent.sh` but BEFORE `source lib-auth.sh`.
#   lib-auth.sh transitively re-sources MergeMill.conf via
#   lib-config.sh::load_MergeMill_conf, and the conf's unconditional
...
DEV_WRAPPER="$SCRIPTS_DIR/MergeMill-dev.sh"
REVIEW_WRAPPER="$SCRIPTS_DIR/MergeMill-review.sh"
...
# -----------------------------------------------------------------------
# Build a sandbox project whose MergeMill.conf mimics podcast-curation
...
build_sandbox() {
  ...
  cat >"$sandbox/MergeMill.conf" <<'CONF'
...
KIRO_AGENT_NAME="MergeMill-review"
AGENT_TIMEOUT="4h"
...
cat >>"$SANDBOX2/MergeMill.conf" <<'CONF'
