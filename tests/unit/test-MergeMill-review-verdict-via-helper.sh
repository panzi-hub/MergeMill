#!/bin/bash
# test-MergeMill-review-verdict-via-helper.sh — issue #202 / INV-56.
#
...
#
# Strategy (mirrors test-MergeMill-review-prompt.sh):
...
#
# Run: bash tests/unit/test-MergeMill-review-verdict-via-helper.sh
...
_RESOLVE_LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-resolve.sh"
...
_ARTIFACT_LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-artifact.sh"
...
_PROVIDER_PROMPTS_LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-provider-prompts.sh"
SANDBOX_OUT=$(
  set +e
  ...
  # this, the ambient PROJECT_ID from whatever project's MergeMill.conf
