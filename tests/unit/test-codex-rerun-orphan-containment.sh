...
#   - Layer 3b: the rc-sidecar write in MergeMill-review.sh is guarded on
...
POLL_LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-poll.sh"
...
(
  ...
  # controller subshell exactly the way MergeMill-review.sh's fan-out loop
...
(
  ...
  export MERGEMILL_CONF_DIR="$E2E_TMPROOT" PROJECT_DIR="$E2E_TMPROOT" PROJECT_ID=e2e406
  ...
  # does BEFORE launch (MergeMill-review.sh: `export ADT_FANOUT_LANE_MARKER=...`).
  export ADT_FANOUT_LANE_MARKER="$E2E_MARKER"
  # shellcheck disable=SC1090
  source "$CODEX_ADAPTER" 2>/dev/null
  # shellcheck disable=SC1090
  source "$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-agent.sh" 2>/dev/null
...
if [[ "$_e2e_started" -eq 1 ]]; then
  ...
  # MergeMill-review.sh's call site byte-for-byte, INV-112 order) ---
  (
    # shellcheck disable=SC1090
    source "$POLL_LIB"
    log() { :; }
