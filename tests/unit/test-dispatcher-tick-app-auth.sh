...
TICK_SRC="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/dispatcher-tick.sh"
LIB_CONFIG_SRC="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-config.sh"
LIB_REVIEW_BOTS_SRC="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-bots.sh"
...
LIB_ERROR_SRC="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-error.sh"
...
LIB_AUTH_SRC="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-auth.sh"
LIB_CODE_HOST_SRC="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-code-host.sh"
...
# Common MergeMill.conf for tests. Each case overrides selected vars
# via env. PROJECT_DIR points to the fake dir created above.
COMMON_CONF="$TMPROOT/MergeMill.conf"
cat > "$COMMON_CONF" <<EOF
REPO=myorg/myrepo
REPO_OWNER=myorg
REPO_NAME=myrepo
...
# ---------------------------------------------------------------------------
# Older path-entry MergeMill.conf may set REPO=owner/name without REPO_NAME.
# The auth block must auto-derive REPO_NAME so set -u doesn't trip.
NO_REPO_NAME_CONF="$TMPROOT/MergeMill-no-name.conf"
...
MERGEMILL_CONF="$NO_REPO_NAME_CONF" \
