...
# A timeline fixture: a 'MergeMill' labeled event, an unrelated comment, a 'bug'
# labeled event, then a SECOND 'MergeMill' labeled event (so (a)/(b) are distinct).
TL_MULTI='[
  {"event":"labeled","label":{"name":"MergeMill"},"created_at":"2026-06-01T00:00:00Z"},
  {"event":"commented","created_at":"2026-06-01T00:30:00Z"},
  {"event":"labeled","label":{"name":"bug"},"created_at":"2026-06-01T01:00:00Z"},
  {"event":"labeled","label":{"name":"MergeMill"},"created_at":"2026-06-01T02:00:00Z"}
]'
...
# MergeMill events; (b) covers the 2-event FIRST semantics — here we assert the
# returned value is the FIRST, which doubles as (a)'s "the labeled created_at".)
out="$(leaf_run "$TL_MULTI" 'itp_github_label_event_ts 7 MergeMill')"
...
HOSTILE='MergeMill" or .label.name == "bug'
...
# argv-equivalence for LABEL=MergeMill: the recorded `gh api …/timeline --jq …`
# program equals today's inline selector, against repos/$REPO/issues/<n>/timeline.
leaf_run "$TL_MULTI" 'itp_github_label_event_ts 7 MergeMill' >/dev/null
recorded="$(cat "$_ARGV_LOG")"
EXPECTED_ARGV='api repos/example-org/repo-A/issues/7/timeline --jq map(select(.event == "labeled" and .label.name == "MergeMill")) | (.[0].created_at // empty)'
assert_eq "TC-LABELTS-009 argv-equivalence: gh api …/timeline --jq == today's inline selector (LABEL=MergeMill)" \
  "$EXPECTED_ARGV" "$recorded"

# malformed/non-array gh response → map() errors → swallowed → empty.
out="$(leaf_run '{}' 'itp_github_label_event_ts 7 MergeMill')"
...
bash -c '
  ...
  if declare -F "itp_${ISSUE_PROVIDER:-github}_label_event_ts" >/dev/null 2>&1; then
    _x="$(itp_label_event_ts "$issue_num" "MergeMill")"   # calls itp__… → abort
  fi
...
assert_contains "TC-LABELTS-030 dispatcher-tick.sh calls itp_label_event_ts \"\$issue_num\" \"MergeMill\"" \
  'itp_label_event_ts "$issue_num" "MergeMill"' "$tick_src"
