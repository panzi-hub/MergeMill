...
DEV_WRAPPER="$SCRIPTS/MergeMill-dev.sh"
...
_seam_trace_one() {
  ...
  env -u MERGEMILL_PROVIDERS_DIR ISSUE_PROVIDER=github _ARGV_FILE="$argv_file" \
...
_seam_trace_one "MergeMill-review.sh no-auto-close gate" \
...
_seam_trace_one "MergeMill-dev.sh primary fetch" \
  'ISSUE_NUMBER=42; ISSUE_BODY=$(itp_read_task "$ISSUE_NUMBER" title,body,comments)'
_seam_trace_one "MergeMill-dev.sh resume-fallback fetch" \
...
_grep_caller_clean "$DEV_WRAPPER" "MergeMill-dev.sh"
_grep_caller_clean "$REVIEW_WRAPPER" "MergeMill-review.sh"
...
grep -qF 'ISSUE_BODY=$(itp_read_task "$ISSUE_NUMBER" title,body,comments)' "$DEV_WRAPPER" \
  && ok "source-pin: MergeMill-dev.sh primary fetch is abstract" \
  || bad "source-pin: MergeMill-dev.sh primary fetch drifted from the abstract form"
grep -qF 'ISSUE_BODY=$(itp_read_task "$ISSUE_NUMBER" title,body)' "$DEV_WRAPPER" \
  && ok "source-pin: MergeMill-dev.sh resume fetch is abstract" \
  || bad "source-pin: MergeMill-dev.sh resume fetch drifted from the abstract form"
grep -qF 'HAS_NO_AUTO_CLOSE=$(itp_read_task "$ISSUE_NUMBER" labels \' "$REVIEW_WRAPPER" \
  && ok "source-pin: MergeMill-review.sh no-auto-close read is abstract" \
  || bad "source-pin: MergeMill-review.sh no-auto-close read drifted"
...
_GH_VIEW_PAYLOAD='{"title":"T","body":"B","state":"OPEN","labels":[{"name":"MergeMill"},{"name":"no-auto-close"}]}'
...
assert_eq "leaf normalizes labels to a name-string array (not {name} objects)" \
  '["MergeMill","no-auto-close"]' "$(jq -c '.labels' <<<"$out")"
...
_GH_VIEW_PAYLOAD='{"title":"t","body":"b","state":"OPEN","labels":[{"name":"MergeMill"}]}'
