#!/bin/bash
# test-MergeMill-dev-pushed-no-pr-resume.sh — verify MergeMill-dev.sh's
...
#      source-of-truth greps (same approach as test-MergeMill-dev-rebase-marker.sh).
#
# Run: bash tests/unit/test-MergeMill-dev-pushed-no-pr-resume.sh
...
# Extract needs_open_pr_only() from MergeMill-dev.sh. Function spans from
