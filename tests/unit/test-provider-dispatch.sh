...
routed=$(
  env -u ISSUE_PROVIDER -u CODE_HOST -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
  bash -c '
    ...
    itp_list_by_state MergeMill
    chp_find_pr_for_issue 42 number,body
  '
)
assert_contains "default ISSUE_PROVIDER=github: itp_list_by_state → itp_github_list_by_state" "ITP-GH-ROUTED:MergeMill" "$routed"
...
# the fixture dir via MERGEMILL_PROVIDERS_DIR — then assert itp_caps/chp_caps
...
fake_caps=$(
  env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
      ISSUE_PROVIDER=degraded CODE_HOST=degraded MERGEMILL_PROVIDERS_DIR="$FAKE_PROVIDER" \
  bash -c '
    source "'"$ITP_LIB"'" 2>/dev/null
    source "'"$CHP_LIB"'" 2>/dev/null
    # Public seam: itp_caps/chp_caps resolve providers/itp-${ISSUE_PROVIDER}.caps
    # / chp-${CODE_HOST}.caps from MERGEMILL_PROVIDERS_DIR (the fixture dir).
...
# ---------------------------------------------------------------------------
echo ""
echo "=== TC-PROVIDER-DISPATCH-031: MERGEMILL_PROVIDERS_DIR override semantics (provider-selection is the seam) ==="
# ---------------------------------------------------------------------------
...
no_override=$(
  env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR -u MERGEMILL_PROVIDERS_DIR \
...
assert_contains "no MERGEMILL_PROVIDERS_DIR: degraded provider not on skill-tree path → itp_caps rc 1" "RC=1" "$no_override"
...
default_via_override=$(
  env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
      ISSUE_PROVIDER=github MERGEMILL_PROVIDERS_DIR="$PROVIDERS" bash -c '
    source "'"$ITP_LIB"'" 2>/dev/null
    echo "MC=$(itp_caps marker_channel)"
  '
)
assert_contains "MERGEMILL_PROVIDERS_DIR=<skill-tree providers/> + github → itp_caps marker_channel=html (default path unchanged)" "MC=html" "$default_via_override"
...
TMP_TREE=$(mktemp -d)
SKILL_TREE="$TMP_TREE/skill/MergeMill-dispatcher/scripts"
mkdir -p "$SKILL_TREE"
