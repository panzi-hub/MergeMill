...
COMMON_SCRIPTS="$PROJECT_ROOT/skills/MergeMill-common/scripts"
...
assert_contains "TC-CHP-FINDPR chp_find_pr_for_issue -F repo=MergeMill-dev-team"       "repo=MergeMill-dev-team" "$argv"
...
assert_contains "TC-CHP-THREADS -F repo derived from REPO" "repo=MergeMill-dev-team" "$argv"
...
if [[ -f "$_RESOLVER_SH" ]]; then
  resolver_out=$(
    env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR REPO="$REPO" \
      ...
      bash '$_RESOLVER_SH' zxkane MergeMill-dev-team 42 2>&1
    "
  )
  ...
  fc_resolver_out=$(
    env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR REPO="$REPO" \
      ...
      chp_resolve_thread() { printf 'true'; }
      export -f gh chp_resolve_thread 2>/dev/null || true
      bash '$_RESOLVER_SH' zxkane MergeMill-dev-team 42
      echo \"|RC=\$?|\"
    " 2>&1
  )
...
if [[ -d "$FAKE_PROVIDER" ]]; then
  fake=$(
    env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
        CODE_HOST=degraded MERGEMILL_PROVIDERS_DIR="$FAKE_PROVIDER" \
    bash -c '
      source "'"$CHP_LIB"'" 2>/dev/null
  ...
  rc_skip=$(
    env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
        REPO="$REPO" CODE_HOST=degraded MERGEMILL_PROVIDERS_DIR="$FAKE_PROVIDER" \
    bash -c '
      log() { :; }
  ...
  env -u PROJECT_DIR REPO="$REPO" MERGEMILL_CONF_DIR="$COMMON_SCRIPTS" \
      CODE_HOST=degraded MERGEMILL_PROVIDERS_DIR="$FAKE_PROVIDER" \
  ...
  # native_issue_pr_link=1. This mirrors MergeMill-dev.sh's _render_close_keyword
  ...
  gh_kw=$(
    env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR REPO="$REPO" CODE_HOST=github \
  ...
  if grep -qE '_render_close_keyword\(\)' "$SCRIPTS/MergeMill-dev.sh" \
     && grep -qE 'chp_caps merge_closes_issue' "$SCRIPTS/MergeMill-dev.sh"; then
    echo -e "  ${GREEN}PASS${NC}: TC-CHP-LEAF-GUARD-SRC MergeMill-dev.sh defines a caps-aware _render_close_keyword"; PASS=$((PASS+1))
  else
    echo -e "  ${RED}FAIL${NC}: TC-CHP-LEAF-GUARD-SRC MergeMill-dev.sh missing caps-aware _render_close_keyword"; FAIL=$((FAIL+1))
  fi
else
...
# MergeMill-review.sh (the merge success path + the mandatory-bot-review wait
...
echo "=== caps=0 caller branches present in MergeMill-review.sh (§4.2) ==="
REVIEW_WRAPPER="$SCRIPTS/MergeMill-review.sh"
...
# Behavioral: the EXACT 3-way branch from MergeMill-review.sh.
_mci0_branch='
    _merge_closes=1
...
bt_sentinel=$(mktemp)
env -u PROJECT_DIR REPO="$REPO" MERGEMILL_CONF_DIR="$COMMON_SCRIPTS" BTFILE="$btfile" SENTINEL="$bt_sentinel" \
  bash -c '
    log() { :; }
...
_ac_files="lib-dispatch.sh MergeMill-dev.sh MergeMill-review.sh lib-review-request-changes.sh lib-review-bots.sh resolve-threads.sh"
exec_gh=$(
...
routed=$(
  env -u CODE_HOST -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR bash -c '
    source "'"$CHP_LIB"'" 2>/dev/null
