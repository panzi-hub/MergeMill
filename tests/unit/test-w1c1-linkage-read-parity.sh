...
# `chp_pr_list` sites in `MergeMill-dev.sh` / `lib-auth.sh`.
#
...
# carried the guard; the other four (:844 in MergeMill-dev.sh, :1174 in
# MergeMill-dev.sh, and both lib-auth.sh sites at :454/:610) did NOT — the
...
LIB_DISP="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-dispatch.sh"
...
# shellcheck source=../../skills/MergeMill-dispatcher/scripts/lib-dispatch.sh
source "$LIB_DISP"
set +e
...
# needs_open_pr_only (MergeMill-dev.sh:438, length; fail-CLOSED on read error)
...
# PR_EXISTS (MergeMill-dev.sh:844, length; fail-soft; used to be UNGUARDED #148 hazard)
...
# _pr_created_at (MergeMill-dev.sh:943, earliest .createdAt)
...
# PR_NUM (MergeMill-dev.sh:1174, .[0].number; used to be UNGUARDED #148 hazard)
...
_DEV_SH="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh"
_AUTH_SH="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-auth.sh"
...
# `$"…"` early).
_pin_source "SRC-PIN-DEV-LEN MergeMill-dev.sh needs_open_pr_only length shape"    "$_DEV_SH"  '[.[] | select((.body | test(\"#${issue_num}[^0-9]\")) or (.body | test(\"#${issue_num}\$\")))] | length'
_pin_source "SRC-PIN-DEV-LEN MergeMill-dev.sh PR_EXISTS length shape"              "$_DEV_SH"  '[.[] | select((.body | test(\"#${ISSUE_NUMBER}[^0-9]\")) or (.body | test(\"#${ISSUE_NUMBER}\$\")))] | length'
...
# Number shape (2 sites — MergeMill-dev.sh PR_NUM, lib-auth.sh pr_number).
_pin_source "SRC-PIN-DEV-NUM MergeMill-dev.sh PR_NUM first-number shape"           "$_DEV_SH"  '[.[] | select((.body | test(\"#${ISSUE_NUMBER}[^0-9]\")) or (.body | test(\"#${ISSUE_NUMBER}\$\")))] | .[0].number // empty'
...
# createdAt shape (metrics site, MergeMill-dev.sh _pr_created_at).
_pin_source "SRC-PIN-DEV-CA MergeMill-dev.sh _pr_created_at earliest-createdAt shape" "$_DEV_SH" '[.[] | select((.body | test(\"#${ISSUE_NUMBER}[^0-9]\")) or (.body | test(\"#${ISSUE_NUMBER}\$\")))] | sort_by(.createdAt) | (.[0].createdAt // empty)'
...
_CALLERS="skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh skills/MergeMill-dispatcher/scripts/MergeMill-review.sh skills/MergeMill-dispatcher/scripts/lib-auth.sh skills/MergeMill-dispatcher/scripts/lib-pr-linkage.sh skills/MergeMill-dispatcher/scripts/lib-dispatch.sh"
...
_run_walker() {
  ...
  env -u CODE_HOST -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
