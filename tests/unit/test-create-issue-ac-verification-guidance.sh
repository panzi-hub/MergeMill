...
# split a true post-merge AC into a NON-blocking, NON-MergeMill follow-up — so
# the MergeMill dev/review loop is never handed a blocking criterion it cannot
...
assert_contains_ci "TC-ACV-005a follow-up must NOT be MergeMill: 'do NOT add'" \
  "$acv_all" "do NOT add"
assert_contains "TC-ACV-005b follow-up rule mentions 'MergeMill'" \
  "$acv_all" "MergeMill"
