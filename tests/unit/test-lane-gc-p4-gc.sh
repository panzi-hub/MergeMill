...
LANE4_DIR=$(find "$ST4/MergeMill-p1/lanes" -maxdepth 1 -type d -not -name '.pending-*' -not -name lanes 2>/dev/null | head -1)
...
LANE7_STILL_THERE=$(find "$ST7/MergeMill-p1/lanes" -maxdepth 1 -type d -not -name lanes 2>/dev/null | wc -l)
...
LANE8_STILL_THERE=$(find "$ST8/MergeMill-p1/lanes" -maxdepth 1 -type d -not -name lanes 2>/dev/null | wc -l)
...
ST9=$(mktemp -d)
mkdir -p "$ST9/MergeMill-p1/lanes/p1.dev.9.111.aaaa"
echo "GARBAGE NOT KV" > "$ST9/MergeMill-p1/lanes/p1.dev.9.111.aaaa/lane"
touch -d "@$(( $(date +%s) - 90000 ))" "$ST9/MergeMill-p1/lanes/p1.dev.9.111.aaaa/lane" 2>/dev/null || true
run_gc_kill "$ST9" >/dev/null
STILL9=$(find "$ST9/MergeMill-p1/lanes" -maxdepth 1 -type d -not -name lanes 2>/dev/null | wc -l)
assert_eq "TC-LGC4-009: unparseable lane, age>24h -> collected/removed" "0" "$STILL9"
rm -rf "$ST9"

ST10=$(mktemp -d)
mkdir -p "$ST10/MergeMill-p1/lanes/p1.dev.10.222.bbbb"
echo "GARBAGE NOT KV" > "$ST10/MergeMill-p1/lanes/p1.dev.10.222.bbbb/lane"
run_gc_kill "$ST10" >/dev/null
STILL10=$(find "$ST10/MergeMill-p1/lanes" -maxdepth 1 -type d -not -name lanes 2>/dev/null | wc -l)
...
ST11=$(mktemp -d)
mkdir -p "$ST11/MergeMill-p1/lanes/.pending-p1.dev.11.333.cccc"
touch -d "@$(( $(date +%s) - 90000 ))" "$ST11/MergeMill-p1/lanes/.pending-p1.dev.11.333.cccc" 2>/dev/null || true
run_gc_kill "$ST11" >/dev/null
STILL11=$(find "$ST11/MergeMill-p1/lanes" -maxdepth 1 -type d -name '.pending-*' 2>/dev/null | wc -l)
assert_eq "TC-LGC4-011: .pending-* orphan aged 24h+ is rm -rf'd" "0" "$STILL11"
rm -rf "$ST11"

ST12=$(mktemp -d)
mkdir -p "$ST12/MergeMill-p1/lanes/.pending-p1.dev.12.444.dddd"
run_gc_kill "$ST12" >/dev/null
STILL12=$(find "$ST12/MergeMill-p1/lanes" -maxdepth 1 -type d -name '.pending-*' 2>/dev/null | wc -l)
...
ST21=$(mktemp -d)
cat > "$P2ROOT/MergeMill.conf" <<'CONF'
PROJECT_ID="p2"
CONF
# -u ADT_LANE_ID: this test suite itself runs inside a real dev-agent
# session that already exports ADT_LANE_ID (and MERGEMILL_CONF_LOADED_FROM/
...
setsid env -u ADT_LANE_ID -u TERM_PROGRAM bash -c '
  export MERGEMILL_CONF_LOADED_FROM="'"$P2ROOT"'/MergeMill.conf"
...
# the legacy signature (MERGEMILL_CONF_LOADED_FROM + CC_USER=MergeMill-
# dev-bot, ppid==1 via genuine kernel reparenting through an intermediate
# that backgrounds it and exits) that shares the STILL-LIVE `MergeMill-
...
FAKE_WRAPPER26="$P2ROOT/MergeMill-dev.sh"
DAEMON26_SCRIPT="$P2ROOT/legacy-daemon-26.sh"
cat > "$DAEMON26_SCRIPT" <<'EOF'
#!/bin/bash
export MERGEMILL_CONF_LOADED_FROM=/fake/MergeMill.conf
export CC_USER=MergeMill-dev-bot
unset TERM_PROGRAM
...
FAKE_WRAPPER26B="$P2ROOT/MergeMill-dev-26b.sh"
...
# (rule 2.3).
FAKE_WRAPPER="$P2ROOT/MergeMill-dev.sh"
...
if [[ -n "$CHILD29_PID" ]]; then
  ...
  HAS_LIVE=$(bash -c '
    source "'"$LIB_LANE"'"
    _gc_group_has_live_wrapper() { local pg="$1"; pgrep -g "$pg" -f "MergeMill-(dev|review)\.sh" 2>/dev/null | grep -vqw "$$"; }
...
# argv does NOT literally contain `MergeMill-dev.sh` (simulating an
# exec-chain launcher bridge), but a LIVE MEMBER of its OWN process group
# does. Rule 2.3's `pgrep -g $PG -f 'MergeMill-(dev|review)\.sh'` matches
...
ST28=$(mktemp -d)
LAUNCHER_BRIDGE_MEMBER="$P2ROOT/MergeMill-dev-launcher-member.sh"
cat > "$LAUNCHER_BRIDGE_MEMBER" <<'EOF'
#!/bin/bash
exec -a MergeMill-dev.sh sleep 400
EOF
...
bash -c '
  ...
  LANE_DIR=$(lane_install p2 "$LANE_ID")
  lane_set "$LANE_DIR" WRAPPER_PID 999999
  # Bridge leader: its OWN argv does NOT contain MergeMill-dev.sh (the
...
if command -v jq >/dev/null 2>&1; then
  MERGEMILL_METRICS_DIR="$ST83/metrics" ADT_STATE_ROOT="$ST83" bash "$ADT_GC" --dry-run >/dev/null 2>&1
...
#!/bin/bash
if [[ "$1" == "-l" ]]; then
  echo "*/10 * * * * bash /some/path/adt-gc.sh # adt-gc-timer (MergeMill-dev-team Lane-GC series, do not edit — managed by install-gc-timer.sh)"
  exit 0
fi
exit 1
EOF
...
cat > "$TESTPROJ/scripts/MergeMill.conf" <<CONF
PROJECT_ID="tc121"
REPO="test/test"
REPO_OWNER="test"
REPO_NAME="test"
...
cat > "$TESTPROJ/scripts/MergeMill-dev.sh" <<'EOF'
#!/bin/bash
echo "stub-dev-wrapper-ran"
sleep 3
exit 0
EOF
chmod +x "$TESTPROJ/scripts/MergeMill-dev.sh"
