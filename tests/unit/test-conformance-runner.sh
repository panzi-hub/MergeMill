...
# MERGEMILL_CONF), an empty-env fixture must STILL pass — the runner scrubs them
...
AGENT_DEV_EXTRA_ARGS='--x' AGENT_REVIEW_EXTRA_ARGS='--y' AGENT_LAUNCHER='cc' \
  AGENT_DEV_LAUNCHER='cc' AGENT_REVIEW_LAUNCHER='cc' MERGEMILL_CONF='/tmp/nonexistent-conformance.conf' \
...
# dc696d40): scrubbing MERGEMILL_CONF alone is NOT enough. lib-agent.sh's
# load_MergeMill_conf has THREE discovery branches — MERGEMILL_CONF (file),
# then MERGEMILL_CONF_DIR/MergeMill.conf, then PROJECT_DIR/scripts/MergeMill.conf
# — all read at SOURCE time. A caller with MERGEMILL_CONF_DIR (or PROJECT_DIR)
...
# must scrub BOTH MERGEMILL_CONF_DIR and PROJECT_DIR inside the classification
...
# A PROJECT_DIR-shaped tree: <root>/scripts/MergeMill.conf (branch 3).
...
cat > "$POISON_CONF_DIR/MergeMill.conf" <<'CONF'
...
cp "$POISON_CONF_DIR/MergeMill.conf" "$POISON_PROJECT_DIR/scripts/MergeMill.conf"

# 035f — MERGEMILL_CONF_DIR leak (branch 2). NOTE: no `env -u PROJECT_DIR`.
confdir_leak_out="$(MERGEMILL_CONF_DIR="$POISON_CONF_DIR" bash "$RUNNER" --adapter claude --mode dev-new 2>/dev/null)"
assert_contains "TC-CONFORMANCE-035f inherited MERGEMILL_CONF_DIR conf does NOT leak (claude dev-new still PASS)" \
...
confdisc_heavy_rc=0
MERGEMILL_CONF_DIR="$POISON_CONF_DIR" PROJECT_DIR="$POISON_PROJECT_DIR" \
...
# `${MERGEMILL_CONF_DIR:-$_LIB_AGENT_DIR}` default leak; see the runner comment).
...
assert_grep "TC-CONFORMANCE-035i runner points MERGEMILL_CONF at a conf-free path" \
  'export MERGEMILL_CONF="\$no_conf_dir/' "$RUNNER"
assert_grep "TC-CONFORMANCE-035j runner points MERGEMILL_CONF_DIR at the conf-free dir" \
  'export MERGEMILL_CONF_DIR="\$no_conf_dir"' "$RUNNER"
...
assert_grep "TC-CONFORMANCE-035o runner resets KIRO_AGENT_NAME + AGENT_TIMEOUT to lib defaults" \
  'export KIRO_AGENT_NAME="MergeMill-dev"' "$RUNNER"
...
# mislabeled config — matching production (MergeMill-review.sh passes the rc).
