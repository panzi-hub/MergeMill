...
# its $MERGEMILL_CONF env on each invocation. The wrapper sees the stub
# via $SCRIPT_DIR/dispatcher-tick.sh resolution, since the test runs the
# wrapper from a sandbox copy of skills/MergeMill-dispatcher/scripts/.
#
...
# defaults there; MERGEMILL_TRUST_CONF=1 disables the gate.
export MERGEMILL_TRUST_CONF=1
SANDBOX="$TMPROOT/scripts"
mkdir -p "$SANDBOX"
cp "$WRAPPER_SRC" "$SANDBOX/dispatcher-multi-tick.sh"

# Stub dispatcher-tick.sh: appends "$MERGEMILL_CONF\n" to a sentinel file
...
#!/bin/bash
echo "$MERGEMILL_CONF" >> "$TICK_RECORD_FILE"
override_var="TICK_RC_FOR_$(basename "$MERGEMILL_CONF" | tr -c 'A-Za-z0-9' '_')"
rc=${!override_var:-0}
exit "$rc"
EOF
...
# ---------------------------------------------------------------------------
echo ""
echo "=== TC-MP-002: MERGEMILL_CONF env propagated per iteration ==="
# ---------------------------------------------------------------------------
expected="$TMPROOT/proj-a.conf
$TMPROOT/proj-b.conf
$TMPROOT/proj-c.conf"
assert_eq "MERGEMILL_CONF recorded in PROJECTS order" "$expected" "$(cat "$RECORD")"
...
# execution. Verify the trust gate fires when MERGEMILL_TRUST_CONF is unset
...
[ "$rc" -ne 0 ] && {
  ...
  echo -e "  ${RED}FAIL${NC}: expected non-zero rc when MERGEMILL_TRUST_CONF unset"
  FAIL=$((FAIL + 1))
}
case "$(cat "$stderr_log")" in
...
# pass MERGEMILL_CONF as a per-iteration env, and not exit on per-project
# failure. Detect drift via grep.
if grep -q '^\s*if ( MERGEMILL_CONF=' "$WRAPPER_SRC" \
   && grep -q 'bash "\$SCRIPT_DIR/dispatcher-tick.sh"' "$WRAPPER_SRC"; then
  echo -e "  ${GREEN}PASS${NC}: loop runs dispatcher-tick.sh in subshell with MERGEMILL_CONF env"
  PASS=$((PASS + 1))
else
