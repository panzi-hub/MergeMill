...
#
# Strategy: source the lib, point MERGEMILL_METRICS_DIR at a temp dir, exercise
...
LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-metrics.sh"
...
# shellcheck source=../../skills/MergeMill-dispatcher/scripts/lib-metrics.sh
source "$LIB"
...
WORK="$(mktemp -d)"
export MERGEMILL_METRICS_DIR="$WORK"
...
# TC-METRICS-009: MERGEMILL_METRICS_DIR override targets that exact dir.
assert_eq "TC-METRICS-009 override dir used" "$WORK" "$(metrics_dir)"

# TC-METRICS-010: XDG_STATE_HOME path, no override.
unset MERGEMILL_METRICS_DIR
XS="$(mktemp -d)"
assert_eq "TC-METRICS-010 XDG_STATE_HOME path" "$XS/MergeMill-testproj" "$(XDG_STATE_HOME="$XS" metrics_dir)"
...
# $HOME/.local/state/MergeMill-<project> — NOT the volatile XDG_RUNTIME_DIR
# (where pid_dir_for_project would have put it, losing retention on reboot).
FAKE_HOME="$(mktemp -d)"
FAKE_RUNTIME="$(mktemp -d)"
_md="$(env -u MERGEMILL_METRICS_DIR -u XDG_STATE_HOME HOME="$FAKE_HOME" XDG_RUNTIME_DIR="$FAKE_RUNTIME" PROJECT_ID=testproj \
        bash -c 'source "'"$LIB"'"; metrics_dir')"
assert_eq "TC-METRICS-011 durable fallback uses HOME/.local/state" "$FAKE_HOME/.local/state/MergeMill-testproj" "$_md"
...
( unset PROJECT_ID MERGEMILL_METRICS_DIR XDG_STATE_HOME XDG_RUNTIME_DIR HOME
...
rc_observed="$(bash -c '
    set -euo pipefail
    source "'"$LIB"'"
    export MERGEMILL_METRICS_DIR="'"$RO"'/cannot-create"
    export PROJECT_ID=x
...
DEV_W="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh"
REV_W="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/MergeMill-review.sh"
DISP_W="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/dispatcher-tick.sh"
unguarded=0
...
# the key names. Splice exactly as MergeMill-dev.sh does (`$_tok` word-split).
INTG="$(mktemp -d)"
( export MERGEMILL_METRICS_DIR="$INTG" PROJECT_ID=intg
...
_rep="$(bash "$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/metrics-report.sh" --file "$INTG/metrics.jsonl" 2>&1)"
...
# MergeMill-dev.sh; replicate it here so a regression on the wrapper line trips.
_missing_off="$(bash -c '
    set -euo pipefail
...
assert_contains "TC-METRICS-074 wrapper guards the offset capture with [[ -f ]]" \
  'if [[ -f "$LOG_FILE" ]]; then' \
  "$(grep -A1 'METRICS_LOG_OFFSET=0' "$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh" | head -2 | tr '\n' ' ')"
...
_prune_rc="$(bash -c '
    set -euo pipefail
    source "'"$LIB"'"
    export MERGEMILL_METRICS_DIR="'"$PRUNE_WORK"'" PROJECT_ID=collprune
...
if command -v flock >/dev/null 2>&1; then
  ...
  _race_prune_pid=$!
  ( export MERGEMILL_METRICS_DIR="$RACE_WORK" PROJECT_ID=race
