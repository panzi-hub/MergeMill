...
#
# Verifies the (a) prompt-prose sites are GONE from MergeMill-dev.sh /
# MergeMill-review.sh / lib-review-bots.sh (replaced by
...
# ---------------------------------------------------------------------------
echo "=== TC-P36-020: MergeMill-dev.sh raw-gh count == 1 (only the command -v gh probe, (b)) ==="
# ---------------------------------------------------------------------------
n=$(count_raw_gh "$DEV_SH")
if [[ "$n" -eq 1 ]] && grep -qF 'command -v gh &>/dev/null' "$DEV_SH"; then
  ok "TC-P36-020 MergeMill-dev.sh raw-gh count is 1 (the command -v gh presence probe)"
else
...
# ---------------------------------------------------------------------------
echo ""
echo "=== TC-P36-021: MergeMill-review.sh raw-gh count == 4 (gh api user fallback + 2 WARNs + INV-33 close, (b)) ==="
# ---------------------------------------------------------------------------
...
if [[ "$n" -eq 4 ]] \
  ...
  ok "TC-P36-021 MergeMill-review.sh raw-gh count is 4 (bot-login fallback + WARNs + INV-33 close)"
else
...
ad=$(jq -r '[.surviving_sites[] | select(.file=="MergeMill-dev.sh")] | length' "$BASELINE")
ar=$(jq -r '[.surviving_sites[] | select(.file=="MergeMill-review.sh")] | length' "$BASELINE")
...
if [[ "$total" -eq 13 ]] && [[ "$la" -eq 5 ]] && [[ "$cpc" -eq 3 ]] && [[ "$ad" -eq 1 ]] && [[ "$ar" -eq 4 ]] && [[ "$lrb" -eq 0 ]]; then
  ok "TC-P36-030/031 baseline shrank to 13 (lib-auth=5, check-provider-cutover=3, MergeMill-dev=1, MergeMill-review=4, lib-review-bots=0)"
else
  bad "TC-P36-030/031 baseline breakdown mismatch: total=$total lib-auth=$la check-provider-cutover=$cpc MergeMill-dev=$ad MergeMill-review=$ar lib-review-bots=$lrb"
fi
...
# Inject a brand-new, never-baselined raw-gh line into a copy of MergeMill-dev.sh.
printf '\n# synthetic injection for TC-P36-034\nSYNTH_TC_P36_034=$(gh issue view 999999 --json body)\n' >> "$WORK/scripts/MergeMill-dev.sh"
...
if [[ $rc -ne 0 ]] && [[ "$out" == *"NEW/unbaselined raw-gh"* ]] && [[ "$out" == *"MergeMill-dev.sh"* ]]; then
  ok "TC-P36-034 synthetic new raw-gh FAILs Check 1 loud, naming MergeMill-dev.sh"
else
