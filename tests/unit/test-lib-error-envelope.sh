...
#
# Unit tests for skills/MergeMill-dispatcher/scripts/lib-error.sh:
...
LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-error.sh"
...
OUT=$(error_envelope ADT_CFG_MISSING_KEY "PROJECT_ID not set" "conf missing PROJECT_ID" "Set PROJECT_ID in scripts/MergeMill.conf")
...
( set -euo pipefail
  export MERGEMILL_CONF_DIR="$TMPROOT/scripts" REPO="panzi-hub/MergeMill"
  source "$LIB"
...
CALL10B=$(cat "$CALLS")
assert_contains "010b fallback repo is panzi-hub/MergeMill (P1-2)" "panzi-hub/MergeMill" "$CALL10B"
...
if [[ "$REPO_ARG10B" == "panzi-hub/MergeMill" ]]; then
  ok "010b gh received --repo panzi-hub/MergeMill (not empty) (P1-2)"
else
  bad "010b gh --repo arg was '${REPO_ARG10B}', expected panzi-hub/MergeMill (P1-2)"
fi

# TC-ERR-ENVELOPE-011 — post FAILS → rc still 0, log-only degrade.
make_stub_gh 1
( set -euo pipefail
  export MERGEMILL_CONF_DIR="$TMPROOT/scripts" REPO="o/r"
  source "$LIB"
...
# fake dir), and leave MERGEMILL_CONF_DIR unset to force the fallback branch.
