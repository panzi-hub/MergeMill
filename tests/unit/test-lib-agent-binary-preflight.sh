...
# ${MERGEMILL_CONF_DIR}/gh that records issue-comment posts. Driving the full
...
preflight() {
  ...
  ( set -uo pipefail
    export MERGEMILL_CONF_DIR="$TMPROOT/scripts" REPO="o/r" ISSUE_NUMBER="$issue"
...
codex_out=$(
  ( set -uo pipefail
    export MERGEMILL_CONF_DIR="$TMPROOT/scripts" REPO="o/r" ISSUE_NUMBER=231
