...
# (reply-to-comments.sh:41, an MergeMill-common util) behind it.
#
...
#    dir (no MergeMill.conf there → the env REPO survives, no operator-conf
...
be_absent=$(
  env -u PROJECT_DIR -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR \
      -u CODE_HOST MERGEMILL_PROVIDERS_DIR="$_NOLEAF_PROV" PATH="$_BE_SANDBOX:$PATH" \
  bash -c 'unset -f gh; bash "$1" "$2" "$3" "$4" "$5" "$6"' _ \
      "$_BE_SANDBOX/reply-to-comments.sh" o r 5 2734892022 "Fixed in abc" 2>&1
); be_absent_rc=$?
