...
#
# MergeMill-dev.sh::resume builds a `REVIEW_COMMENTS` shell variable from
...
# status comments like "Dispatching MergeMill review" / "Moving to
...
# TC-RFB-003 — real findings, then "Dispatching MergeMill review..." later
fixture=$(jq -n \
  --argjson c1 "$(mk_comment '2026-05-14T01:00:00Z' "$REAL_FINDINGS_R1")" \
  --argjson c2 "$(mk_comment '2026-05-14T01:30:00Z' "$DISPATCH_REVIEW_TOKEN")" \
  '{comments: [$c1, $c2]}')
out=$(run_filter "$fixture")
assert_body_match "TC-RFB-003 real findings preferred over 'Dispatching MergeMill review' status" "[BLOCKING] Missing input validation" "$out"
...
fixture=$(jq -n \
  --argjson c1 "$(mk_comment '2026-05-14T00:30:00Z' "Dispatching MergeMill development...")" \
  --argjson c2 "$(mk_comment '2026-05-14T00:35:00Z' "Resuming MergeMill development...")" \
  '{comments: [$c1, $c2]}')
out=$(run_filter "$fixture")
