...
# the lib (mirroring MergeMill-review.sh's lib-code-host→lib-review-e2e order)
...
export REPO_NAME=MergeMill-dev-team
...
# shellcheck source=../../skills/MergeMill-dispatcher/scripts/lib-issue-provider.sh
source "$ITP_LIB"
set +e
...
_GH_LABEL_PROBE_RC=1
reset_call_log
out=$(itp_github_provision_states MergeMill 0E8A16 "Issue should be processed by MergeMill pipeline")
assert_eq "TC-GT-PROVISION-CREATE create argv with --color hex --description" \
  "label create MergeMill --repo $REPO --color 0E8A16 --description Issue should be processed by MergeMill pipeline" "$(recorded_argv)"
assert_contains "TC-GT-PROVISION-CREATE prints [created]" "[created] 'MergeMill'" "$out"
assert_contains "TC-GT-PROVISION-CREATE probes via gh api repos/*/labels/* --silent" \
  "api repos/$REPO/labels/MergeMill --silent" "$(call_log)"
...
reset_call_log
out=$(itp_github_provision_states MergeMill 0E8A16 "desc")
assert_eq "TC-GT-PROVISION-SKIP REST-probe-only argv (no create emitted)" \
  "api repos/$REPO/labels/MergeMill --silent" "$(recorded_argv)"
assert_contains "TC-GT-PROVISION-SKIP prints [skip]" "[skip] 'MergeMill'" "$out"
...
if gh label view MergeMill --repo "$REPO" &>/dev/null; then
...
reset_call_log
_LABELS_ALL9=(
  "MergeMill|0E8A16|d1" "in-progress|FBCA04|d2" "pending-review|1D76DB|d3"
...
routed=$(
  env -u MERGEMILL_PROVIDERS_DIR ISSUE_PROVIDER=github bash -c '
    set -uo pipefail
...
if [[ -d "$FAKE_PROVIDER" ]]; then
  fake=$(
    env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
        ISSUE_PROVIDER=degraded MERGEMILL_PROVIDERS_DIR="$FAKE_PROVIDER" \
    bash -c '
      source "'"$ITP_LIB"'"
      echo "EDIT=$(itp_caps edit_comment)"
      echo "CHECKBOX=$(itp_caps body_checkbox)"
  ...
  _CAP_FILE="$(mktemp)"
  env -u MERGEMILL_PROVIDERS_DIR ISSUE_PROVIDER=degraded MERGEMILL_PROVIDERS_DIR="$FAKE_PROVIDER" \
  ...
  env -u MERGEMILL_PROVIDERS_DIR ISSUE_PROVIDER=github \
      PR_NUMBER=42 REPO_OWNER=o REPO_NAME=r PR_HEAD_SHA=deadbeef \
      WRAPPER_START_TS=2026-01-01T00:00:00Z REPO=o/r BOT_LOGIN= _CAP_FILE="$_CAP_FILE" \
  bash -c '
    set -uo pipefail
...
GHEOF
  ...
  cb_degraded=$(
    env -u PROJECT_DIR ISSUE_PROVIDER=degraded MERGEMILL_PROVIDERS_DIR="$FAKE_PROVIDER" \
...
chmod +x "$_MIC_ISO/gh"
mic_absent=$(
  env -u PROJECT_DIR -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u MERGEMILL_PROVIDERS_DIR \
...
_HP_FILE="$(mktemp)"
hp_out=$(
  env -u PROJECT_DIR -u MERGEMILL_PROVIDERS_DIR ISSUE_PROVIDER=github REPO=o/r \
      _HP_FILE="$_HP_FILE" \
  bash -c '
    set -uo pipefail
...
for _f in "$LIB" \
          "$SCRIPTS/MergeMill-dev.sh" \
          "$SCRIPTS/MergeMill-review.sh" \
...
#    finds NO MergeMill.conf → the env REPO=o/r survives, no contamination from an
...
# (d) REPO-FALLBACK — self-repo, no MergeMill.conf, REPO/GITHUB_REPO unset → REPO
