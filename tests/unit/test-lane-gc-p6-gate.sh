...
sleep 5
STUB
  chmod +x "$proj/scripts/MergeMill-dev.sh"
...
; do
  ...
  MARKER="$TMPROOT/state-sig-$sig/MergeMill-gatetest/lanes/.defer-issue-${issue}"
  if [[ -f "$MARKER" ]]; then
...
if [[ -f "$STATE010/MergeMill-gatetest/lanes/.defer-issue-9002" ]]; then
...
if [[ -f "$STATE030/MergeMill-gatetest/lanes/.defer-issue-9004" ]]; then
...
mkdir -p "$STATE041/MergeMill-proj041a" "$STATE041/MergeMill-proj041b"
sleep 30 & PID041A=$!
sleep 30 & PID041B=$!
disown 2>/dev/null || true
echo "$PID041A" > "$STATE041/MergeMill-proj041a/issue-1.pid"
echo "$PID041B" > "$STATE041/MergeMill-proj041b/review-2.pid"
echo "999999999" > "$STATE041/MergeMill-proj041b/issue-3.pid"  # dead pid, must not count
OUT041=$(bash -c '
...
mkdir -p "$STATE042/MergeMill-proj042/lanes"
OUT042=$(bash -c '
...
export REPO=panzi-hub/MergeMill REPO_OWNER=zxkane PROJECT_ID=test-lgc6-050 MAX_RETRIES=3 MAX_CONCURRENT=5
...
MARKER070="$STATE070/MergeMill-gatetest/lanes/.defer-issue-9006"
...
ATTEMPT071="$STATE071/MergeMill-gatetest/lanes/.attempt-issue-9007"
...
_drive_snippet_defer_block() {
  ...
  SSM_REMOTE_PROJECT_ID=testproj ADT_STATE_ROOT="$state_root" bash -c "
    ...
    LANE_DIR=\"\${ADT_STATE_ROOT:-\$HOME/.local/state}/MergeMill-\${SSM_REMOTE_PROJECT_ID}/lanes\"
...
OUT090=$(bash -c '
  export REPO=panzi-hub/MergeMill REPO_OWNER=zxkane PROJECT_ID=lgc6-090 MAX_RETRIES=3 MAX_CONCURRENT=5
  source "'"$LIB_DISPATCH"'"
...
OUT100=$(bash -c '
  export REPO=panzi-hub/MergeMill REPO_OWNER=zxkane PROJECT_ID=lgc6-100 MAX_RETRIES=3 MAX_CONCURRENT=5
  source "'"$LIB_DISPATCH"'"
  set +e
...
OUT101=$(bash -c '
  export REPO=panzi-hub/MergeMill REPO_OWNER=zxkane PROJECT_ID=lgc6-101 MAX_RETRIES=3 MAX_CONCURRENT=5
  source "'"$LIB_DISPATCH"'"
  set +e
  export _LIVENESS_CHECK_DRIVER_OVERRIDE="'"$FAKE_DRIVER100"'"
...
HARNESS110="$TMPROOT/harness-110.sh"
{
  ...
  echo 'candidates='"'"'[{"number":9999,"labels":["MergeMill","in-progress"]}]'"'"''
  echo 'cand_count=1'
  echo "$STEP5_BODY"
