...
#
# Why: a typo in MergeMill.conf (e.g. REVIEW_BOTS="q codx") would
...
#
# Strategy: build a sandbox MergeMill.conf with the bad value, run
...
# Sandbox: fake MERGEMILL_CONF with all the required vars so dispatcher-tick
...
write_conf() {
  ...
  cat > "$TMPROOT/MergeMill.conf" <<EOF
PROJECT_ID="testproj"
REPO="owner/repo"
REPO_OWNER="owner"
REPO_NAME="repo"
...
run_tick() {
  : > "$TMPROOT/gh-calls"
  PATH="$BIN:$PATH" \
  MERGEMILL_CONF="$TMPROOT/MergeMill.conf" \
  bash "$TICK" 2>&1
}
...
PATH="$BIN:$PATH" MERGEMILL_CONF="$TMPROOT/MergeMill.conf" \
...
assert_contains "stderr mentions MergeMill.conf" "MergeMill.conf" "$output"
