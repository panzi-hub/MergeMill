...
# ---------------------------------------------------------------------------
S="$(fresh_scratch 005)"
# Append a 2nd identical copy of an already-baselined MergeMill-dev.sh line.
# shellcheck disable=SC2016
...
grep -v 'gh issue close "\$ISSUE_NUMBER" --repo "\$REPO" --reason completed' \
  "$SCRIPTS/MergeMill-review.sh" > "$S/MergeMill-review.sh"
out="$(bash "$CHECK" --scripts-dir "$S" --baseline "$BASELINE" 2>&1)"; rc=$?
...
# ---------------------------------------------------------------------------
# AC #41: every surviving raw-gh in skills/MergeMill-dispatcher/scripts/ must
...
if command -v git >/dev/null 2>&1; then
  ...
  rm -rf "$GROOT"; mkdir -p "$GROOT/skills/MergeMill-dispatcher/scripts"
  cp -rL "$S"/. "$GROOT/skills/MergeMill-dispatcher/scripts/" 2>/dev/null
  ...
  GS="$GROOT/skills/MergeMill-dispatcher/scripts"
...
declare -A FB_SITE=(
  [dev740]="MergeMill-dev.sh|agent never ran, no ISSUE_NUMBER|ISSUE_NUMBER|no ISSUE_NUMBER or gh —"
  [err303]="lib-error.sh|CLI proxy not resolvable|MERGEMILL_CONF_DIR|token-refresh gh proxy not resolvable"
...
# ===========================================================================
echo "=== TC-CUTOVER-GLAB-001: injected raw 'glab issue create' in scratch MergeMill-dev.sh FAILs (#416 R4) ==="
# ===========================================================================
...
# ===========================================================================
echo "=== TC-CUTOVER-GLAB-004: injected '/api/v4' curl in scratch MergeMill-dev.sh FAILs ==="
# ===========================================================================
