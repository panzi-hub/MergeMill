#!/bin/bash
# test-MergeMill-review-request-changes.sh — issue #193 / INV-52.
#
...
#   2. source-of-truth greps against MergeMill-review.sh: the helper is wired
...
#
# Run: bash tests/unit/test-MergeMill-review-request-changes.sh
...
RC_LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-request-changes.sh"
SKILL="$PROJECT_ROOT/skills/MergeMill-review/SKILL.md"
DECISION_GATE="$PROJECT_ROOT/skills/MergeMill-review/references/decision-gate.md"
...
if [[ -f "$RC_LIB" ]]; then
  ...
  out=$(
    # shellcheck source=../../skills/MergeMill-dispatcher/scripts/lib-review-request-changes.sh
    source "$RC_LIB"
    log() { :; }
    REPO="owner/repo"
