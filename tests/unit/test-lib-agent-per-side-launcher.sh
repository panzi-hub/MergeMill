...
# ---------------------------------------------------------------------------
echo ""
echo "=== PSL-S9: MergeMill-dev.sh — launcher rebind AFTER source lib-auth.sh ==="
# ---------------------------------------------------------------------------
...
# MergeMill.conf, which would otherwise reset AGENT_LAUNCHER (and
...
assert_eq "MergeMill-dev.sh: launcher rebind lands AFTER source lib-auth.sh" \
  "MATCH" "$hit"
...
assert_eq "MergeMill-dev.sh: launcher rebind does NOT precede source lib-auth.sh" \
  "REACHED_LIB_AUTH" "$hit_pre"

# ---------------------------------------------------------------------------
echo ""
echo "=== PSL-S10: MergeMill-review.sh — launcher rebind AFTER source lib-auth.sh ==="
# ---------------------------------------------------------------------------
hit=$(awk '
  /source "\$\{LIB_DIR\}\/lib-auth\.sh"/ {
    found_lib_auth = NR
...
assert_eq "MergeMill-review.sh: launcher rebind lands AFTER source lib-auth.sh" \
  "MATCH" "$hit"
...
assert_eq "MergeMill-review.sh: launcher rebind does NOT precede source lib-auth.sh" \
  "REACHED_LIB_AUTH" "$hit_pre"
