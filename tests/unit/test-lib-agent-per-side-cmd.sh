...
# ---------------------------------------------------------------------------
echo ""
echo "=== PSC-S9: MergeMill-dev.sh — rebind AFTER source lib-auth.sh ==="
# ---------------------------------------------------------------------------
# The rebind MUST come AFTER `source lib-auth.sh`. lib-auth.sh transitively
# sources lib-config.sh::load_MergeMill_conf which re-sources
# MergeMill.conf, and conf's unconditional `AGENT_CMD="claude"` line
...
assert_eq "MergeMill-dev.sh: AGENT_CMD=\$AGENT_DEV_CMD lands AFTER source lib-auth.sh" \
  "MATCH" "$hit"
...
assert_eq "MergeMill-dev.sh: rebind does NOT precede source lib-auth.sh" \
  "REACHED_LIB_AUTH" "$hit_pre"

# ---------------------------------------------------------------------------
echo ""
echo "=== PSC-S10: MergeMill-review.sh — rebind AFTER source lib-auth.sh ==="
# ---------------------------------------------------------------------------
hit=$(awk '
  /source "\$\{LIB_DIR\}\/lib-auth\.sh"/ {
    found_lib_auth = NR
...
assert_eq "MergeMill-review.sh: AGENT_CMD=\$AGENT_REVIEW_CMD lands AFTER source lib-auth.sh" \
  "MATCH" "$hit"
...
assert_eq "MergeMill-review.sh: rebind does NOT precede source lib-auth.sh" \
  "REACHED_LIB_AUTH" "$hit_pre"
