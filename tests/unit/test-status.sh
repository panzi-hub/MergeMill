...
# since status.sh re-resolves gh itself), a stub PID dir (MERGEMILL_PID_DIR) and
# run dir base (MERGEMILL_RUN_DIR_BASE). The stub gh reads a fixture file named
...
# regular file, so load_MergeMill_conf's `[[ -f ]]` tier-1 check MISSES it and
# falls through to the `$PROJECT_DIR/scripts/MergeMill.conf` tier — which loads
...
run_status() {  # run_status <issue> [extra args...]
  ...
  # (a real regular file) satisfies load_MergeMill_conf's tier-1 `-f` check so
  # the test never sources a project's MergeMill.conf (#235 review [P1]).
  env -u PROJECT_DIR \
  PATH="$BIN:$PATH" \
...
write_fixture "$GH_FIXTURE" "MergeMill pending-dev" "OPEN" "[]"
...
write_fixture "$GH_FIXTURE" "MergeMill in-progress" "OPEN" "[]"
...
write_fixture "$GH_FIXTURE" "MergeMill approved no-auto-close" "OPEN" \
...
write_fixture "$GH_FIXTURE" "MergeMill pending-review" "OPEN" "[]"
...
out="$(env -u PROJECT_DIR PATH="$BIN:$PATH" REPO=x/y REPO_OWNER=x PROJECT_ID=test-proj \
  MERGEMILL_CONF="$EMPTY_CONF" bash "$STATUS_SH" notanumber 2>&1; echo "rc=$?")"
