...
cat > "$PROJ_SCRIPTS/MergeMill.conf" <<'CONF'
...
# lib-agent.sh / lib-auth.sh must pass an UNRESOLVED dir to load_MergeMill_conf,
...
for f in lib-agent.sh lib-auth.sh; do
  # The load_MergeMill_conf call must NOT pass the *_REAL_DIR var.
  if grep -nE 'load_MergeMill_conf[[:space:]]+"\$\{?_LIB_[A-Z]+_REAL_DIR' "$DISPATCHER_SCRIPTS/$f" >/dev/null; then
    echo -e "  ${RED}FAIL${NC}: 012: $f passes the realpath dir to load_MergeMill_conf (INV-14 broken)"
    ((FAIL++))
  else
