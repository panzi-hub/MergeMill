...
# and the composed --body to files, plus an MergeMill.conf providing REPO.
...
HELPER_SRC="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/post-verdict.sh"
LIBCONFIG_SRC="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-config.sh"
...
# ---------------------------------------------------------------------------
# Sandbox factory: a scripts/ dir holding MergeMill.conf + a stub `gh`.
...
# pin MERGEMILL_PID_DIR to "<sandbox>/piddir" so the path is hermetic and the
...
# MERGEMILL_PID_DIR + PROJECT_ID so pid_dir_for_project() resolves hermetically.
# ---------------------------------------------------------------------------
...
PROJECT_ID=tproj MERGEMILL_PID_DIR="$PID" \
...
OUT=$(PROJECT_ID=tproj MERGEMILL_PID_DIR="$PID" \
...
OUT=$(env -u PROJECT_ID -u MERGEMILL_PID_DIR \
...
SB=$(make_sandbox 1)
printf 'body' > "$SB/body.md"
# Point MERGEMILL_PID_DIR at a path under a file (so mkdir -p fails).
touch "$SB/notadir"
OUT=$(PROJECT_ID=tproj MERGEMILL_PID_DIR="$SB/notadir/sub" \
