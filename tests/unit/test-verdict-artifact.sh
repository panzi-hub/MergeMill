...
#   2. Source-of-truth greps against MergeMill-review.sh: assert the wiring the
...
DISP="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts"
WRAPPER="$DISP/MergeMill-review.sh"
...
assert_eq "TC-VERDICT-ARTIFACT-011 path honors XDG_STATE_HOME" \
  "/xdg/state/MergeMill-proj/runs/RID-1/verdict-codex.json" \
...
assert_eq "TC-VERDICT-ARTIFACT-012 path falls back to HOME/.local/state" \
  "/home/u/.local/state/MergeMill-proj/runs/RID-1/verdict-agy.json" \
...
run_fleet() {
  ...
  for spec in "$@"; do
    ...
    rid="run-${_run}-${i}"
    local dir="$TMP/state/MergeMill-proj/runs/$rid"
    mkdir -p "$dir"
