...
#   the MergeMill-review.sh no-auto-close gate, status.sh's state/labels
#   read, and the two MergeMill-dev.sh issue-body fetches.
#
...
# ===========================================================================
echo ""
echo "=== TC-W1B-PARITY-020..021: MergeMill-review.sh no-auto-close gate ==="
# ===========================================================================
run_noautoclose() {
  local labels_json="$1"
  env -u MERGEMILL_PROVIDERS_DIR ISSUE_PROVIDER=github REPO="$REPO" _LABELS="$labels_json" \
  bash -c '
    set -uo pipefail
    gh() {
...
r_present=$(run_noautoclose '["MergeMill","no-auto-close"]')
r_absent=$(run_noautoclose '["MergeMill","reviewing"]')
...
run_status_state() {
  ...
  env -u MERGEMILL_PROVIDERS_DIR ISSUE_PROVIDER=github REPO="$REPO" _STATE="$state" _LABELS="$labels_json" \
  bash -c '
    set -uo pipefail
    gh() {
      # Raw gh shape: labels as {name} objects — the LEAF normalizes them.
...
# ===========================================================================
echo ""
echo "=== TC-W1B-PARITY-040..041: MergeMill-dev.sh issue-body fetch (text parity) ==="
# ===========================================================================
run_dev_fetch() {
  local fields_csv="$1" title="$2" body="$3"
  env -u MERGEMILL_PROVIDERS_DIR ISSUE_PROVIDER=github REPO="$REPO" _TITLE="$title" _BODY="$body" \
  bash -c '
    set -uo pipefail
    gh() {
