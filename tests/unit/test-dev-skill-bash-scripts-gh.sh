...
#
# Asserts that agent-facing docs in the MergeMill-dev skill never tell the
...
#
# Scope: skills/MergeMill-dev/SKILL.md plus skills/MergeMill-dev/references/*.md.
...
# scoped_files: every agent-facing markdown file in the MergeMill-dev skill.
mapfile -t scoped_files < <(
  find "$PROJECT_ROOT/skills/MergeMill-dev" -type f -name '*.md' | sort
)

if [[ ${#scoped_files[@]} -eq 0 ]]; then
  echo -e "${RED}FAIL${NC}: no agent-facing markdown files found under skills/MergeMill-dev"
  exit 1
fi
...
# ---------------------------------------------------------------------------
echo ""
echo "=== TC-COMMENT-004b: MergeMill-mode.md documents the dual-wrapper rule ==="
# ---------------------------------------------------------------------------
...
AUTO_MODE_MD="$PROJECT_ROOT/skills/MergeMill-dev/references/MergeMill-mode.md"
if [[ ! -f "$AUTO_MODE_MD" ]]; then
  ...
  if grep -q 'bash scripts/gh issue comment' "$AUTO_MODE_MD" \
    ...
    echo -e "  ${GREEN}PASS${NC}: MergeMill-mode.md mentions both bash scripts/gh and gh-as-user.sh"
    PASS=$((PASS + 1))
  else
    echo -e "  ${RED}FAIL${NC}: MergeMill-mode.md does not mention both wrappers (bash scripts/gh and gh-as-user.sh)"
    FAIL=$((FAIL + 1))
  fi
fi
