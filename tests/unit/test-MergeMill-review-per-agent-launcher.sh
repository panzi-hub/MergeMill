#!/bin/bash
# test-MergeMill-review-per-agent-launcher.sh — issue #173 / INV-42.
#
...
# test-MergeMill-review-per-agent-model.sh):
#
...
#   3. Source-of-truth greps against MergeMill-review.sh + the resolver lib +
#      MergeMill.conf.example.
#
# Run: bash tests/unit/test-MergeMill-review-per-agent-launcher.sh
...
RESOLVE_LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-resolve.sh"
...
# with MergeMill-review.sh by the TC-PAL-SRC greps below (which assert the
...
echo "=== TC-PAL-SRC-07: bash -n on MergeMill-review.sh ==="
