...
LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-review-codex.sh"
...
CODEX_ADAPTER="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/adapters/codex.sh"
WRAPPER="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/MergeMill-review.sh"
AGENT_LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-agent.sh"
...
if bash -n "$WRAPPER" 2>/dev/null; then
  echo -e "  ${GREEN}PASS${NC}: TC-CXRS-WIRE-07b MergeMill-review.sh parses (bash -n)"; PASS=$((PASS + 1))
else
  echo -e "  ${RED}FAIL${NC}: TC-CXRS-WIRE-07b MergeMill-review.sh fails bash -n"; FAIL=$((FAIL + 1))
fi
...
# actual MergeMill-review.sh so it can't silently regress (a non-zero exit must
...
printf '%s\n' 'You are an MergeMill PR reviewer.' '## Step 0: Merge Conflict Resolution — MANDATORY PRE-REVIEW' 'Prefix EACH blocking finding with [P1].' 'Post via post-verdict.sh.' > "$MF"
...
printf '%s\n' 'You are an MergeMill PR reviewer for PR #999.' '## Step 0: Merge Conflict Resolution — MANDATORY PRE-REVIEW' 'Check mergeable.' '## Step 0.5: Requirement Drift Detection — MANDATORY PRE-REVIEW' 'Read ALL comments.' > "$MF"
