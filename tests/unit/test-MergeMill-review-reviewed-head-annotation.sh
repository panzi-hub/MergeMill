#!/bin/bash
# test-MergeMill-review-reviewed-head-annotation.sh — verify that
# MergeMill-review.sh's `Reviewed HEAD:` trailer carries `agent` /
...
#
# Strategy mirrors test-MergeMill-dev-cleanup-startup-failure.sh: extract
# the trailer-emit block from MergeMill-review.sh, run it inside a
...
#
# Run: bash tests/unit/test-MergeMill-review-reviewed-head-annotation.sh
