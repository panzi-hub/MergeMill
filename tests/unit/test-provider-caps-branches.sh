...
# (ISSUE_PROVIDER=degraded / CODE_HOST=degraded + MERGEMILL_PROVIDERS_DIR), per
...
# mark-issue-checkbox.sh is a symlink into MergeMill-common/scripts/ (the real
# source of the body_checkbox=0 branch); grep follows it transparently.
CALLER_FILES=(
  "$SCRIPTS/lib-dispatch.sh" "$SCRIPTS/MergeMill-dev.sh" "$SCRIPTS/MergeMill-review.sh"
...
e2e1="$(env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
...
#     _render_close_keyword bytes from MergeMill-dev.sh (NOT a hand copy) with a
#     degraded chp_caps stub; the non-closing `Related to #N` backref must render.
render_kw() {
  local mc="$1" nipl="$2" issue="$3"
  bash -c '
    eval "$(sed -n "/^_render_close_keyword()/,/^}/p" "'"$SCRIPTS"'/MergeMill-dev.sh")"
...
STUB
chmod +x "$_bc_tmp/gh"
e2e_bc="$(env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
          PATH="$_bc_tmp:$PATH" \
          ISSUE_PROVIDER=degraded MERGEMILL_PROVIDERS_DIR="$FAKE_PROVIDER" \
