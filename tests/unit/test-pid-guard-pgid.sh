...
LIB_AGENT="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-agent.sh"
DEV_SCRIPT="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh"
REVIEW_SCRIPT="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/MergeMill-review.sh"
DISPATCH_SCRIPT="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/dispatch-local.sh"
...
echo
echo "=== TC-STATIC-003: MergeMill-dev.sh on_sigterm forwards to PGID ==="
echo
...
echo
echo "=== TC-STATIC-004: MergeMill-review.sh has SIGTERM trap parity ==="
echo
...
# (preferred — same contract as MergeMill-dev.sh).
...
# Provide a stub MergeMill.conf so load_MergeMill_conf doesn't fail.
mkdir -p "$TMPDIR/scripts"
cat > "$TMPDIR/scripts/MergeMill.conf" <<EOF
PROJECT_ID=test
REPO=test/test
REPO_OWNER=test
REPO_NAME=test
...
# as the PROJECT_DIR fallback, but load_MergeMill_conf walks tier-1/2/3 paths.
# Easiest route: cp lib-agent + lib-config into TMPDIR and point at the tmp.
mkdir -p "$TMPDIR/skills/MergeMill-dispatcher/scripts"
cp "$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-config.sh" \
   "$TMPDIR/skills/MergeMill-dispatcher/scripts/lib-config.sh"
cp "$LIB_AGENT" "$TMPDIR/skills/MergeMill-dispatcher/scripts/lib-agent.sh"
ln -sf "$TMPDIR/scripts/MergeMill.conf" "$TMPDIR/skills/MergeMill-dispatcher/scripts/MergeMill.conf"
...
source "$TMPDIR/skills/MergeMill-dispatcher/scripts/lib-agent.sh" 2>"$TMPDIR/source.err"
src_rc=$?
if [[ "$src_rc" -ne 0 ]]; then
...
# selects the dev wrapper since the fake tree is named MergeMill-dev.sh.
...
FAKE_TREE_SCRIPT="$PROJECT_DIR/scripts/MergeMill-dev.sh"
