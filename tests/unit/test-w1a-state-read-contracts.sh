...
_GH_PAYLOAD='[
  {"number":5,"title":"t5","labels":[{"name":"MergeMill"},{"name":"in-progress"}],"assignees":[{"login":"alice"},{"login":"bob"}],"comments":[]},
  {"number":3,"title":"t3","labels":[{"name":"MergeMill"}],"assignees":[],"comments":[{"url":"https://x/issues/3#issuecomment-1","author":{"login":"alice"},"body":"hi","createdAt":"2026-01-01T00:00:00Z"}]}
]'
gh() { printf '%s' "$_GH_PAYLOAD"; }
export -f gh
export _GH_PAYLOAD
# shellcheck source=../../skills/MergeMill-dispatcher/scripts/providers/itp-github.sh
source "$ITP_GITHUB"
set +e

out="$(itp_github_list_by_state open MergeMill 100 "number,title,labels,comments")"
...
_GH_PAYLOAD='[]'
out="$(itp_github_list_by_state open MergeMill 100 "number")"
assert_eq "no matches → [] (never null, never empty string)" "[]" "$out"

echo ""
echo "=== [#435] LEAF SHAPE: itp_github_list_by_state — assignees field ==="
_GH_PAYLOAD='[
  {"number":5,"title":"t5","labels":[{"name":"MergeMill"}],"assignees":[{"login":"alice"},{"login":"bob"}],"comments":[]},
  {"number":3,"title":"t3","labels":[{"name":"MergeMill"}],"assignees":[],"comments":[]}
]'
out="$(itp_github_list_by_state open MergeMill 100 "number,assignees")"
...
out="$(itp_github_list_by_state open MergeMill 100 "number,labels")"
...
_GH_PAYLOAD='[{"number":1,"title":"t","labels":[{"name":"MergeMill"}],"comments":[]}]'
out="$(itp_github_list_by_state open MergeMill 100 "number,labels")"
...
out="$(itp_github_count_by_state open MergeMill 100 "in-progress,reviewing")"
assert_eq "any-of count: 2 of 3 match in-progress OR reviewing" "2" "$out"
out="$(itp_github_count_by_state open MergeMill 100 "")"
...
_GH_PAYLOAD='[
  ...
  {"number":2,"title":"","labels":[{"name":"MergeMill"}],"assignees":[],"comments":[]},
...
out="$(itp_github_list_forbidden_combos open MergeMill 100)"
...
export -f gh
out="$(itp_github_list_by_state open MergeMill 100 "number" 2>/dev/null)"; rc=$?
...
out="$(itp_github_count_by_state open MergeMill 100 "" 2>/dev/null)"; rc=$?
...
out="$(itp_github_list_forbidden_combos open MergeMill 100 2>/dev/null)"; rc=$?
