...
INSTALLER="$PROJECT_ROOT/skills/MergeMill-common/scripts/install-project-hooks.sh"
...
make_repo_with_skills() {
  ...
  local disp="$repo/.agents/skills/MergeMill-dispatcher/scripts"
  local common_hooks="$repo/.agents/skills/MergeMill-common/hooks"
  ...
  echo "# upstream MergeMill-dev" > "$disp/MergeMill-dev.sh"
  echo "# upstream MergeMill-review" > "$disp/MergeMill-review.sh"
...
echo "PROJECT=local" > "$repo/scripts/MergeMill.conf"
echo "echo deploy" > "$repo/scripts/deploy.sh"
conf_before="$(cat "$repo/scripts/MergeMill.conf")"

(cd "$repo" && bash "$INSTALLER" --no-git-hook >/dev/null 2>&1)

if [[ ! -L "$repo/scripts/MergeMill.conf" ]] && \
   [[ "$(cat "$repo/scripts/MergeMill.conf")" == "$conf_before" ]]; then
  echo -e "  ${GREEN}PASS${NC}: MergeMill.conf preserved (real file, contents intact)"
  PASS=$((PASS + 1))
else
  echo -e "  ${RED}FAIL${NC}: MergeMill.conf was overwritten or symlinked"
  FAIL=$((FAIL + 1))
fi
...
# files coexist.
if [[ -L "$repo/scripts/MergeMill-dev.sh" ]]; then
...
echo "# upstream new entry" \
  > "$repo/.agents/skills/MergeMill-dispatcher/scripts/setup-labels.sh"
...
echo "# upstream lib-review-verdict" \
  > "$repo/.agents/skills/MergeMill-dispatcher/scripts/lib-review-verdict.sh"
chmod +x "$repo/.agents/skills/MergeMill-dispatcher/scripts/"*.sh
...
ln -s "$repo/.agents/skills/MergeMill-dispatcher/scripts/lib-agent.sh" \
      "$repo/scripts/lib-agent.sh"
ln -s "$repo/.agents/skills/MergeMill-dispatcher/scripts/lib-auth.sh" \
      "$repo/scripts/lib-auth.sh"

# Remove an ENTRY point upstream (dangling-symlink prune path).
rm "$repo/.agents/skills/MergeMill-dispatcher/scripts/MergeMill-review.sh"

(cd "$repo" && bash "$INSTALLER" --no-git-hook >/dev/null 2>&1)

if [[ ! -e "$repo/scripts/MergeMill-review.sh" ]] && [[ ! -L "$repo/scripts/MergeMill-review.sh" ]]; then
  echo -e "  ${GREEN}PASS${NC}: dangling entry symlink MergeMill-review.sh pruned"
  PASS=$((PASS + 1))
else
...
if [[ -L "$repo/scripts/MergeMill-dev.sh" ]]; then
  ...
  echo -e "  ${RED}FAIL${NC}: pruning collateral-damaged MergeMill-dev.sh"
  FAIL=$((FAIL + 1))
fi
...
rc="${rc:-0}"
if [[ "$rc" -ne 0 ]] && grep -q "MergeMill-dispatcher" <<<"$err" \
...
# We need a real install-git-pre-push.sh in the MergeMill-common/hooks
...
ln -sf "$PROJECT_ROOT/skills/MergeMill-common/hooks/install-git-pre-push.sh" \
       "$repo/.agents/skills/MergeMill-common/hooks/install-git-pre-push.sh"
...
if [[ ! -e "$repo/scripts/MergeMill-dev.sh" ]]; then
  echo -e "  ${GREEN}PASS${NC}: --dry-run created no entry symlink"
  PASS=$((PASS + 1))
else
  echo -e "  ${RED}FAIL${NC}: --dry-run created scripts/MergeMill-dev.sh"
  FAIL=$((FAIL + 1))
fi
# It should still PRINT a plan.
if grep -q 'dry-run' <<<"$out" && grep -q 'MergeMill-dev.sh' <<<"$out"; then
...
printf 'PROJECT_ID=x\n' > "$repo/scripts/MergeMill.conf"
chmod 600 "$repo/scripts/MergeMill.conf"
...
rm "$repo/.agents/skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh"
doc_out="$(cd "$repo" && bash "$INSTALLER" --doctor 2>&1)"; doc_rc=$?
if [[ "$doc_rc" -ne 0 ]] && grep -qiE 'broken|missing' <<<"$doc_out" \
   && grep -q 'MergeMill-dev.sh' <<<"$doc_out"; then
...
# ---------------------------------------------------------------------------
echo ""
echo "=== TC-ENTRY-SHIM-025: --doctor flags a missing MergeMill.conf ==="
# ---------------------------------------------------------------------------
repo="$TMPDIR/repo-doc-noconf"
make_repo_with_skills "$repo"
(cd "$repo" && bash "$INSTALLER" --no-git-hook >/dev/null 2>&1)
# No MergeMill.conf at all.
doc_out="$(cd "$repo" && bash "$INSTALLER" --doctor 2>&1)"; doc_rc=$?
if [[ "$doc_rc" -ne 0 ]] && grep -q 'MergeMill.conf is MISSING' <<<"$doc_out"; then
...
mkdir -p "$repo/scripts"
ln -s "$repo/.agents/skills/MergeMill-dispatcher/scripts/dispatch-remote-aws-ssm.sh" \
      "$repo/scripts/dispatch-remote-aws-ssm.sh"
ln -s "$repo/.agents/skills/MergeMill-dispatcher/scripts/liveness-check-remote-aws-ssm.sh" \
...
doc_out="$(cd "$repo" && printf 'PROJECT_ID=x\n' > "$repo/scripts/MergeMill.conf"; chmod 600 "$repo/scripts/MergeMill.conf"; bash "$INSTALLER" --doctor 2>&1)"; doc_rc=$?
