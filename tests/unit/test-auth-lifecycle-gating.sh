...
LIB_AUTH="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-auth.sh"
DISPATCHER_TICK="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/dispatcher-tick.sh"
...
# Provide a minimal MergeMill.conf so lib-auth.sh's load_MergeMill_conf
# doesn't misbehave. lib-auth.sh uses MERGEMILL_CONF_DIR to locate its
# project-side dir.
cat > "$FAKE_PROJECT_DIR/MergeMill.conf" <<EOF
REPO="test/repo"
REPO_OWNER="test"
REPO_NAME="repo"
EOF
chmod 600 "$FAKE_PROJECT_DIR/MergeMill.conf"
...
DRV
  env -u PROJECT_DIR PATH="$PATH" \
      ...
      MERGEMILL_CONF_DIR="$FAKE_PROJECT_DIR" \
...
DISP_TICK="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/dispatcher-tick.sh"
...
out=$(env -u PROJECT_DIR -u ISSUE_PROVIDER -u CODE_HOST PATH="$PATH" \
      GH_AUTH_MODE="app" MERGEMILL_CONF_DIR="$FAKE_PROJECT_DIR" \
      REPO="test/repo" REPO_OWNER="test" REPO_NAME="repo" \
      DISPATCHER_APP_ID="" DISPATCHER_APP_PEM="" \
      LIB_DIR="$(dirname "$LIB_AUTH")" \
...
# ===========================================================================
echo "=== TC-AUTH-011..014: REAL wrapper startup segments (MergeMill-{dev,review}.sh) — P1-2 ==="
# ===========================================================================
# [P1-2] Codex found the pre-fix code checked `GH_AUTH_MODE=app + require App
# creds` at MergeMill-dev.sh:~170 and MergeMill-review.sh:~389 BEFORE the
...
REVIEW_SH="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/MergeMill-review.sh"
...
[[ -n "$DEV_BLOCK" ]]    || bad "TC-AUTH-011: could not extract auth block from MergeMill-dev.sh"
[[ -n "$REVIEW_BLOCK" ]] || bad "TC-AUTH-013: could not extract auth block from MergeMill-review.sh"
...
# TC-AUTH-011: MergeMill-dev.sh REAL block, gitlab/gitlab app-mode → NO FATAL.
out=$(_drive_wrapper_block "$DEV_BLOCK" gitlab gitlab app 2>&1); rc=$?
assert_eq "TC-AUTH-011: MergeMill-dev.sh REAL block on gitlab/gitlab app-mode → rc 0" "0" "$rc"
...
# TC-AUTH-012: MergeMill-dev.sh REAL block, github/gitlab mixed app-mode → FATAL.
out=$(_drive_wrapper_block "$DEV_BLOCK" github gitlab app 2>&1); rc=$?
if [[ "$rc" -ne 0 ]]; then
  ok "TC-AUTH-012: MergeMill-dev.sh github/gitlab mixed → real block FATAL (rc 1)"
else
...
# TC-AUTH-013: MergeMill-review.sh REAL block, gitlab/gitlab app-mode → NO FATAL.
out=$(_drive_wrapper_block "$REVIEW_BLOCK" gitlab gitlab app 2>&1); rc=$?
assert_eq "TC-AUTH-013: MergeMill-review.sh REAL block on gitlab/gitlab app-mode → rc 0" "0" "$rc"
...
# TC-AUTH-014: MergeMill-review.sh REAL block, github/gitlab mixed app-mode → FATAL.
out=$(_drive_wrapper_block "$REVIEW_BLOCK" github gitlab app 2>&1); rc=$?
if [[ "$rc" -ne 0 ]]; then
  ok "TC-AUTH-014: MergeMill-review.sh github/gitlab mixed → real block FATAL"
else
