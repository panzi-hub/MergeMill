#!/bin/bash
# test-MergeMill-launcher-verdict-fresh.sh —
...
# the actual code paths in MergeMill-review.sh, lib-agent.sh, and
# lib-dispatch.sh.
#
# Run: bash tests/unit/test-MergeMill-launcher-verdict-fresh.sh
...
SCRIPTS_DIR="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts"
...
# two-sources-of-truth hazard with test-MergeMill-review-verdict-regex.sh).
WRAPPER="$SCRIPTS_DIR/MergeMill-review.sh"
...
run_agent_capture() {
  ...
  (
    ...
    # lib-config.sh's load_MergeMill_conf will fail to find conf — that's
...
# TC-LCH-007: MergeMill-dev.sh exports CC_USER/CC_ROLE_KIND.
DEV_LINE=$(grep -E '^export CC_USER=' "$SCRIPTS_DIR/MergeMill-dev.sh" | head -1)
REVIEW_LINE=$(grep -E '^export CC_USER=' "$SCRIPTS_DIR/MergeMill-review.sh" | head -1)
assert_contains "TC-LCH-007 MergeMill-dev exports CC_USER=MergeMill-dev-bot" "MergeMill-dev-bot" "$DEV_LINE"
assert_contains "TC-LCH-008 MergeMill-review exports CC_USER=MergeMill-review-bot" "MergeMill-review-bot" "$REVIEW_LINE"
...
run_is_completed() {
  ...
  (
    ...
    # set them before sourcing so the unit test isn't gated on MergeMill.conf.
    export REPO="test/repo"
    export REPO_OWNER="test"
...
# ---------------------------------------------------------------------------
# TC-PTL: MergeMill-dev.sh resume-fallback posts standalone Dev Session ID marker.
# ---------------------------------------------------------------------------
DEV_SCRIPT="$SCRIPTS_DIR/MergeMill-dev.sh"
