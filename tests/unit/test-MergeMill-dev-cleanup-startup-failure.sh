#!/bin/bash
# test-MergeMill-dev-cleanup-startup-failure.sh — verify that
# MergeMill-dev.sh's cleanup trap posts an "Agent Session Report (Dev)"
...
#
# Strategy: extract the cleanup() function from MergeMill-dev.sh and
# run it inside a harness with a stubbed `gh` and a stubbed
# `cleanup_github_auth`, varying AGENT_RAN/ISSUE_NUMBER per case.
#
# Run: bash tests/unit/test-MergeMill-dev-cleanup-startup-failure.sh
...
# Extract cleanup() from MergeMill-dev.sh. The function spans from
...
run_cleanup() {
  ...
  bash -c "
    ...
    # [#296 B8] cleanup()'s startup-failure label flip (MergeMill-dev.sh:728)
...
# reason; see TC-CL-006 docstring + MergeMill-dev.sh comment.
HARNESS_FILE="$0"
