...
jq '(.transitions[] | select(.id=="dispatch-new").mermaid) = "MergeMill --> in_progress: TAMPERED"' \
...
# MergeMill-review.sh reviewing|pending-dev declares 8; add a 9th → discovered 9 ≠ 8.
cp "$SCRIPTS"/*.sh "$SCRATCH/" 2>/dev/null
printf '\ngh issue edit "$n" --repo "$R" --remove-label "reviewing" --add-label "pending-dev"\n' >> "$SCRATCH/MergeMill-review.sh"
out="$(bash "$CHECK" --transitions "$DOCS/transitions.json" --guard-map "$DOCS/spec-guard-map.json" \
  --codesite-map "$DOCS/spec-codesite-map.json" --doc "$DOCS/state-machine.md" --scripts-dir "$SCRATCH" 2>&1)"
rc=$?
...
cp "$SCRIPTS/MergeMill-review.sh" "$SCRATCH/MergeMill-review.sh"  # restore
...
missing=""
for lbl in MergeMill in-progress pending-review reviewing pending-dev approved no-auto-close stalled; do
...
# label flips (13 MergeMill-review.sh + 3 MergeMill-dev.sh) behind
...
AC9_311='- #296 B8 (#311): live-wrapper label flips (16 sites: 13 MergeMill-review.sh, 3 MergeMill-dev.sh) → itp_transition_state — byte-identical; baseline counts shrank by 16 occurrences (review-bare 14→1, dev-bare 4→1), no signature removed; ZERO manifest edit (Form-3 coverage-neutral, #313).'
...
AC4_328='- #296 second-tier (#328): the dev-resume PR inline-review-comment read (`PR_REVIEW_COMMENTS`, MergeMill-dev.sh) migrated from raw `gh api repos/$REPO/pulls/$PR_NUM/comments --jq` to the NEW verb `chp_list_inline_comments` — byte-identical (the `--jq` formatter stays caller-side, #281); baseline shrank by 1 sig.'
...
# The merge_closes_issue=0 fallback at MergeMill-review.sh:3518 is a real, shipped
...
jq 'del(.sites[] | select(.file=="MergeMill-review.sh" and .movement=="reviewing|approved" and .anchor=="merge_closes_issue=0 — transitioning issue"))' \
