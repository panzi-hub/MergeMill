...
# resolution path lib-error.sh uses (${MERGEMILL_CONF_DIR}/gh):
#
...
LIB_ERROR="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-error.sh"
...
# the real ${MERGEMILL_CONF_DIR}/gh → gh-with-token-refresh.sh symlink.
cat > "$SANDBOX/scripts/gh" <<EOF
#!/bin/bash
{ echo "GH-INVOCATION"; printf '%s\n' "\$@"; echo "---"; } >> "$CALLS"
echo "https://github.com/panzi-hub/MergeMill/issues/231#issuecomment-9999"
exit 0
EOF
...
(
  set -euo pipefail
  export MERGEMILL_CONF_DIR="$SANDBOX/scripts"
...
LIB_RUN_ARTIFACTS="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-run-artifacts.sh"
...
#!/bin/bash
{ echo "GH-INVOCATION"; printf '%s\n' "\$@"; echo "---"; } >> "$CALLS2"
echo "https://github.com/panzi-hub/MergeMill/issues/231#issuecomment-10000"
exit 0
EOF
chmod +x "$SANDBOX/scripts2/gh"
RUN_STATE_BASE="$SANDBOX/state/MergeMill-errenv-proj"
(
  set -euo pipefail
  export MERGEMILL_CONF_DIR="$SANDBOX/scripts2"
  export REPO="panzi-hub/MergeMill"
  export PROJECT_ID="errenv-proj"
  export MERGEMILL_RUN_DIR_BASE="$RUN_STATE_BASE"
  # shellcheck disable=SC1090
...
DISPATCH_TICK="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/dispatcher-tick.sh"
