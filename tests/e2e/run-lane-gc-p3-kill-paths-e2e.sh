...
run_scenario() {
  ...
  # child (the stub MergeMill-dev.sh) in addition to the fixture leader —
...
cat > "$PROJDIR_A/scripts/MergeMill-dev.sh" <<'STUB'
#!/bin/bash
sleep 2
STUB
chmod +x "$PROJDIR_A/scripts/MergeMill-dev.sh"
...
# The orphan leader must KEEP the MergeMill-dev.sh cmdline (no exec) so the
# fallback's pgrep anchor (${PROJECT_DIR}/scripts/MergeMill-dev.sh … --issue
# N) matches it, while it carries a persistent TERM-trapping member in its
# own pgid.
cat > "$PROJDIR_B/scripts/MergeMill-dev.sh" <<'STUB'
#!/bin/bash
(trap "" TERM; while :; do sleep 1; done) & disown
sleep 30
STUB
chmod +x "$PROJDIR_B/scripts/MergeMill-dev.sh"

VERDICT_B=$(run_scenario "b" "" "$PIDDIR_B" "$PROJDIR_B" "$ISSUE_B" true \
  setsid bash "$PROJDIR_B/scripts/MergeMill-dev.sh" --issue "$ISSUE_B")
