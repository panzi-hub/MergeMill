...
case "$want" in
  issue) echo '{"state":"OPEN","title":"e2e issue","labels":[{"name":"MergeMill"},{"name":"pending-review"}]}' ;;
  pr)    echo "" ;;   # no PR
  *)     echo "" ;;
esac
GH
...
# regular file, so load_MergeMill_conf's `-f` tier-1 misses it and falls through
# to `$PROJECT_DIR/scripts/MergeMill.conf` when PROJECT_DIR is exported (#235
...
STATUS_OUT="$(
  ...
  MERGEMILL_PID_DIR="$PID_DIR" MERGEMILL_RUN_DIR_BASE="$STATE_ROOT/MergeMill-e2e-proj" \
...
# MergeMill-review.sh against the real run_footer.
# ---------------------------------------------------------------------------
