...
_run_entry() {
  ...
  (
    ...
    #   1. source the smoke lib — lib-agent.sh's `load_MergeMill_conf` re-sources
    #      the project's MergeMill.conf, which assigns globals like
    ...
    # Source the smoke lib FIRST (loads MergeMill.conf + tokenizes the launcher).
    ...
    # on a healthy non-claude CLI. When the operator's MergeMill.conf sets a
    ...
    # operator's MergeMill.conf tunes it for ONE CLI — e.g. `--trust-all-tools`
    ...
    # last writer and its overrides win over the auto-loaded MergeMill.conf.
    ...
    # the lib (+ re-loads MergeMill.conf) before skipping — cheap, and the
