...
# and lib-code-host.sh each read the SHARED MERGEMILL_PROVIDERS_DIR env var
...
# per run containing symlinks for BOTH axes so a single MERGEMILL_PROVIDERS_DIR
...
ITP_LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-issue-provider.sh"
CHP_LIB="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/lib-code-host.sh"
DEV_SH="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh"
...
_cap_read() {
  ...
  [[ "$cap" == "-" ]] && return 1
  seam="$(_seam_for "$verb")"
  env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
      ISSUE_PROVIDER="$ITP_NAME" CODE_HOST="$CHP_NAME" MERGEMILL_PROVIDERS_DIR="$SCRATCH_DIR" \
      PATH="$ISOLATED_PATH" \
  bash -c '
...
_run_list_inline_comments_assert() {
  ...
  # and silently drops review feedback from MergeMill-dev.sh's dev-resume
...
_run_close_keyword_assert() {
  ...
  [[ -f "$DEV_SH" ]] || { emit FAIL "$verb" "MergeMill-dev.sh not found at $DEV_SH"; return; }
  local render='
...
_leaf_present() {
  ...
  env -u MERGEMILL_CONF -u MERGEMILL_CONF_DIR -u PROJECT_DIR \
      ...
      MERGEMILL_PROVIDERS_DIR="$SCRATCH_DIR" PATH="$ISOLATED_PATH" \
  bash -c "
    source \"$ITP_LIB\" 2>/dev/null
    source \"$CHP_LIB\" 2>/dev/null
...
_assert_verb() {
  ...
  case "$verb" in
    ...
    itp_provision_states)
      case "$ITP_NAME" in
        gitlab)
          ...
          # /labels/<name>.
          _run_gl_write_assert "$verb" \
            'itp_provision_states MergeMill 0E8A16 desc' \
            "GET" "path=projects/group%2Fproject/labels/MergeMill" ""
          ;;
        *)
    ...
    itp_label_event_ts)
      case "$ITP_NAME" in
        gitlab)
          _run_gl_failsoft_assert "$verb" 'itp_label_event_ts 42 MergeMill' '^$'
          ;;
        *)
          _run_failsoft_assert "$verb" 'itp_label_event_ts 42 MergeMill' '^$'
          ;;
      esac
      ;;
    itp_list_by_state)
      case "$ITP_NAME" in
        gitlab)
          ...
          _run_gl_shape_assert "$verb" \
            'itp_list_by_state open MergeMill 100 number,title,labels' \
          ...
          local _lbs_gl_out
          _lbs_gl_out="$(_invoke _PCF_GL_PAYLOAD="$PAYLOADS/gitlab-issues-list.json" _PCF_GL_ARGV_FILE="$work_root/.argv-gl-lbs-435.json" 'itp_list_by_state open MergeMill 100 number,assignees' 2>/dev/null)"
          ...
          _lbs_gl_out="$(_invoke _PCF_GL_PAYLOAD="$PAYLOADS/gitlab-issues-list.json" _PCF_GL_ARGV_FILE="$work_root/.argv-gl-lbs-435b.json" 'itp_list_by_state open MergeMill 100 number,labels' 2>/dev/null)"
          ...
          ;;
        github)
          _run_shape_assert "$verb" 'itp_list_by_state open MergeMill 100 number,title,labels,comments' \
          ...
          local _lbs_gh_out
          _lbs_gh_out="$(_invoke _PCF_GH_MODE="ok" _PCF_GH_PAYLOAD="$PAYLOADS/issue-list-valid.json" _PCF_ARGV_FILE="$work_root/.argv-gh-lbs-435.json" 'itp_list_by_state open MergeMill 100 number,assignees' 2>&1)"
          if jq -e '(length == 2) and all(.[]; (.assignees | type) == "array" and all(.assignees[]; type == "string"))
                     and ([.[] | select(.number == 5)][0].assignees == ["alice","bob"])
          ...
          _lbs_gh_out="$(_invoke _PCF_GH_MODE="ok" _PCF_GH_PAYLOAD="$PAYLOADS/issue-list-valid.json" _PCF_ARGV_FILE="$work_root/.argv-gh-lbs-435b.json" 'itp_list_by_state open MergeMill 100 number,labels' 2>&1)"
      ...
      case "$ITP_NAME" in
        gitlab)
          _run_gl_count_assert "$verb" \
            'itp_count_by_state open MergeMill 100 in-progress' \
            "$PAYLOADS/gitlab-issues-list.json"
          ;;
        *)
          _run_count_assert "$verb" 'itp_count_by_state open MergeMill 100 in-progress' \
    ...
    itp_list_forbidden_combos)
      case "$ITP_NAME" in
        gitlab)
          _run_gl_shape_assert "$verb" \
            'itp_list_forbidden_combos open MergeMill 100' \
            "$PAYLOADS/gitlab-issues-list.json" number
          ...
          local _fc_gl_out
          _fc_gl_out="$(_invoke _PCF_GL_PAYLOAD="$PAYLOADS/gitlab-forbidden-combos-valid.json" _PCF_GL_ARGV_FILE="$work_root/.argv-gl-fc-435.json" 'itp_list_forbidden_combos open MergeMill 100' 2>/dev/null)"
          ...
          ;;
        github)
          _run_shape_assert "$verb" 'itp_list_forbidden_combos open MergeMill 100' \
            "$PAYLOADS/issue-list-valid.json" number
          ...
          local _fc_gh_out
          _fc_gh_out="$(_invoke _PCF_GH_MODE="ok" _PCF_GH_PAYLOAD="$PAYLOADS/forbidden-combos-valid.json" _PCF_ARGV_FILE="$work_root/.argv-gh-fc-435.json" 'itp_list_forbidden_combos open MergeMill 100' 2>&1)"
          if jq -e '(length == 1) and (.[0].number == 7)
                     and ((.[0] | keys_unsorted | sort) == ["assignees","labels","number"])
