...
Dev-side cases extend `tests/unit/test-MergeMill-dev-cleanup-startup-failure.sh`
(harness extracts `cleanup()` from `MergeMill-dev.sh`, stubs `gh`, varies
env per case). Review-side case lives in
`tests/unit/test-MergeMill-review-reviewed-head-annotation.sh` (parallel
function-extraction harness for the trailer-emit block).

Run via:

```bash
bash tests/unit/test-MergeMill-dev-cleanup-startup-failure.sh
bash tests/unit/test-MergeMill-review-reviewed-head-annotation.sh
```
...
- Extract the `if [[ -n "$LATEST_COMMENT" && -n "$PR_HEAD_SHA" ]]; then ...
  fi` block from `MergeMill-review.sh`.
