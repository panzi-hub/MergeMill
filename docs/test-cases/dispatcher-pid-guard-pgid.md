...
**Setup**: spawn `MergeMill-dev.sh` (mocked end-to-end with a fake `claude` that ignores SIGTERM for 30s and forks a grandchild that also ignores it). SIGTERM the wrapper after 1 second.
...
**Setup**: extract `on_sigterm` from `MergeMill-dev.sh`, set `_AGENT_RUN_PID` to a session leader's PID, send SIGTERM to the test shell.
...
| TC-STATIC-003 | `MergeMill-dev.sh` `on_sigterm` references `_AGENT_RUN_PID` and uses `kill -- -<pid>` group syntax |
| TC-STATIC-004 | `MergeMill-review.sh` has the same SIGTERM trap (parity) |
