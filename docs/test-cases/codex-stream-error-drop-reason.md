...
suite) plus source-of-truth assertions against `MergeMill-review.sh`.
...
## TC-CODEX-DROP-SRC: source-of-truth wiring (`MergeMill-review.sh`)
...
| TC-CODEX-DROP-SRC-04 | `bash -n` parses `lib-review-codex.sh` AND `MergeMill-review.sh` |
...
| TC-CODEX-DROP-REG-03 | INV-40 aggregation truth table unchanged — a stream-error codex is still DROPPED (`unavailable`), not a deciding FAIL; `test-MergeMill-review-multi-agent` / `test-review-agent-timeout` stay green |
