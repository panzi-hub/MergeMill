...
| ID | Scenario | Expected |
|----|----------|----------|
| TC-CXRS-WIRE-01 | the fan-out codex branch calls `_run_codex_review` | grep hit in `MergeMill-review.sh` |
...
| TC-CXRS-WIRE-04 | the INV-55 `DIFF_START_`/`DIFF_END_` inline-diff block is GONE from the codex branch | no grep hit in `MergeMill-review.sh` |
...
| TC-CXRS-WIRE-07 | `bash -n` parses lib-review-codex.sh + MergeMill-review.sh | both parse |
