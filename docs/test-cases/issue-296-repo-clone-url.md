...
local `origin` remote name — mirroring the INV-45 precedent at `MergeMill-dev.sh:397`
...
- `skills/MergeMill-dispatcher/scripts/lib-auth.sh` — `drain_agent_pr_create` head-resolution.
...
- **Action**: `grep -c 'gh repo view' skills/MergeMill-dispatcher/scripts/lib-auth.sh`.
- **Expected**: `0`.
- **AC**: AC1.
