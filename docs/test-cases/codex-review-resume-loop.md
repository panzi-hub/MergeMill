...
| TC-CXR-ISO-04 | `MergeMill-review.sh` sources `lib-review-codex.sh` | source line present, ShellCheck clean |
| TC-CXR-ISO-05 | `bash -n` parse of `lib-review-codex.sh` and `MergeMill-review.sh` | both parse cleanly |
...
- `tests/unit/test-MergeMill-review-multi-agent.sh` — fan-out for non-codex.
- `tests/unit/test-MergeMill-review-per-agent-model.sh`,
  `test-MergeMill-review-per-agent-launcher.sh` — per-agent resolution unchanged.
