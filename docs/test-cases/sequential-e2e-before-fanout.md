...
## TC-SE2E-SRC: source-of-truth greps (MergeMill-review.sh / build_review_prompt)
...
| TC-SE2E-DOC-03 | `skills/MergeMill-review/references/e2e-command-mode.md` updated for the wrapper-run lane |

## Backward-compat gate

`bash -n` + the existing INV-40 / INV-43 / INV-44 suites stay green:
`test-MergeMill-review-multi-agent`, `test-review-e2e-command-poll-budget`,
`test-review-cli-exit-grace`, `test-MergeMill-review-mergeable-gate`,
`test-e2e-mode-command`, `test-MergeMill-review-prompt`,
`test-MergeMill-review-per-agent-model`, `test-MergeMill-review-per-agent-launcher`.
