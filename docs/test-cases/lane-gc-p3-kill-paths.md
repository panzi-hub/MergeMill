...
| TC-LGC3-042 | Grep-pin, repo-wide | zero occurrences of `pkill -f 'MergeMill-'` (or any `-f`-based wrapper-script-name match) anywhere — the widening the design explicitly forbids (would cross-kill sibling lanes) |
...
| TC-LGC3-E2E-02 | Same fixture shape, no PID file — `dispatch-local.sh dev-new` must find it via the pgrep-fallback orphan sweep (leader's cmdline anchors on `${PROJECT_DIR}/scripts/MergeMill-dev.sh … --issue N`) | whole process group gone within grace(5s)+2s |

Both scenarios use per-run temp `PROJECT_DIR`/`MERGEMILL_PID_DIR` anchors, so
