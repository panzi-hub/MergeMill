...
Strategy: source-of-truth greps over `MergeMill-review.sh` + the agent-facing
...
`test-MergeMill-review-auto-merge-failure.sh` and `…-mergeable-gate.sh`.
...
## Unit tests — `tests/unit/test-MergeMill-review-request-changes.sh`
...
`test-MergeMill-review-auto-merge-failure.sh`, `…-mergeable-gate.sh`,
...
and does not touch the `−MergeMill` accounting, so the #145 / INV-44 pins hold.
