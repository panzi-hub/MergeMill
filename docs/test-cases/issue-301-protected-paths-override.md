...
- **Defect 1** — the review-agent classification **prompt** (`MergeMill-review.sh`
...
- **Defect 2** — `lib-review-classify.sh:39` uses `:=` (`${VAR:=default}`), which
  ...
  default list. The `MergeMill.conf.example` doc already promises `""` disables
...
- `lib-review-classify.sh` is sourced into `MergeMill-review.sh` at line 168, BEFORE
