...
The helper is extracted from `MergeMill-dev.sh` via awk and run with a stubbed `gh`
...
| ID | Assertion |
|----|---|
| TC-PAF-W01 | `emit_post_approval_findings_block` is defined in `MergeMill-dev.sh` |
...
| TC-PAF-W04 | `MergeMill-dev.sh` still passes `bash -n` and `shellcheck -S error` |
| TC-PAF-D01 | `MergeMill-mode.md` Resume Awareness documents approval-vs-findings timestamp ordering governing the exit decision |
| TC-PAF-D02 | `MergeMill-mode.md` documents broadened findings recognition (BLOCKING/`[P1]`, not only the exact `Review findings:` prefix) |
...
| ID | Assertion | Layer |
|----|---|---|
| TC-RE2-01 | The resume `-q` findings selector line(s) are locatable in `MergeMill-dev.sh` | static |
