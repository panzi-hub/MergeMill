...
`<repo>/.agents/skills/MergeMill-{dispatcher,common}/`. Dispatcher
scripts dir contains `lib-agent.sh`, `lib-auth.sh`,
`MergeMill-dev.sh`, `MergeMill-review.sh`.

**Run**: `bash install-project-hooks.sh --no-git-hook`

**Expect**:
- `<repo>/scripts/lib-agent.sh` is a symlink → dispatcher's `lib-agent.sh`
- Same for `lib-auth.sh`, `MergeMill-dev.sh`, `MergeMill-review.sh`
- `<repo>/hooks` is a symlink → MergeMill-common's `hooks/`
- Exit code 0

## TC-IPH-02 — does not overwrite project-local files

**Setup**: as TC-IPH-01, plus a real (non-symlink)
`<repo>/scripts/MergeMill.conf` and a real `<repo>/scripts/deploy.sh`.

**Run**: `bash install-project-hooks.sh --no-git-hook`

**Expect**:
- `MergeMill.conf` is still a real file (not a symlink), unchanged.
...
**Expect**:
- Exit code != 0.
- Stderr contains "MergeMill-dispatcher" and "npx skills add" guidance.
...
<repo>/scripts/MergeMill-review.sh` succeeds. (Indirectly verifies
