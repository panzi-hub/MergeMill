...
3. `skills/MergeMill-common/hooks/lib.sh` — new `read_hook_stdin` helper
...
| TC-LGC1-034 | grep-pin: all 12 hooks under `skills/MergeMill-common/hooks/` that previously had `input=$(cat)` now call `input=$(read_hook_stdin)` | 0 remaining `input=$(cat)` occurrences; 12 `read_hook_stdin` call sites |
