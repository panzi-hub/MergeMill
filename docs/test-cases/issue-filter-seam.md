...
|---|---|---|
| TC-IFSEAM-001 | GitHub leaf: `itp_list_by_state open MergeMill 100 "number,labels,assignees"` against a fixture with two assignees on one issue | `assignees` present as `["alice","bob"]` — array of login strings, not `{login}` objects |
...
| ID | Scenario | Expected |
|---|---|---|
| TC-IFSEAM-010 | GitHub leaf: `itp_list_forbidden_combos open MergeMill 100` against a fixture matching the forbidden combo | each row's keys are EXACTLY `number,labels,assignees` (no more, no fewer) — unconditional widening, no `FIELDS_CSV` arg on this verb |
