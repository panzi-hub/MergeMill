...
| TC-ENTRY-SHIM-004 | Symlinked entry sources libs from skill tree while reading conf from symlink dir | wrapper loads `lib-config.sh` from skill tree AND sources project-side `MergeMill.conf` (PROJECT_ID from project conf) |
...
| TC-ENTRY-SHIM-012 | conf lookups still use the unresolved dir (INV-14 preserved) | `load_MergeMill_conf` / `MergeMill.conf` sources use `CONF_DIR`/`SCRIPT_DIR` (BASH_SOURCE-based), never `LIB_DIR` |
...
| TC-ENTRY-SHIM-025 | `--doctor` missing conf | exit 1 (or warn), flags missing `MergeMill.conf` |
| TC-ENTRY-SHIM-026 | `--doctor` + `--dry-run` both pure read-only | no fs mutation |
| TC-ENTRY-SHIM-027 | real project-local files (MergeMill.conf, deploy.sh) never overwritten/pruned | preserved |

## E2E (`tests/e2e/test-entry-point-startup.sh`)

| ID | Scenario | Expected |
|---|---|---|
| TC-ENTRY-SHIM-030 | temp project: symlinked `MergeMill-review.sh` entry + project conf + one lib (`lib-review-*.sh`) deliberately NOT symlinked → run startup path to first log line | wrapper sources all libs from skill tree, loads project conf, reaches config-validated startup without `No such file` crash |
