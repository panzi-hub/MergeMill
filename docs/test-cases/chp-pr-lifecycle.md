...
| TC-CHP-CLOSEKW-CALLER | `MergeMill-dev.sh` interpolates `chp_close_keyword`'s output, not a hardcoded literal |
...
PUBLIC seam (`CODE_HOST=degraded` + `MERGEMILL_PROVIDERS_DIR`).
