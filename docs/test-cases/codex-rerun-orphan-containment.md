...
2. `lib-review-poll.sh::_reap_fanout_controller_subshells` (new) +
   `MergeMill-review.sh` — direct-PID reap of the fan-out controller subshells
...
3. `adapters/codex.sh::_run_codex_review` (liveness gate) +
   `MergeMill-review.sh` (rc-sidecar write guard) — a re-run iteration
...
| TC-RFCS-006 | wrapper wiring: `MergeMill-review.sh` calls `_reap_fanout_controller_subshells` at the post-resolution reap call site, fed `${_fanout_pids[@]:-}` | source-of-truth grep |
...
source-of-truth grep on `MergeMill-review.sh`.
...
| ID | Scenario | Expected |
|----|----------|----------|
| TC-RSG-004 | a controller subshell is reaped (Layer 2) BEFORE it wrote its `.rc` sidecar | the wrapper's existing sidecar-missing branch (`AGENT_LAUNCH_RC["$_sid"]=1`, `MergeMill-review.sh` around the `_rc_file` read) classifies it gracefully — no crash/hang; verdict resolution for OTHER agents is unaffected (this is pre-existing code, pinned as a regression guard, not new behavior) |
...
`MergeMill-review.sh` uses, rather than reimplementing them:
