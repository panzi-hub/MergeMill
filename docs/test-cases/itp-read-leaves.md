...
| ID | Caller | Asserts |
|---|---|---|
| TC-GT-COUNT | `count_active` | emits `issue list --repo R --state open --limit 100 --label MergeMill --json labels -q '… \| length'` byte-identical; returns an INTEGER |
| TC-GT-NEW | `list_new_issues` | `--json number,labels,title` + the no-state subtraction `-q` byte-identical |
| TC-GT-PREV | `list_pending_review` | `--label MergeMill,pending-review --json number,labels` + INV-25 subtraction byte-identical |
...
| ID | Asserts |
|---|---|
| TC-CAP-NEG0 | `ISSUE_PROVIDER=degraded` + `MERGEMILL_PROVIDERS_DIR=<fixture>` → `itp_caps server_side_state_negation` = `0` (the read-side caps=0 branch is reachable through the PUBLIC seam) |
