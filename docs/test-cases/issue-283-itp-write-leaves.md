...
| TC-GT-PROVISION-CREATE | `itp_github_provision_states MergeMill 0E8A16 "desc"` (not-exists) | create argv == `label create MergeMill --repo $REPO --color 0E8A16 --description desc` |
...
+ `MERGEMILL_PROVIDERS_DIR`). Leaf dispatch stubbed inline (fixture endorses this).
