...
| TC-W1B-PARITY-020 | `MergeMill-review.sh` no-auto-close gate | labels include `no-auto-close` | same `HAS_NO_AUTO_CLOSE=true` gate decision as OLD |
| TC-W1B-PARITY-021 | `MergeMill-review.sh` no-auto-close gate | labels exclude `no-auto-close` | same `HAS_NO_AUTO_CLOSE=false` gate decision as OLD |
...
| TC-W1B-PARITY-040 | `MergeMill-dev.sh` issue-body fetch (title,body,comments) | normal fixture | normalized object contains the SAME title/body TEXT as OLD (labels/comments shape change documented, not parity-pinned per R5) |
| TC-W1B-PARITY-041 | `MergeMill-dev.sh` resume-fallback fetch (title,body) | normal fixture | same title/body TEXT as OLD |
...
- `test-itp-read-task-b5b7.sh` (entire file — the two byte-identical argv
  pins for the `MergeMill-dev.sh` issue-body read and the
  `MergeMill-review.sh` no-auto-close gate no longer apply to an abstract
  contract).
...
| `MergeMill-dev.sh` issue-body fetch (×2) | drops the forwarded `-q '.'` — the normalized object is embedded directly |
| `MergeMill-review.sh` no-auto-close gate | `-q '[.labels[].name] \| any(...)'` → `\| jq -r '.labels \| any(...)'` (no `.name` — already normalized) |
