...
and migrate all **7** raw PR-comment write sites (`MergeMill-review.sh:3342,3538`
...
|----|------|----------------------------------|
| TC-CPC-010 | `MergeMill-review.sh:3342` | `chp_pr_comment "$PR_NUMBER" \` … `--body "Auto-merge failed: PR is CONFLICTING …" 2>/dev/null \|\| true` |
| TC-CPC-011 | `MergeMill-review.sh:3538` | `if ! _comment_err=$(chp_pr_comment "$PR_NUMBER" \` … `2>&1 >/dev/null); then` (capture form) |
...
| ID | Scenario | Expected |
|----|----------|----------|
| TC-CPC-030 | Executable (non-comment) raw `gh pr comment` in `MergeMill-review.sh` | 0 |
