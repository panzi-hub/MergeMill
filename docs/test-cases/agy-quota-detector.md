...
against `MergeMill-review.sh` as the source of truth.
...
## TC-AGYQ-SRC: source-of-truth wiring (`MergeMill-review.sh`)
...
| TC-AGYQ-SRC-06 | `bash -n` parses `lib-review-agy.sh` AND `MergeMill-review.sh` |
...
| TC-AGYQ-REG-03 | INV-40 aggregation truth table unchanged (quota agy still dropped, not a deciding FAIL) — existing `test-MergeMill-review-multi-agent` / `test-review-agent-timeout` stay green |
