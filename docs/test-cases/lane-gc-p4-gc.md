...
| TC-LGC4-021 | Legacy-sig sleep: `MERGEMILL_CONF_LOADED_FROM` set ∧ `CC_USER=MergeMill-dev-bot` ∧ `ppid==1`, age ≥ 600s | **would-kill (legacy)** — `would_kill_legacy_signature` counter increments |
| TC-LGC4-022 | Legacy-sig-WITHOUT-CC_USER decoy: `MERGEMILL_CONF_LOADED_FROM` set (simulating an operator hand-sourcing `MergeMill.conf` in an SSM debug shell), `CC_USER` UNSET, `ppid==1` | **skip** — the `∧` conjunction (not `∨`) means a bare conf-load signature alone never authorizes a kill (design falsekill:F1 / R15) |
...
| TC-LGC4-026 | Mid-upgrade legacy LIVE wrapper's daemon: spawn a fake `MergeMill-dev.sh`-argv-matching process (the "live wrapper"), then a CHILD of it with `ppid==1`-simulated legacy signature (own `setsid`, no lane dir) | **skip** — rule 2.4's live-wrapper-ancestry gate (`_gc_pgid_in_live_wrapper_ancestry`) protects it even though its ppid/signature would otherwise match rule 2.1's legacy arm |
...
| TC-LGC4-028 | Launcher-bridge live wrapper: the "wrapper" process's own argv does NOT literally contain `MergeMill-dev.sh` (simulating an exec-chain launcher bridge), but a live member of its OWN process group does | rule 2.3's `pgrep -g $PG -f 'MergeMill-(dev\|review)\.sh'` matches on the GROUP, not the single top argv — **no kill** (group excluded) |
