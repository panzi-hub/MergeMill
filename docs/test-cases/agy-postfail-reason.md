...
post), plus source-of-truth assertions against `MergeMill-review.sh`.

The breadcrumb path is `pid_dir_for_project()/verdict-postfail-<session_id>`. Tests
pin `MERGEMILL_PID_DIR` (and `PROJECT_ID`) to a temp dir so the path is hermetic.
...
## TC-PF-SRC: source-of-truth wiring (`MergeMill-review.sh`)
...
| TC-PF-SRC-06 | `bash -n` parses `lib-review-postfail.sh` AND `post-verdict.sh` AND `MergeMill-review.sh` |
...
| TC-PF-REG-02 | a post-failed agent is STILL dropped `unavailable` from the vote — INV-40 aggregation truth table unchanged (`test-MergeMill-review-multi-agent` / `test-review-agent-timeout` stay green) |
