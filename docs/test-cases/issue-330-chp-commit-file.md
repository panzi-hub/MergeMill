...
| ID | Scenario | Expected |
|----|----------|----------|
| TC-CCF-040 | `upload-screenshot.sh` resolves `lib-code-host.sh` via `readlink -f` skill-tree idiom (not `$_SCRIPT_DIR`) | source-grep finds the `readlink -f` + `../../MergeMill-dispatcher/scripts/lib-code-host.sh` resolution |
