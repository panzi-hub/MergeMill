# Test cases — issue #296: migrate `MergeMill-dev.sh` resume comment-reads behind `itp_list_comments`

Tracks the migration of the two resume-time review-feedback comment scanners in
`skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh` from raw
...
- A **static guard** asserts the 2 old `gh issue view ... --json comments -q`
  sites are GONE from `MergeMill-dev.sh`.
