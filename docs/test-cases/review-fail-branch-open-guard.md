...
Test file: `tests/unit/test-MergeMill-review-fail-branch-open-guard.sh`

Two-pronged, mirroring `test-MergeMill-review-mergeable-gate.sh` (the wrapper is
...
2. **Source-of-truth greps** against `MergeMill-review.sh` — assert the hoisted
