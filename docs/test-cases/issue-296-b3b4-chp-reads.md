...
| ID | Asserts |
|---|---|
| TC-308-AC7-DEV-CREATE | `MergeMill-dev.sh` calls `drain_agent_pr_create "$ISSUE_NUMBER" "$REPO"` |
| TC-308-AC7-DEV-BOT | `MergeMill-dev.sh` calls `drain_agent_bot_triggers "$ISSUE_NUMBER" "$REPO" …` |
| TC-308-AC7-REVIEW-BOT | `MergeMill-review.sh` calls `drain_agent_bot_triggers "$ISSUE_NUMBER" "$REPO" …` |
...
| `tests/unit/test-MergeMill-review-sequential-e2e.sh` | TC-SE2E-FETCH | the `_fetch_harness` sources `lib-code-host.sh` so `chp_pr_view` is live; new assert the stub OBSERVED `gh pr view` through the verb |
