...
The original Bug C hypothesis (downstream investigation note) claimed the dev wrapper's exit path flipped `approved` issues back to `in-progress`, forming a loop. Code inspection of `MergeMill-dev.sh::cleanup` proved that hypothesis WRONG — every label-edit branch of the cleanup trap only `--remove-label "in-progress"`; none re-add it.
...
|----|---|---|---|
| TC-PREV-001 | `MergeMill, pending-review, approved` | `[]` | Approved is terminal; selector must not pick |
| TC-PREV-002 | `MergeMill, pending-review, stalled` | `[]` | Stalled is terminal; same rule |
| TC-PREV-003 | `MergeMill, pending-review` | one entry | Pre-existing happy path |
| TC-PREV-004 | `MergeMill, pending-review, reviewing` | `[]` | Pre-existing `reviewing` exclusion still works |

### `list_pending_dev`

| ID | Labels on fixture | Returns | Why |
|----|---|---|---|
| TC-PDEV-001 | `MergeMill, pending-dev, approved` | `[]` | Approved-side defense |
| TC-PDEV-002 | `MergeMill, pending-dev, stalled` | `[]` | Stalled-side defense |
| TC-PDEV-003 | `MergeMill, pending-dev` | one entry | Pre-existing happy path |
