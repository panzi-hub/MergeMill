...
| TC-PAML-SRC-04 | `MergeMill-review.sh` passes `bash -n` |

## Backward-compat (must stay green)

- `tests/unit/test-MergeMill-review-per-agent-model.sh` — all existing INV-41
  `TC-PAM-*` cases.
...
- `tests/unit/test-post-verdict.sh`, `test-MergeMill-review-verdict-via-helper.sh`
