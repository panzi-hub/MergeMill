...
|---|---|---|
| TC-STR-001 | Extract `cleanup()` from `MergeMill-dev.sh`, grep the source-order of the Agent Session Report post vs. `drain_agent_pr_create` / `drain_agent_bot_triggers` | Session-report post's line number < `drain_agent_pr_create` call's line number < `drain_agent_bot_triggers` call's line number |
