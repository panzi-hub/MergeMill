...
Covers the new `skills/MergeMill-dispatcher/scripts/post-verdict.sh` helper and
the three `build_review_prompt` routing spots in `MergeMill-review.sh`.

Test scripts:
- `tests/unit/test-post-verdict.sh` — helper behavior (stubbed `gh`).
- `tests/unit/test-MergeMill-review-verdict-via-helper.sh` — prompt source-of-truth.
...
## Prompt source-of-truth — `tests/unit/test-MergeMill-review-verdict-via-helper.sh`
