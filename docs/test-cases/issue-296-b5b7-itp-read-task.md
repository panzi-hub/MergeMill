...
`MergeMill-dev.sh:1174`):

| Tag | File:line | Verb call |
|---|---|---|
| B5 | `MergeMill-dev.sh:887` | `itp_read_task "$ISSUE_NUMBER" title,body,comments -q '.'` |
| B7 | `MergeMill-review.sh:3439` | `itp_read_task "$ISSUE_NUMBER" labels -q '[.labels[].name] \| any(. == "no-auto-close")'` |
...
| ID | Assertion |
|---|---|
| TC-RT-B5-SRC | `MergeMill-dev.sh` defines `ISSUE_BODY=$(itp_read_task "$ISSUE_NUMBER" title,body,comments -q '.')` and has NO raw `gh issue view … title,body,comments` executable line |
| TC-RT-B7-SRC | `MergeMill-review.sh` defines `HAS_NO_AUTO_CLOSE=$(itp_read_task "$ISSUE_NUMBER" labels …)` and has NO raw `gh issue view … --json labels` executable line at the no-auto-close gate |
