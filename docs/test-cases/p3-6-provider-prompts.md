...
parameterizable agent-prompt heredoc-prose sites (9 `MergeMill-dev.sh`, 10
`MergeMill-review.sh`, 1 `lib-review-bots.sh`) and (b) 13 executable
github-gated residue sites that STAY baselined (5 `lib-auth.sh`, 3
`check-provider-cutover.sh`, 1 `MergeMill-dev.sh` `command -v gh` probe, 4
`MergeMill-review.sh` — the `gh api user` bot-login fallback block + its 2
...
| TC-P36-017 | Render `review.watch_ci_checks`; syntax-check with `bash -n` (no fence — this key renders bare, spliced directly into a bash heredoc after `sleep 10` in `MergeMill-review.sh`'s Step 0 rebase block) | valid shell — a real poll loop on `.head_pipeline.status`, not an English sentence that would be a syntax error at that splice point |
...
| ID | Scenario | Expected |
|----|----------|----------|
| TC-P36-020 | `MergeMill-dev.sh` non-comment raw-`gh` line count (the RE2-safe `(^|[^A-Za-z_-])gh ` scan, matching `check-provider-cutover.sh`'s detector) | exactly 1 — the `command -v gh &>/dev/null` presence probe (executable, (b), unchanged) |
| TC-P36-021 | `MergeMill-review.sh` non-comment raw-`gh` line count | exactly 4 — the `gh api user` bot-login fallback assignment, its 2 WARN diagnostics, and the INV-33 `gh issue close … --reason completed` interim close (all executable, (b), unchanged) |
...
| TC-P36-023 | `MergeMill-dev.sh` / `MergeMill-review.sh` / `lib-review-bots.sh` each source `lib-provider-prompts.sh` BEFORE `lib-review-bots.sh` is sourced (dev/review wrappers) or is available at call time (lib-review-bots.sh, sourced by the wrappers) | present, correct order |
...
| TC-P36-031 | Per-file breakdown of the 13 surviving (b) sites | `lib-auth.sh`=5, `check-provider-cutover.sh`=3, `MergeMill-dev.sh`=1, `MergeMill-review.sh`=4 |
