...
| ID | Scenario | Expected |
|---|---|---|
| TC-VERDICT-ARTIFACT-011 | Path honors `XDG_STATE_HOME` when set | `$XDG_STATE_HOME/MergeMill-<p>/runs/<rid>/verdict-<agent>.json` |
| TC-VERDICT-ARTIFACT-012 | Path falls back to `$HOME/.local/state` when XDG unset | `$HOME/.local/state/MergeMill-<p>/runs/<rid>/verdict-<agent>.json` |
