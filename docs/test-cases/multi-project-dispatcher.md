...
## TC-MP-002: MERGEMILL_CONF env propagated per iteration

**Given** PROJECTS=("/tmp/conf-a" "/tmp/conf-b")
**When** dispatcher-multi-tick.sh runs (with a stub that records `$MERGEMILL_CONF`)
...
**Given** DISPATCHER_CONF env unset (and no $HOME/.MergeMill/dispatcher.conf, no $XDG_CONFIG_HOME/.../dispatcher.conf)
**When** dispatcher-multi-tick.sh runs
...
**Then** the loop body runs `dispatcher-tick.sh` inside parentheses (subshell isolation), passes MERGEMILL_CONF as a per-iteration env, and does NOT exit on per-project failure.
