...
Issue: #142 — `fix(MergeMill-dev): step-12 summary comment posts as host gh user instead of bot in app mode`

## Background

The MergeMill-dev wrapper sets up a `${_LIB_AUTH_DIR}` PATH-prepend symlink so
...
The fix is documentation-only: agent-facing skill docs and the MergeMill-mode
...
**Setup**: throwaway issue in a test repo with the MergeMill label and the
required `## Dependencies` / `## Testing Requirements` sections so the
dispatcher will pick it up.

**Steps**:
1. Run `bash scripts/MergeMill-dev.sh --issue <N> --mode new` against the
   throwaway issue.
...
`MergeMill.conf` to `GH_AUTH_MODE=token` and `gh auth status` shows the host
user logged in.

**Steps**:
1. Run `bash scripts/MergeMill-dev.sh --issue <N> --mode new`.
...
**Setup**: same as TC-COMMENT-001, but the project's MergeMill.conf declares
...
- All files under `skills/MergeMill-dev/references/*.md`
...
default wrapper. The MergeMill-mode reference and review-commands reference
...
| `MergeMill-mode.md` documents the rule distinguishing `bash scripts/gh` (status/summary) from `bash scripts/gh-as-user.sh` (review-bot triggers) | Manual review of the PR diff; reinforced by TC-COMMENT-003 (regression guard) |
