...
A **consumer lib** is any `skills/MergeMill-dispatcher/scripts/lib-*.sh` that
...
`test-MergeMill-review-verdict-via-helper.sh` (`_FN_SLICE`),
...
- `tests/unit/test-token-split-234.sh` — TC-TOKEN-SPLIT-072's two `bash -c`
  ...
  `lib-review-e2e.sh` source, mirroring `MergeMill-review.sh`'s
