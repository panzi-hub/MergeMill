...
|---|---|
| `dev-*` dispatch never SIGTERMs `MergeMill-review.sh`, and vice versa | TC-PGREP-001, TC-PGREP-002 |
...
| Cross-project isolation (multiple MergeMill projects on same host, overlapping issue N) | TC-PGREP-005 |
...
**Given**:
- A live decoy process whose argv0 path ends in `MergeMill-review.sh` and whose
...
- The decoy `MergeMill-review.sh --issue 998877` process is **still alive**.

### TC-PGREP-002 — review must NOT kill live dev wrapper (mirror of TC-PGREP-001)

**Given**:
- A live decoy `MergeMill-dev.sh --issue 998877`.
...
**Expect**:
- `kill_stale_wrapper` exits 0.
- The decoy `MergeMill-dev.sh --issue 998877` is **still alive**.

### TC-PGREP-003 — same-type orphan IS group-killed (no regression of #109 fallback)

**Given**:
- A live decoy `MergeMill-dev.sh --issue 998877` (orphan; no PID file).
...
**Given**:
- A live decoy `MergeMill-dev.sh --issue 99` (note: NOT 9).
...
- Two distinct `PROJECT_DIR` paths (e.g. `/tmp/.../proj-a/` and
  `/tmp/.../proj-b/`), each with its own `scripts/MergeMill-dev.sh` copy.
- A live decoy at `${PROJ_B}/scripts/MergeMill-dev.sh --issue 998877`
  (project B's wrapper).
...
### TC-REGEX-001 — `TYPE=dev-new` selects `${PROJECT_DIR}/scripts/MergeMill-dev\.sh`

Static-behavioral assertion: `script_re` for `TYPE=dev-new` must contain
the regex-quoted `${PROJECT_DIR}/scripts/` AND `MergeMill-dev\.sh`, and
...
regex-quoted `${PROJECT_DIR}/scripts/` AND `MergeMill-review\.sh`, and
...
matches `${PROJECT_DIR}/scripts/MergeMill-(dev|review)\.sh`. The project
