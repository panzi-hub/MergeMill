...
**When** multi-tick runs
**Then** both projects' ticks are attempted; local goes via source-MergeMill.conf path, remote goes via inline-eval path; per-project failure isolation preserved.
...
**When** multi-tick processes it
**Then** behavior is byte-identical to PR-8: source MergeMill.conf via MERGEMILL_CONF env override, run dispatcher-tick.sh in subshell, exit code captured.
