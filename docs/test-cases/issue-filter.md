...
| TC-IFILT-046 | `label:MergeMill` | rejected — the baseline label |
...
| TC-IFILT-123 | path-entry project (`MergeMill.conf` file, not inline) with `ISSUE_FILTER` set | no charset restriction — the file is sourced directly, same as any other conf var |
