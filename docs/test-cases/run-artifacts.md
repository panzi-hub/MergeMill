...
| ID | Scenario | Expected |
|----|----------|----------|
| TC-RUN-ARTIFACTS-010 | `run_dir_for <run-id>` under `XDG_STATE_HOME=/x` | echoes `/x/MergeMill-proj/runs/<run-id>` |
| TC-RUN-ARTIFACTS-011 | `XDG_STATE_HOME` unset, `HOME=/h` | falls back to `/h/.local/state/MergeMill-proj/runs/<run-id>` |
| TC-RUN-ARTIFACTS-012 | parent `runs/` dir equals #233's `_verdict_artifact_dir` parent | same `…/MergeMill-proj/runs` prefix (coordination, not duplication) |
