...
| Secondary guard | Source grep over `lib-dispatch.sh` / `MergeMill-review.sh` | zero `--json`/`-q` tokens on the `chp_ci_status`/`chp_mergeable` caller lines (outside `providers/`) |
...
## Caller-line pins — `test-MergeMill-review-mergeable-gate.sh`
...
| `test-MergeMill-review-fail-branch-open-guard.sh` + `test-MergeMill-review-e2e-gate-open-guard.sh` open-guard line-order pins | Line-order structure of the PASS/fail branches | PASSES UNCHANGED |
