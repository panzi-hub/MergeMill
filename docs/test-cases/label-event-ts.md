...
|----|----------|-------|----------|
| TC-LABELTS-001 | (a) one labeled event for LABEL | timeline with 1 `labeled`/`MergeMill` event | its `created_at` |
| TC-LABELTS-002 | (b) multiple labeled events for LABEL | 2 `labeled`/`MergeMill` events | the FIRST (`.[0].created_at`) |
...
| TC-LABELTS-006 | (f) injection — selector NOT widened | LABEL=`MergeMill" or .label.name == "bug`, timeline has ONLY a `bug` event | empty (NOT the bug `created_at`) |
...
| TC-LABELTS-009 | argv-equivalence for `LABEL=MergeMill` | recorded `--jq` program | equals today's inline selector `map(select(.event == "labeled" and .label.name == "MergeMill")) \| (.[0].created_at // empty)`, against `repos/${REPO}/issues/<n>/timeline` |
...
| ID | Scenario | Expected |
|----|----------|----------|
| TC-LABELTS-030 | migrated site calls `itp_label_event_ts "$issue_num" "MergeMill"` (NOT raw `gh api`) | source pin in `dispatcher-tick.sh` |
...
| TC-FINAL2-042 (flipped) | the timeline `gh api` survivor is now GONE (was "stays" in #321) — co-required with: (i) `gh api .*timeline` ABSENT from `dispatcher-tick.sh`; (ii) `itp_label_event_ts "$issue_num" "MergeMill"` PRESENT; (iii) the specific timeline wire-string removed from the baseline | in `test-final2-marker-scanners.sh` |
