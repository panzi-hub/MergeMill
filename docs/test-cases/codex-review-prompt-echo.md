...
Implementation: `skills/MergeMill-dispatcher/scripts/lib-review-codex.sh`,
...
## Terminal routing — all-unavailable path (`test-MergeMill-review-multi-agent.sh`)
...
## Wrapper fallback (behavioral) — `MergeMill-review.sh` INV-62 fallback
...
| ID | Scenario | Expected |
|----|----------|----------|
| TC-CXRS-MAL-WIRE-01 | The wrapper fallback handles the `malformed` token (does not compose/post a body from it) | grep pins the `malformed` branch in `MergeMill-review.sh` |

## E2E (stub fleet)

| ID | Scenario | Expected |
|----|----------|----------|
| TC-CXRS-MAL-E2E-01 | Drive `MergeMill-review.sh` with a stubbed codex emitting the prompt-echo shape (rc 0) alongside a surviving agent | no phantom FAIL vote; the drop reason names `malformed-output`; the aggregate is decided by the surviving agent |
