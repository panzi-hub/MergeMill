...
**Setup**: Review wrapper has parsed a PASS verdict for an open PR. PR is mergeable. Issue has `reviewing` + `MergeMill` labels and the PR body contains `Closes #N`.

**Action**: Wrapper runs `gh pr merge --squash --delete-branch` and the call returns 0.

**Expected**:
- Wrapper does NOT call `gh issue close`.
- Wrapper removes `reviewing` and `MergeMill` from issue, adds `approved`.
...
- Wrapper does NOT remove `MergeMill` (so the dispatcher selector still matches).
- Issue state is `OPEN`.
...
- Throughout the flow, no path inside the MergeMill wrappers calls `gh issue close`.
...
- Issue stays OPEN with `pending-dev` + `MergeMill` labels (next tick will retry, eventually hitting MAX_RETRIES → `stalled`).
...
**Setup**: Source-of-truth grep against `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh`.
...
**Setup**: Source-of-truth grep against `skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh`.
