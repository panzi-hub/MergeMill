...
- `tests/unit/test-MergeMill-review-per-agent-model.sh` — source-of-truth grep (dual-var alias)
...
- `lib-agent.sh` — the lib-layer `run_agent`/`resume_agent` var mismatch is the
  ...
  `MergeMill-dev.sh:734`) and is deferred to a separate follow-up issue.
- The `MergeMill-dev.sh` dev-resume path — out of scope; the corrected comment is
