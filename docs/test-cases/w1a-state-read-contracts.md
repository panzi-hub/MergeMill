...
| ID | Scenario | Expected |
|---|---|---|
| combo filter | 4 issues: terminal+transitional, MergeMill-only, terminal+transitional, terminal-only | only the two terminal+transitional issues survive |
...
| `docs/pipeline/spec-guard-map.json`'s `only-MergeMill-label` guard anchor | `contains(["in-progress"])` → `. == "in-progress"` (the new literal `list_new_issues` emits) |
...
ONLY `itp_read_task` call sites (`status.sh`, `MergeMill-review.sh:3530`,
