...
pronged (mirrors `test-MergeMill-review-per-agent-model.sh`):
...
3. **Source-of-truth greps** — assert the structural wiring in
   `MergeMill-review.sh` without executing the wrapper.

Test file: `tests/unit/test-MergeMill-review-per-agent-launcher.sh`.
...
## Source-of-truth greps (against `MergeMill-review.sh` + `lib-review-resolve.sh`)
...
| TC-PAL-SRC-08 | `MergeMill.conf.example` documents `AGENT_REVIEW_LAUNCHER_<AGENT>` with a `codex` example |
...
`test-MergeMill-review-multi-agent` (incl. TC-MAR-SRC-07 non-claude zeroing),
`test-MergeMill-review-per-agent-model`, `test-MergeMill-review-prompt`,
`test-lib-agent-per-side-launcher`, `test-lib-agent-per-side-cmd`,
`test-MergeMill-launcher-verdict-fresh`, plus `bash -n`.
