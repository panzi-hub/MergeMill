...
running it) plus **source-of-truth greps** against `MergeMill-review.sh` for the
...
`gh`), so we follow the established `test-MergeMill-review-multi-agent.sh` /
...
| TC-RPB-DOC-06 | `MergeMill.conf.example` cross-references the two windows |

## Existing suites that MUST stay green (no regression)

- `tests/unit/test-MergeMill-review-multi-agent.sh`
- `tests/unit/test-e2e-mode-command.sh`
- `tests/unit/test-MergeMill-review-prompt.sh`
- `tests/unit/test-MergeMill-review-per-agent-model.sh`
- `tests/unit/test-MergeMill-review-verdict-trailer.sh`
