...
| TC-ITV-003 | **CSV multi-remove, empty add** `… 42 "reviewing,MergeMill" ""` | two `--remove-label`, NO `--add-label` (empty-side omitted) |
...
|---|---|---|
| TC-ITV-020 | **A1 dev:835** migrated | `MergeMill-dev.sh` block has `itp_transition_state "$ISSUE_NUMBER" "in-progress,pending-dev" "pending-review"` AND the `\|\| log "WARNING: Failed to update issue labels"` framing survives on the same logical call |
| TC-ITV-021 | **A2 review:3466** migrated | `MergeMill-review.sh` has `itp_transition_state "$ISSUE_NUMBER" "reviewing,MergeMill" "approved"` AND `2>/dev/null \|\| true` framing survives |
...
| TC-ITV-023 | **B review:3552** migrated | `MergeMill-review.sh` has `if ! _edit_err=$(itp_transition_state "$ISSUE_NUMBER" "reviewing" "pending-dev" 2>&1 >/dev/null); then` — the stderr-capture framing preserved |
...
| TC-ITV-041 | **RED-without-reanchor** — the 3 re-anchored `code_sites` entries (`dev-trap-success-pr`, `review-pass-merged`, `review-no-pr`) point at the migrated anchors, NOT the raw `gh issue edit` literals | assert each entry's `anchor` is the migrated literal AND is NOT one of `--add-label "pending-review"` / `--remove-label "MergeMill"` / `--remove-label "reviewing"` (test FAILS if the map still lists the raw literals) |
