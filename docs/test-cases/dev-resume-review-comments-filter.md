...
`MergeMill-dev.sh::resume` constructs a prompt that includes the most
...
MergeMill review`, `Moving to pending-review for retry`,
...
| TC-RFB-003 | Real `Review findings` AT T0, then dispatcher `Dispatching MergeMill review...` AT T1 | the real comment |
