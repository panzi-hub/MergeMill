...
| TC-RESET-REMOTE-3 (dispatcher-tick PTL branch, `tests/unit/test-MergeMill-launcher-verdict-fresh.sh::TC-PTL-007`-style harness) | PTL branch, `EXECUTION_BACKEND=remote-aws-ssm`, remote truncate succeeds | `dev-new` dispatched after the remote truncate call. |
