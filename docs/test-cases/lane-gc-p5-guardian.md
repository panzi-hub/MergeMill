...
1. **`skills/MergeMill-dispatcher/scripts/lib-guardian.sh`** (new
...
2. **Wrapper integration** (`MergeMill-dev.sh`, `MergeMill-review.sh`):
...
| TC-LGC5-101/102/103 | Per-file MINIMUM close-guard counts (`lib-agent.sh` ≥ 3, `lib-lane.sh` ≥ 2, `MergeMill-review.sh` ≥ 2) | catches a regression that removes ONE site while leaving another, which a presence-only check would miss |
...
| ID | Scenario | Expected |
|----|----------|----------|
| TC-LGC5-E2E-01a | The REAL, unmodified `MergeMill-dev.sh` driven against a fixture `gh` (token-mode auth stub + a minimal normalized `gh issue view`/comments response) and a fixture `claude` (drops a marker on launch, then sleeps) | the wrapper reaches the real agent spawn (fixture claude launches) |
