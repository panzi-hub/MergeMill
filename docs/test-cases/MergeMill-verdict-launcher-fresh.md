...
Replaces `tests/unit/test-MergeMill-review-verdict-regex.sh` cases TC-RVR-009 and TC-RVR-010 (which assert "no/wrong session-id → no-match"). Under the new actor+time-window model those comments DO match if the author and timestamp are right, so we re-test those scenarios for the new model and add new anti-spoof cases.
...
| TC-LCH-007 | MergeMill-dev.sh exports `CC_USER=MergeMill-dev-bot` and `CC_ROLE_KIND=dev` before run_agent | both env vars visible in claude's env |
| TC-LCH-008 | MergeMill-review.sh exports `CC_USER=MergeMill-review-bot` and `CC_ROLE_KIND=review` | both env vars visible in claude's env |
...
Tested by feeding synthetic JSONL log files into `is_session_completed` and exercising MergeMill-dev.sh's resume→fallback path with a stub claude that exits non-zero on resume.
...
| TC-PTL-005 | MergeMill-dev.sh MODE=resume, stub claude exits non-zero | fallback path mints NEW_SESSION_ID, posts standalone `Dev Session ID: \`${NEW_SESSION_ID}\`` comment to issue, then runs new session |
...
1. Confirm `AGENT_LAUNCHER` set in their `MergeMill.conf` causes `cc` env (`CLAUDE_CODE_USE_BEDROCK=1`, etc.) to flow into the wrapper. Verify by checking the next dispatch's `/tmp/agent-*-issue-N.log` JSONL `modelUsage` field — should now show `global.anthropic.claude-opus-4-7` instead of `anthropic.claude-haiku-4-5`.
2. Confirm `CC_USER=MergeMill-dev-bot` / `MergeMill-review-bot` shows up in their telemetry attribution.
