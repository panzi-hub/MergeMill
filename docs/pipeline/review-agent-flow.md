...
sequenceDiagram
    ...
    alt verdict PASS
        ...
        opt no-auto-close NOT set
            ...
            alt merge succeeded
                W->>GH: remove MergeMill and reviewing, add approved
                ...
                W->>GH: remove reviewing, add pending-dev (MergeMill KEPT)
            end
        end
        opt no-auto-close set
            W->>GH: remove reviewing, add approved (MergeMill KEPT)
        end
...
The wrapper binds the issue to the open PR that **closes** it via the shared `resolve_pr_for_issue` ([`lib-pr-linkage.sh`](../../skills/MergeMill-dispatcher/scripts/lib-pr-linkage.sh)) — GitHub's parsed close linkage, NOT a `#N` body mention:
...
The review wrapper supports three E2E modes via `E2E_MODE` in `MergeMill.conf`:
...
In `command` mode the wrapper substitutes the literal `${PR_NUMBER}` placeholder in `E2E_COMMAND`, `E2E_COMMAND_PRE_HOOKS`, and `E2E_COMMAND_EVIDENCE_PARSER` with the resolved PR number before pasting them into the prompt. Operators MUST single-quote those assignments in `MergeMill.conf` so the shell does not eagerly expand `${PR_NUMBER}` when sourcing the conf file. Unbraced `$PR_NUMBER` is rejected at config-validation time (would silently render as empty since the var is exported only inside the command-mode block, after substitution).
...
For the project-side contract (`E2E_COMMAND` semantics, evidence-block format, parser PR_HEAD_SHA usage), see `skills/MergeMill-review/references/e2e-command-mode.md`.
...
Post-resolution, `MergeMill-review.sh` runs **four** reap calls at the same call site, immediately after verdict resolution (see [Multi-agent fan-out (INV-40)](#multi-agent-fan-out-inv-40) above for the fan-out loop that populates their inputs):
...
`${XDG_STATE_HOME:-$HOME/.local/state}/MergeMill-<project>/runs/<run-id>/verdict-<agent>.json`
...
| Rebase worktree dir (Step 0) | `PROJECT_ID`, agent name, `PR_NUMBER` | `/tmp/rebase-${PROJECT_ID}-${agent}-pr-${PR_NUMBER}`; idempotent pre-clean before `git worktree add` (a crashed prior lane can't wedge a retry). Mirrored in `skills/MergeMill-review/references/merge-conflict-resolution.md`. |
...
4. Read no-auto-close label
   if no-auto-close set:
     ...
     −reviewing +approved (MergeMill and no-auto-close kept)
   else (auto-merge path):
     MERGE_OUT=$(gh pr merge --squash --delete-branch 2>&1); MERGE_RC=$?
     if MERGE_RC == 0:
       −MergeMill −reviewing +approved
       ...
       −reviewing +pending-dev (MergeMill retained)
...
3. Does NOT call `gh issue close`. Does NOT add `+approved`. Does NOT remove `MergeMill` (the dispatcher's `list_pending_dev` selector gates on `MergeMill`).
...
A maintainer running `/q review` plus a manual `gh pr merge` while the MergeMill review wrapper is in flight can cause the PR to be merged before the wrapper reaches the approve step. Without the guard, `gh pr review --approve` against a closed PR would fail noisily and the wrapper would fall into the approval-failure branch — incorrect, since the PR was already approved+merged by the human. The guard short-circuits to a silent `−reviewing` (no add) and exit 0.

Since [INV-54](invariants.md#inv-54-the-pr-still-open-guard-gates-all-pass-chain-exits-not-just-pass) (#196) this guard is **hoisted to the top of the `PASSED_VERDICT == true` chain** (see [§ PR-open guard (INV-54)](#pr-open-guard-inv-54)) so it also protects the INV-44 block-substantive / block-nonsubstantive branches — not just the approve/merge path. The old in-PASS-branch copy was removed (DRY). The original concurrent-merge motivation is unchanged. See [`state-machine.md` § Concurrent reviews on the same PR](state-machine.md#concurrent-reviews-on-the-same-pr) and [#31](https://github.com/panzi-hub/MergeMill/issues/31).
