...
operator knobs in `skills/MergeMill-dispatcher/scripts/lib-agent.sh`
...
- `MergeMill-dev.sh`     → `AGENT_LAUNCHER_ARGV=("${AGENT_DEV_LAUNCHER_ARGV[@]}")`
- `MergeMill-review.sh`  → `AGENT_LAUNCHER_ARGV=("${AGENT_REVIEW_LAUNCHER_ARGV[@]}")`
...
> `lib-config.sh::load_MergeMill_conf` which re-sources
> `MergeMill.conf`. If the rebind happened earlier, the conf's
...
- **Tokenization model** is unchanged — `eval` (same trust as
  `MergeMill.conf` itself), with WARN on empty-tokenize and ERROR
...
`MergeMill.conf.example` gets a new comment block above the per-CLI
...
- [INV-31](invariants.md#inv-31-operator-tunable-per-cli-flags-live-in-conf-not-in-lib-agentsh) — operator-tunable flags live in `MergeMill.conf`. The new vars follow that contract.
...
| PSL-S9 | Structural — `MergeMill-dev.sh` rebinds `AGENT_LAUNCHER_ARGV=("${AGENT_DEV_LAUNCHER_ARGV[@]}")` AFTER `source lib-auth.sh` (the position that survives the conf re-source, post-INV-37 fix on 2026-05-26) |
| PSL-S10 | Structural — `MergeMill-review.sh` rebinds `AGENT_LAUNCHER_ARGV=("${AGENT_REVIEW_LAUNCHER_ARGV[@]}")` AFTER `source lib-auth.sh` (same reason as PSL-S9) |
...
2. MergeMill-dev.sh: add `AGENT_LAUNCHER_ARGV=("${AGENT_DEV_LAUNCHER_ARGV[@]}")`
...
3. MergeMill-review.sh: symmetric, using `AGENT_REVIEW_LAUNCHER_ARGV`,
   after `source lib-auth.sh` + `lib-review-bots.sh` + `lib-review-verdict.sh`.
   PSL-S10 asserts placement structurally.
4. MergeMill.conf.example: add the operator-facing comment block.
