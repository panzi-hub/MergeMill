...
[`lib-error.sh`](../../skills/MergeMill-dispatcher/scripts/lib-error.sh).
...
|---|---|---|---|---|
| `ADT_CFG_MISSING_KEY` | A required `MergeMill.conf` key is unset | One of `PROJECT_ID` / `REPO` / `REPO_OWNER` / `REPO_NAME` / `PROJECT_DIR` is missing | Set the named key in the project's `scripts/MergeMill.conf` (see `MergeMill.conf.example`), then re-dispatch | issue-comment / dispatcher-alert |
| `ADT_CFG_PROJECT_DIR_INVALID` | The wrapper cannot enter `PROJECT_DIR` | `cd "$PROJECT_DIR"` failed (path missing or not a directory) | Fix `PROJECT_DIR` in `MergeMill.conf` so it points at the project checkout on the execution host, then re-dispatch | issue-comment |
...
| `ADT_CFG_LAUNCHER_PARSE` | An `AGENT_*_LAUNCHER` value does not tokenize as a shell argv list | The launcher string has unbalanced quotes / invalid shell words | Fix the launcher value in `MergeMill.conf` to a valid shell argv (e.g. `cc --role dev`), then re-dispatch | issue-comment / dispatcher-alert |
| `ADT_CFG_E2E_MODE_REQUIRED` | `E2E_ENABLED=true` but `E2E_MODE` is unset | E2E is enabled without an explicit mode | Set `E2E_MODE` to `none`, `browser`, or `command` in `MergeMill.conf`, then re-dispatch | issue-comment |
...
| `ADT_CFG_E2E_COMMAND_MISSING` | `E2E_MODE=command` but `E2E_COMMAND` is unset | Command-mode E2E selected without a command | Set `E2E_COMMAND` in `MergeMill.conf`, then re-dispatch | issue-comment |
| `ADT_CFG_E2E_PARSER_MISSING` | `E2E_MODE=command` but `E2E_COMMAND_EVIDENCE_PARSER` is unset | Command-mode E2E selected without an evidence parser | Set `E2E_COMMAND_EVIDENCE_PARSER` in `MergeMill.conf`, then re-dispatch | issue-comment |
...
| `ADT_CFG_REVIEW_BOTS_INVALID` | `REVIEW_BOTS` contains an unrecognized token | A `REVIEW_BOTS` entry is not a known bot short-name (`q` / `codex` / `claude` / a configured custom bot) | Fix `REVIEW_BOTS` in `MergeMill.conf` to a space-separated list of known bot short-names (or empty), then re-dispatch | issue-comment / dispatcher-alert |
| `ADT_CFG_EXECUTION_BACKEND_INVALID` | `EXECUTION_BACKEND` has an unrecognized value | The dispatcher's `EXECUTION_BACKEND` is not `local` or `remote-aws-ssm` | Set `EXECUTION_BACKEND` to `local` or `remote-aws-ssm` in `dispatcher.conf`/`MergeMill.conf` | dispatcher-alert |
| `ADT_CFG_ISSUE_FILTER_INVALID` | `ISSUE_FILTER` failed validation | The filter is malformed (compiler error), references a reserved pipeline-state/`MergeMill` label, or contains an `assignee:` atom against a provider whose caps lack `assignees=1` ([INV-121]) | Fix `ISSUE_FILTER` in `MergeMill.conf`/`dispatcher.conf` per the grammar in `MergeMill.conf.example`, then re-dispatch | dispatcher-alert |
| `ADT_CFG_ISSUE_SCAN_LIMIT_INVALID` | `ISSUE_SCAN_LIMIT` has an invalid value | The value is not a positive integer | Set `ISSUE_SCAN_LIMIT` to a positive integer (or unset for the default 100) in `MergeMill.conf`/`dispatcher.conf`, then re-dispatch | dispatcher-alert |

## Authentication class (`class: auth`)

| Code | Problem | Cause | Remediation | Surface |
|---|---|---|---|---|
| `ADT_AUTH_APP_CREDS_MISSING` | `GH_AUTH_MODE=app` but the App credentials are unset | The side's `*_APP_ID` / `*_APP_PEM` (dev/review/dispatcher) is missing | Set the App id + PEM path for this side in `MergeMill.conf`/`dispatcher.conf` (see `docs/github-app-setup.md`), then re-dispatch | issue-comment / dispatcher-alert |
...
- **`error_surface` resolves the `gh` proxy with a skill-tree fallback**, so a
  ...
  prefers the project-side `${MERGEMILL_CONF_DIR}/gh` symlink and falls back to
