...
The dispatcher runs as a cron job (default every 5 minutes) and is **stateless across ticks**. Each tick reads the current label set on every `MergeMill` issue, reads PID files for any wrappers it might be tracking, makes decisions, dispatches subprocesses, and updates labels. There is no in-memory carry-over from the previous tick.
...
- **Local project — file path**: a path to a per-project `MergeMill.conf` on this dispatcher box. The wrapper sources it via the `MERGEMILL_CONF` priority-1 path. Used when the dispatcher and project source live on the same machine.
- **Remote project — inline metadata block**: a multi-line string of bash assignments (REPO, EXECUTION_BACKEND=remote-aws-ssm, SSM_INSTANCE_ID, SSM_REMOTE_PROJECT_DIR, etc.). Used when the project source lives on a remote dev EC2 — the dispatcher box does NOT have the project's `MergeMill.conf`. The wrapper validates the block (KEY=VALUE lines only — defense-in-depth against accidental injection), eval's it in the subshell, and auto-derives `REPO_OWNER`/`REPO_NAME` from `REPO`.
...
- `--doctor` — read-only health report: broken/missing entry symlinks, stale per-lib symlinks (which it suggests pruning), `MergeMill.conf` presence + 0600 permissions, and entry-resolution sanity (does the real skill tree hold `lib-config.sh`?). Exits 0 clean / 1 on problems.
...
2. **Reserved-label gate** — no atom may reference a pipeline state label (`in-progress`, `reviewing`, `pending-review`, `pending-dev`, `stalled`, `approved`) or the `MergeMill` baseline label. Slice membership keyed on a state label would mutate as the state machine runs.
...
```
ACTIVE = count of issues labeled MergeMill AND (in-progress OR reviewing)
...
Find issues labeled `MergeMill` with **no other active state label** (no `in-progress`, `pending-review`, `reviewing`, `pending-dev`, `stalled`, `approved`).
...
3. **Post dispatch token** ([INV-18](invariants.md#inv-18-cold-start-grace-period-before-stale-detection), [INV-108](invariants.md#inv-108-every-dispatcher-tick-dispatch-site-acquires-a-controller-side-per-issuemode-marker-atomically-before-any-side-effect--a-losing-acquire-skips-cleanly-never-dispatches-the-marker-expires-via-ttl-never-wedging-the-issue-the-dispatch-token-gains-a-run-field-for-post-hoc-attribution)): `<!-- dispatcher-token: <id> at <iso> mode=dev-resume run=<run-id> -->` + "Resuming MergeMill development...".
...
| Agent invocation hangs in CLI | wrapper, not dispatcher | Bounded by future wall-clock timeout ([#60](https://github.com/panzi-hub/MergeMill/issues/60), [INV-13](invariants.md#inv-13-wall-clock-cap-on-agent-invocations)). Until then the dispatcher's Step 5a is the only way to clear it. |
