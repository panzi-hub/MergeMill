...
```
  ...
  │  ─────────────────                            │
  │  Box B — WRAPPER                              │
  │  • MergeMill-{dev,review}.sh                 │
  │  • lib-agent.sh — runs claude / codex / …     │
  │  • PID file: ~/.local/state/MergeMill-       │
...
- the PID-file schema (`${XDG_RUNTIME_DIR:-$HOME/.local/state}/MergeMill-${PROJECT_ID}/${kind}-${N}.pid`), stable since [INV-29];
