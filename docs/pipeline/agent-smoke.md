...
on every MergeMill-reviewed PR. Unit tests stub the CLIs, so the launch → auth
...
that project's `MergeMill.conf`, so the smoke inherits the live conf's env
...
1. **source the smoke lib** — `lib-agent.sh` loads `MergeMill.conf` (assigning
...
- `skills/MergeMill-review/references/e2e-command-mode.md` — the command-mode evidence parser the `SMOKE` lines feed.
