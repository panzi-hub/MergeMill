...
`skills/MergeMill-dispatcher/scripts/`. Line numbers are documentation (re-verify
...
issue carries `MergeMill` plus **at most one** active/terminal state label.
...
| `labels.no_auto_close` | `MergeMill-review.sh` (PASS branch) | reads `no-auto-close` membership to gate auto-merge skip |

Guards grounded here: `only-MergeMill-label`, `still-reviewing`,
...
| `pr.state` | `MergeMill-review.sh` `_pr_open_gate` (INV-54) | `gh pr view --json state`; ≠ OPEN ⇒ silent `−reviewing` |
| `pr.review_decision` | `MergeMill-review.sh` (INV-52, wrapper-owned `--approve`/`--request-changes`) | `gh pr view --json reviewDecision` |
| `pr.mergeable` | `MergeMill-review.sh:2480-2493` (PASS branch) | `gh pr view --json mergeable`; retried up to `MERGEABLE_RETRIES` (default 3) while `UNKNOWN`; classified by `lib-review-mergeable.sh::_classify_mergeable_gate` ([INV-44](invariants.md#inv-44-mergeable-hard-gate--a-conflicting-pr-can-never-reach-approved)) |
