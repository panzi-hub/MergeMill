...
knobs in `skills/MergeMill-dispatcher/scripts/lib-agent.sh` that
...
init block, MergeMill-dev.sh / MergeMill-review.sh entry blocks.
...
- `MergeMill-dev.sh`     → `AGENT_CMD="$AGENT_DEV_CMD"`
- `MergeMill-review.sh`  → `AGENT_CMD="$AGENT_REVIEW_CMD"`
...
sees the project-default `$AGENT_CMD` from `MergeMill.conf`. Two
...
`MergeMill.conf.example` gets a new comment block immediately after
...
| PSC-S9 | Structural — `MergeMill-dev.sh` contains `AGENT_CMD="$AGENT_DEV_CMD"` AFTER `source ${SCRIPT_DIR}/lib-auth.sh` (asserts the rebind survives lib-auth's transitive conf re-source) |
| PSC-S10 | Structural — `MergeMill-review.sh` same pattern (rebind AFTER `source ${SCRIPT_DIR}/lib-auth.sh`) |
...
3. MergeMill-review.sh: add `AGENT_CMD="$AGENT_REVIEW_CMD"` in the
   ...
   plausibly re-source `MergeMill.conf` in the future. PSC-S10
...
the operator example in `MergeMill.conf.example` already covers the
