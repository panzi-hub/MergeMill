# MergeMill Pipeline — Authoritative Flow Reference

This directory is the **single source of truth** for how the MergeMill pipeline behaves end-to-end. The dispatcher, dev-agent wrapper, and review-agent wrapper are all required to match what is documented here. Bug fixes and features MUST update the relevant doc(s) before (or in the same PR as) the code change. See [`CONTRIBUTING.md`](../../CONTRIBUTING.md) for the rule.

For the high-level overview and quick start, see [`docs/MergeMill-pipeline.md`](../MergeMill-pipeline.md). This directory is the spec; that file is the orientation.
