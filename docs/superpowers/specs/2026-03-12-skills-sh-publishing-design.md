# Design Spec: Publish MergeMill Dev Team Skills to skills.sh
...
Restructure the `MergeMill-dev-team` repository so that its skills are discoverable and installable via the [skills.sh](https://skills.sh) ecosystem (`npx skills add panzi-hub/MergeMill`). Users can install these skills into any supported coding agent IDE/CLI (Claude Code, Cursor, Windsurf, Gemini CLI, Kiro CLI, etc.) and use them for TDD-driven development, MergeMill PR review, and fully automated issue-to-merge pipelines.
...
| `MergeMill-dev` | `.claude/skills/MergeMill-dev/` | MergeMill dev mode (wraps github-workflow) |
| `MergeMill-review` | `.claude/skills/MergeMill-review/` | MergeMill PR review + E2E verification |

Additionally:
- `openclaw/skills/MergeMill-dispatcher/` — pipeline dispatcher (not under `.claude/skills/`)
...
| Wrapper scripts | `scripts/MergeMill-dev.sh`, `scripts/MergeMill-review.sh` |
...
| Dispatch | `openclaw/skills/MergeMill-dispatcher/dispatch-local.sh` |
| State manager | `.claude/hooks/state-manager.sh` |
| Config template | `scripts/MergeMill.conf.example` (`DEV_SKILL_CMD="/github-workflow"`) |
...
3. **Fragmented skills** — `github-workflow` and `MergeMill-dev` overlap significantly (MergeMill-dev is a thin wrapper over github-workflow)
...
|-------|------|-------------|
| **MergeMill-dev** | `MergeMill-dev` | Unified TDD development workflow — merges `github-workflow` + `MergeMill-dev`. Supports both interactive mode (ask user for approval at gates) and MergeMill mode (make decisions independently, report to GitHub issue) |
| **MergeMill-review** | `MergeMill-review` | PR code review, checklist verification, merge conflict resolution, E2E testing, auto-merge |
| **MergeMill-dispatcher** | `MergeMill-dispatcher` | GitHub issue scanner + task dispatcher for the MergeMill pipeline |
...
│   ├── MergeMill-dev/
│   │   ├── SKILL.md                         # Merged skill (interactive + MergeMill)
│   │   └── references/
...
npx skills add panzi-hub/MergeMill -s MergeMill-dev

# Install globally (user-level)
npx skills add panzi-hub/MergeMill -g

# Target a specific IDE
npx skills add panzi-hub/MergeMill -a cursor
npx skills add panzi-hub/MergeMill -a claude-code
npx skills add panzi-hub/MergeMill -a kiro
```
...
### 4.1 Merging `github-workflow` + `MergeMill-dev`

The merged `MergeMill-dev/SKILL.md` has two modes, determined by context:
...
**MergeMill mode**: When triggered by the `MergeMill-dev.sh` wrapper script
...
`openclaw/skills/MergeMill-dispatcher/` moves to `skills/MergeMill-dispatcher/`:

- `SKILL.md` moves to `skills/MergeMill-dispatcher/SKILL.md`
...
### 4.12 MergeMill.conf.example Update
...
The `MergeMill-dev.sh` wrapper uses `${DEV_SKILL_CMD:-/MergeMill-dev}` in its prompt — the default also changes from `/github-workflow` to `/MergeMill-dev`.
...
2. Move `.claude/skills/github-workflow/references/` → `skills/MergeMill-dev/references/`
...
5. Move `openclaw/skills/MergeMill-dispatcher/SKILL.md` → `skills/MergeMill-dispatcher/SKILL.md`
6. Move `openclaw/skills/MergeMill-dispatcher/dispatch-local.sh` → `scripts/dispatch-local.sh`
7. Remove `openclaw/` directory

### Phase 2: Rewrite skill content

8. Create merged `skills/MergeMill-dev/SKILL.md` (github-workflow + MergeMill-dev, cross-platform language)
9. Rewrite `skills/MergeMill-review/SKILL.md` with cross-platform language
10. Update `skills/MergeMill-dispatcher/SKILL.md` with updated script paths (`$PROJECT_DIR/scripts/dispatch-local.sh`)
11. Remove old `.claude/skills/` directory entirely (github-workflow/, MergeMill-dev/, MergeMill-review/)
...
17. Update `scripts/MergeMill-dev.sh` — change default `DEV_SKILL_CMD` from `/github-workflow` to `/MergeMill-dev`
18. Update `scripts/MergeMill.conf.example` — change `DEV_SKILL_CMD="/github-workflow"` to `DEV_SKILL_CMD="/MergeMill-dev"`
...
22. Update `README.md`:
    ...
    - Update skill path references
23. Update `docs/MergeMill-pipeline.md` — reflect new paths for skills, hooks, and dispatcher
...
| OpenClaw users expect `openclaw/skills/` path | Dispatcher skill not found by OpenClaw | Update OpenClaw cron command path to `skills/MergeMill-dispatcher/SKILL.md` |
...
1. `npx skills add panzi-hub/MergeMill` successfully installs all 3 skills
...
6. Existing MergeMill pipeline (`MergeMill-dev.sh`, `MergeMill-review.sh`, dispatcher) continues to work with new paths
...
- Modifying the MergeMill pipeline logic (only paths change)
