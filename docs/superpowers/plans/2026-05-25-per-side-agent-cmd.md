...
|---|---|---|
| `skills/MergeMill-dispatcher/scripts/lib-agent.sh` | Modify | Add `AGENT_DEV_CMD` / `AGENT_REVIEW_CMD` init lines; rewrite `AGENT_LAUNCHER` guard to check both sides. |
| `skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh` | Modify | Add one-line `AGENT_CMD="$AGENT_DEV_CMD"` immediately after `source ${SCRIPT_DIR}/lib-agent.sh`, before any other source. |
| `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` | Modify | Add one-line `AGENT_CMD="$AGENT_REVIEW_CMD"` in the same position. |
| `skills/MergeMill-dispatcher/scripts/MergeMill.conf.example` | Modify | Add `# AGENT_DEV_CMD / AGENT_REVIEW_CMD` operator-facing comment block. |
...
**Files:**
- Modify: `skills/MergeMill-dispatcher/scripts/lib-agent.sh` (line ~63 for the new init, lines 101–109 for the guard rewrite)
...
Run:
```bash
sed -n '58,110p' skills/MergeMill-dispatcher/scripts/lib-agent.sh
```
...
#     after source lib-agent.sh in both MergeMill-{dev,review}.sh
#
...
REVIEW_WRAPPER="$PROJECT_ROOT/skills/MergeMill-dispatcher/scripts/MergeMill-review.sh"
...
In `skills/MergeMill-dispatcher/scripts/lib-agent.sh`, find the existing line `AGENT_CMD="${AGENT_CMD:-claude}"` (line 60). Insert the following two lines immediately after it (preserving the existing `AGENT_DEV_MODEL` line below):

```bash
# Per-side AGENT_CMD overrides (INV-37). Default to AGENT_CMD so existing
# deployments are unchanged. MergeMill-dev.sh and MergeMill-review.sh
...
In `skills/MergeMill-dispatcher/scripts/lib-agent.sh`, find the existing guard at lines 101-109:
...
Run:
```bash
shellcheck -S error skills/MergeMill-dispatcher/scripts/lib-agent.sh tests/unit/test-lib-agent-per-side-cmd.sh
```
...
Run:
```bash
git add skills/MergeMill-dispatcher/scripts/lib-agent.sh tests/unit/test-lib-agent-per-side-cmd.sh
...
## Task 2: Add `AGENT_CMD="$AGENT_DEV_CMD"` override to `MergeMill-dev.sh`

**Files:**
- Modify: `skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh` (insert one line after line 22, immediately after `source "${SCRIPT_DIR}/lib-agent.sh"`)
...
# ---------------------------------------------------------------------------
echo ""
echo "=== PSC-S9: MergeMill-dev.sh structural placement ==="
# ---------------------------------------------------------------------------
...
assert_eq "MergeMill-dev.sh: AGENT_CMD=\$AGENT_DEV_CMD lands ≤2 lines after source lib-agent.sh" \
  "MATCH" "$hit"
```
...
- [ ] **Step 2.3: Read current `MergeMill-dev.sh` lines 21-25**

Run:
```bash
sed -n '21,25p' skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh
```
...
# Validate required config (loaded by lib-agent.sh from MergeMill.conf)
```
...
Run:
```bash
shellcheck -S error skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh
```
...
Run:
```bash
git add skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh tests/unit/test-lib-agent-per-side-cmd.sh
git commit -m "feat(MergeMill-dev): apply AGENT_DEV_CMD override after lib-agent source
...
## Task 3: Add `AGENT_CMD="$AGENT_REVIEW_CMD"` override to `MergeMill-review.sh`

**Files:**
- Modify: `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` (insert one line after line 22)
...
# ---------------------------------------------------------------------------
echo ""
echo "=== PSC-S10: MergeMill-review.sh structural placement ==="
# ---------------------------------------------------------------------------
hit=$(awk '
  /source "\$\{SCRIPT_DIR\}\/lib-agent\.sh"/ {
    found_source = NR
...
assert_eq "MergeMill-review.sh: AGENT_CMD=\$AGENT_REVIEW_CMD lands ≤2 lines after source lib-agent.sh" \
  "MATCH" "$hit"
```
...
- [ ] **Step 3.3: Read current `MergeMill-review.sh` lines 21-30**

Run:
```bash
sed -n '21,30p' skills/MergeMill-dispatcher/scripts/MergeMill-review.sh
```
...
```
source "${SCRIPT_DIR}/lib-agent.sh"
# Per-side AGENT_CMD override (INV-37). See MergeMill-dev.sh for the
...
Run:
```bash
shellcheck -S error skills/MergeMill-dispatcher/scripts/MergeMill-review.sh
```
...
Run:
```bash
git add skills/MergeMill-dispatcher/scripts/MergeMill-review.sh tests/unit/test-lib-agent-per-side-cmd.sh
git commit -m "feat(MergeMill-review): apply AGENT_REVIEW_CMD override after lib-agent source
...
## Task 4: Document `AGENT_DEV_CMD` / `AGENT_REVIEW_CMD` in `MergeMill.conf.example`

**Files:**
- Modify: `skills/MergeMill-dispatcher/scripts/MergeMill.conf.example` (insert ~15 lines after the `AGENT_PERMISSION_MODE="auto"` line at ~line 150, before the per-CLI blocks)

- [ ] **Step 4.1: Locate the insertion point**

Run:
```bash
grep -nE '^AGENT_PERMISSION_MODE=|^# --- claude block' skills/MergeMill-dispatcher/scripts/MergeMill.conf.example
```
...
Run:
```bash
grep -nE '^AGENT_REVIEW_EXTRA_ARGS=|^# --- claude block' skills/MergeMill-dispatcher/scripts/MergeMill.conf.example
```
...
Run:
```bash
bash -n skills/MergeMill-dispatcher/scripts/MergeMill.conf.example
```
...
Run:
```bash
git add skills/MergeMill-dispatcher/scripts/MergeMill.conf.example
git commit -m "docs(conf): document AGENT_DEV_CMD / AGENT_REVIEW_CMD in MergeMill.conf.example
...
**Rule**: `lib-agent.sh` exposes `AGENT_DEV_CMD` and `AGENT_REVIEW_CMD` as side-specific overrides of `AGENT_CMD`. Both default to `${AGENT_CMD:-claude}` so existing deployments are byte-for-byte unchanged. `MergeMill-dev.sh` sets `AGENT_CMD="$AGENT_DEV_CMD"` exactly once, immediately after sourcing `lib-agent.sh` and before any other `source`. `MergeMill-review.sh` sets `AGENT_CMD="$AGENT_REVIEW_CMD"` in the same position. After the override, the `case "$AGENT_CMD"` statements in `run_agent` / `resume_agent` dispatch to the right CLI per-side without any signature change.
...
**Consumer**: `MergeMill-dev.sh` and `MergeMill-review.sh` entry blocks set the active `AGENT_CMD` before any `run_agent` / `resume_agent` call.
...
- [INV-31](#inv-31-operator-tunable-per-cli-flags-live-in-conf-not-in-lib-agentsh) — the new vars are operator-tunable and live in `MergeMill.conf`, following INV-31's contract.
...
```bash
shellcheck -S error \
  skills/MergeMill-dispatcher/scripts/lib-agent.sh \
...
- `skills/MergeMill-dispatcher/scripts/lib-agent.sh` — 2-line init + guard rewrite
- `skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh` — 1-line override after `source lib-agent.sh`
- `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` — 1-line override (symmetric)
- `skills/MergeMill-dispatcher/scripts/MergeMill.conf.example` — operator-facing comment block with worked example
...
- [`docs/pipeline/per-side-agent-cmd.md`](https://github.com/panzi-hub/MergeMill/blob/feat/per-side-agent-cmd/docs/pipeline/per-side-agent-cmd.md)
- INV-37: per-side AGENT_CMD precedence
...
- [x] `bash -n MergeMill.conf.example` syntax OK
