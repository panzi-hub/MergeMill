...
|---|---|---|
| `skills/MergeMill-dispatcher/scripts/lib-agent.sh` | Modify | Add `agy)` cases to `run_agent` + `resume_agent`; add three private helpers next to `_codex_thread_*` helpers. |
| `skills/MergeMill-dispatcher/scripts/MergeMill.conf.example` | Modify | Add `# --- agy block ---` after the `# --- opencode block ---` block; mention `agy` in the AGENT_CMD comment header. |
| `skills/MergeMill-dispatcher/SKILL.md` | Modify | Add `agy` to "Supported Agent CLIs" listing. |
...
**Files:**
- Modify: `skills/MergeMill-dispatcher/scripts/lib-agent.sh` (insert after the existing opencode helpers around line 445, before the `acquire_pid_guard` function)
...
Run:
```bash
sed -n '305,450p' skills/MergeMill-dispatcher/scripts/lib-agent.sh
```
...
Find the line `acquire_pid_guard() {` in `skills/MergeMill-dispatcher/scripts/lib-agent.sh` (around line 455). Insert the following block *immediately before* that line, after the closing `}` of the last opencode helper:
...
(
  PATH="$PATH" \
  MERGEMILL_PID_DIR="$PID_DIR" \
  PROJECT_ID="testproj" \
  PROJECT_DIR="$TMPROOT" \
  AGENT_CMD=agy \
  bash -c '
    unset MERGEMILL_CONF AGENT_LAUNCHER AGENT_LAUNCHER_ARGV AGENT_PID_FILE
    source "'"$LIB"'"
...
Run:
```bash
shellcheck -S error skills/MergeMill-dispatcher/scripts/lib-agent.sh
```
...
Run:
```bash
git add skills/MergeMill-dispatcher/scripts/lib-agent.sh tests/unit/test-lib-agent-agy.sh
...
**Files:**
- Modify: `skills/MergeMill-dispatcher/scripts/lib-agent.sh` — `run_agent` case statement (around line 496) and `resume_agent` case statement (around line 662).
...
In `skills/MergeMill-dispatcher/scripts/lib-agent.sh`, find the line with `opencode)` inside the `run_agent` case statement (around line 605). Insert the following case *before* the `*)` (generic) fallback — i.e., after the `;;` that closes the `opencode)` case:
...
In `skills/MergeMill-dispatcher/scripts/lib-agent.sh`, find the `opencode)` case inside `resume_agent` (around line 738). Insert the following case after the closing `;;` of the `opencode)` case, before the `*)` fallback:
...
run_stderr=$(
  (
    ...
    bash -c '
      unset MERGEMILL_CONF AGENT_LAUNCHER AGENT_LAUNCHER_ARGV AGENT_PID_FILE _LIB_AGENT_AGY_MODEL_WARNED
      source "'"$LIB"'"
...
## Task 3: Add `# --- agy block ---` to `MergeMill.conf.example`

**Files:**
- Modify: `skills/MergeMill-dispatcher/scripts/MergeMill.conf.example`

- [ ] **Step 3.1: Locate the opencode block (the last block in the file)**

Run:
```bash
grep -n "^# --- opencode block ---" skills/MergeMill-dispatcher/scripts/MergeMill.conf.example
```
...
git commit -m "docs(conf): document agy block in MergeMill.conf.example
...
**Producer**: `_agy_capture_conversation` in `skills/MergeMill-dispatcher/scripts/lib-agent.sh`.
...
## Task 5: Add agy to `MergeMill-dispatcher/SKILL.md`
...
Run:
```bash
grep -n -i "claude\|codex\|gemini\|kiro\|opencode" skills/MergeMill-dispatcher/SKILL.md | head -10
```
...
Touches `skills/MergeMill-dispatcher/scripts/lib-agent.sh`, so the gate requires a `docs/pipeline/` change. Satisfied by:
