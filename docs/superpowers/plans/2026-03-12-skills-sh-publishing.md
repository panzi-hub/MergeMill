...
**Goal:** Restructure the repo so skills are discoverable via `npx skills add panzi-hub/MergeMill` — move skills to `skills/`, hooks to `hooks/`, merge github-workflow + MergeMill-dev, update all paths, create cross-platform documentation.
...
cd /data/git/MergeMill-dev-team
```
...
**Files:**
- Move: `.claude/skills/github-workflow/references/` → `skills/MergeMill-dev/references/`
...
```bash
mkdir -p skills/MergeMill-dev/references
git mv .claude/skills/github-workflow/references/commit-conventions.md skills/MergeMill-dev/references/commit-conventions.md
git mv .claude/skills/github-workflow/references/review-commands.md skills/MergeMill-dev/references/review-commands.md
...
```bash
ls skills/MergeMill-dev/references/
...
**Files:**
- Move: `openclaw/skills/MergeMill-dispatcher/SKILL.md` → `skills/MergeMill-dispatcher/SKILL.md`
- Move: `openclaw/skills/MergeMill-dispatcher/dispatch-local.sh` → `scripts/dispatch-local.sh`
- Remove: `openclaw/` directory

- [ ] **Step 1: Move dispatcher files**

```bash
mkdir -p skills/MergeMill-dispatcher
git mv openclaw/skills/MergeMill-dispatcher/SKILL.md skills/MergeMill-dispatcher/SKILL.md
git mv openclaw/skills/MergeMill-dispatcher/dispatch-local.sh scripts/dispatch-local.sh
```
...
```bash
ls skills/MergeMill-dispatcher/SKILL.md
# Expected: exists
...
Create the merged cross-platform `MergeMill-dev` SKILL.md, rewrite `MergeMill-review` SKILL.md, update dispatcher SKILL.md paths, then replace `.claude/skills/` with a symlink.

### Task 4: Create merged `skills/MergeMill-dev/SKILL.md`

**Files:**
- Create: `skills/MergeMill-dev/SKILL.md`
- Reference (read only): `.claude/skills/github-workflow/SKILL.md`, `.claude/skills/MergeMill-dev/SKILL.md`

This is the largest task — merging github-workflow (495 lines) + MergeMill-dev (209 lines) into a single cross-platform skill. The merged file follows the structure from spec Section 4.1.

- [ ] **Step 1: Read both source skills to understand full content**

Read `.claude/skills/github-workflow/SKILL.md` and `.claude/skills/MergeMill-dev/SKILL.md`.

- [ ] **Step 2: Write merged `skills/MergeMill-dev/SKILL.md`**
...
description: >
  ...
  Supports interactive and fully MergeMill modes for GitHub issue implementation.
---
```

Content sections (in order):
1. **Mode Detection** — interactive vs MergeMill (from spec Section 4.1)
...
4. **MergeMill Mode Specifics** — from MergeMill-dev SKILL.md:
...
```bash
head -5 skills/MergeMill-dev/SKILL.md
# Expected: YAML frontmatter with name: MergeMill-dev
wc -l skills/MergeMill-dev/SKILL.md
...
```bash
git add skills/MergeMill-dev/SKILL.md
git commit -m "feat: create merged cross-platform MergeMill-dev skill"
```

### Task 5: Rewrite `skills/MergeMill-review/SKILL.md`

**Files:**
- Create: `skills/MergeMill-review/SKILL.md`
- Reference (read only): `.claude/skills/MergeMill-review/SKILL.md`

- [ ] **Step 1: Read current MergeMill-review SKILL.md**

Read `.claude/skills/MergeMill-review/SKILL.md`.

- [ ] **Step 2: Write cross-platform `skills/MergeMill-review/SKILL.md`**

```yaml
---
name: MergeMill-review
description: >
  MergeMill PR code review with checklist verification, merge conflict
  resolution, E2E testing via browser automation, and auto-merge.
  Triggered by the MergeMill review wrapper script.
---
```
...
```bash
head -5 skills/MergeMill-review/SKILL.md
# Expected: YAML frontmatter with name: MergeMill-review
```

- [ ] **Step 4: Commit**

```bash
git add skills/MergeMill-review/SKILL.md
git commit -m "feat: create cross-platform MergeMill-review skill"
```

### Task 6: Update `skills/MergeMill-dispatcher/SKILL.md` paths

**Files:**
- Modify: `skills/MergeMill-dispatcher/SKILL.md`

- [ ] **Step 1: Read current dispatcher SKILL.md**

Read `skills/MergeMill-dispatcher/SKILL.md` (already moved in Task 3).
...
```bash
grep -c 'SKILL_DIR.*dispatch-local' skills/MergeMill-dispatcher/SKILL.md
# Expected: 0
grep -c 'PROJECT_DIR.*dispatch-local' skills/MergeMill-dispatcher/SKILL.md
...
```bash
git add skills/MergeMill-dispatcher/SKILL.md
...
**Files:**
- Remove: `.claude/skills/github-workflow/`, `.claude/skills/MergeMill-dev/`, `.claude/skills/MergeMill-review/`
...
ls .claude/skills/MergeMill-dev/SKILL.md
# Expected: file exists (via symlink)
ls .claude/skills/MergeMill-review/SKILL.md
# Expected: file exists
ls .claude/skills/MergeMill-dispatcher/SKILL.md
# Expected: file exists
```
...
Update hook scripts' internal paths, state-manager, settings.json, dispatch-local.sh, and MergeMill-dev.sh.
...
### Task 13: Update `scripts/MergeMill-dev.sh` and `scripts/MergeMill.conf.example`

**Files:**
- Modify: `scripts/MergeMill-dev.sh` (3 occurrences of `/github-workflow`)
- Modify: `scripts/MergeMill.conf.example:35`

- [ ] **Step 1: Update DEV_SKILL_CMD default in `MergeMill-dev.sh`**

Replace all occurrences of `/github-workflow` with `/MergeMill-dev` in `scripts/MergeMill-dev.sh`. There are 3 occurrences in the prompt templates (lines 194, 232, 280):
```bash
${DEV_SKILL_CMD:-/MergeMill-dev}
```

- [ ] **Step 2: Update `scripts/MergeMill.conf.example`**
...
```bash
grep -c 'github-workflow' scripts/MergeMill-dev.sh
# Expected: 0
grep 'DEV_SKILL_CMD' scripts/MergeMill.conf.example
# Expected: DEV_SKILL_CMD="/MergeMill-dev"
```

- [ ] **Step 4: Commit**

```bash
git add scripts/MergeMill-dev.sh scripts/MergeMill.conf.example
git commit -m "refactor: update DEV_SKILL_CMD from /github-workflow to /MergeMill-dev"
```
...
Create new docs (hooks/README.md, AGENTS.md) and update existing docs (CLAUDE.md, README.md, MergeMill-pipeline.md).
...
Install via: `npx skills add panzi-hub/MergeMill`
...
interactive and MergeMill modes.
...
schedule. Manages the MergeMill pipeline lifecycle via labels.
...
- `.claude/skills/github-workflow/` → `skills/MergeMill-dev/` (skill directory)
- `.claude/skills/MergeMill-dev/` → `skills/MergeMill-dev/` (merged)
- `.claude/skills/MergeMill-review/` → `skills/MergeMill-review/`
- `openclaw/skills/MergeMill-dispatcher/` → `skills/MergeMill-dispatcher/`
...
- `openclaw/skills/MergeMill-dispatcher/dispatch-local.sh` → `scripts/dispatch-local.sh`
...
- `.claude/skills/github-workflow/` → `skills/MergeMill-dev/`
- `.claude/skills/MergeMill-dev/` → `skills/MergeMill-dev/`
- `.claude/skills/MergeMill-review/` → `skills/MergeMill-review/`
- `openclaw/skills/MergeMill-dispatcher/` → `skills/MergeMill-dispatcher/`
- `openclaw/skills/MergeMill-dispatcher/dispatch-local.sh` → `scripts/dispatch-local.sh`
...
### Task 18: Update `docs/MergeMill-pipeline.md`

**Files:**
- Modify: `docs/MergeMill-pipeline.md`

- [ ] **Step 1: Read and update path references**

Read `docs/MergeMill-pipeline.md` and update:
- `.claude/skills/MergeMill-dev/SKILL.md` → `skills/MergeMill-dev/SKILL.md`
- `.claude/skills/MergeMill-review/SKILL.md` → `skills/MergeMill-review/SKILL.md`
- `openclaw/skills/MergeMill-dispatcher/SKILL.md` → `skills/MergeMill-dispatcher/SKILL.md`
- `openclaw/skills/MergeMill-dispatcher/dispatch-local.sh` → `scripts/dispatch-local.sh`
...
```bash
grep -c 'openclaw/' docs/MergeMill-pipeline.md
# Expected: 0
grep -c '\.claude/skills/' docs/MergeMill-pipeline.md
# Expected: 0
```
...
```bash
git add docs/MergeMill-pipeline.md
git commit -m "docs: update MergeMill-pipeline.md paths for new structure"
```
...
Expected: Lists 3 skills: `MergeMill-dev`, `MergeMill-review`, `MergeMill-dispatcher`
...
ls .claude/skills/MergeMill-dev/SKILL.md .claude/skills/MergeMill-review/SKILL.md .claude/skills/MergeMill-dispatcher/SKILL.md
...
# Expected: prints the repo root path (e.g., /data/git/MergeMill-dev-team)
```
