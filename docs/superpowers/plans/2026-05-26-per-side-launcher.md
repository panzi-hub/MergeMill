...
|---|---|---|
| `skills/MergeMill-dispatcher/scripts/lib-agent.sh` | Modify | Add `AGENT_DEV_LAUNCHER` / `AGENT_REVIEW_LAUNCHER` init + per-side argv tokenization. Replace single `[INV-37]` guard with two per-side guards. |
| `skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh` | Modify | Add `AGENT_LAUNCHER_ARGV=("${AGENT_DEV_LAUNCHER_ARGV[@]}")` immediately after the existing `AGENT_CMD="$AGENT_DEV_CMD"` line. |
| `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` | Modify | Symmetric: rebind to `AGENT_REVIEW_LAUNCHER_ARGV[@]`. |
| `skills/MergeMill-dispatcher/scripts/MergeMill.conf.example` | Modify | Add `AGENT_DEV_LAUNCHER` / `AGENT_REVIEW_LAUNCHER` operator-facing comment block above the per-CLI blocks. |
...
Active worktree for this work is `worktree-feat+per-side-launcher` at `/data/git/MergeMill-dev-team/.claude/worktrees/feat+per-side-launcher`. All commits below assume the executor is in that worktree.
...
**Files:**
- Modify: `skills/MergeMill-dispatcher/scripts/lib-agent.sh` (insert after the existing `AGENT_LAUNCHER` block ~line 106; replace the single guard at lines 108-115)
...
Run:
```bash
sed -n '73,115p' skills/MergeMill-dispatcher/scripts/lib-agent.sh
```
...
#     of `source lib-agent.sh` in MergeMill-{dev,review}.sh
#
...
# MergeMill-dev.sh and MergeMill-review.sh each rebind
...
Run:
```bash
shellcheck -S error skills/MergeMill-dispatcher/scripts/lib-agent.sh tests/unit/test-lib-agent-per-side-launcher.sh
```
...
Run:
```bash
git add skills/MergeMill-dispatcher/scripts/lib-agent.sh \
...
## Task 2: Add `AGENT_LAUNCHER_ARGV` rebind to `MergeMill-dev.sh` + PSL-S9

**Files:**
- Modify: `skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh` (insert one line after the existing `AGENT_CMD="$AGENT_DEV_CMD"` from PR #156 at line ~26)
...
# ---------------------------------------------------------------------------
echo ""
echo "=== PSL-S9: MergeMill-dev.sh structural placement ==="
# ---------------------------------------------------------------------------
...
assert_eq "MergeMill-dev.sh: AGENT_LAUNCHER_ARGV=(\${AGENT_DEV_LAUNCHER_ARGV[@]}) within 5 lines of source lib-agent.sh" \
  "MATCH" "$hit"
```
...
- [ ] **Step 2.3: Read current MergeMill-dev.sh lines 21-28**

Run:
```bash
sed -n '21,28p' skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh
```
...
Run:
```bash
git add skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh tests/unit/test-lib-agent-per-side-launcher.sh
git commit -m "feat(MergeMill-dev): rebind AGENT_LAUNCHER_ARGV to dev-side after lib-agent source
...
## Task 3: Add `AGENT_LAUNCHER_ARGV` rebind to `MergeMill-review.sh` + PSL-S10

**Files:**
- Modify: `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` (insert one line after the existing `AGENT_CMD="$AGENT_REVIEW_CMD"` line)
...
# ---------------------------------------------------------------------------
echo ""
echo "=== PSL-S10: MergeMill-review.sh structural placement ==="
# ---------------------------------------------------------------------------
...
assert_eq "MergeMill-review.sh: AGENT_LAUNCHER_ARGV=(\${AGENT_REVIEW_LAUNCHER_ARGV[@]}) within 6 lines of source lib-agent.sh" \
  "MATCH" "$hit"
```
...
- [ ] **Step 3.3: Read current MergeMill-review.sh lines 21-32**

Run:
```bash
sed -n '21,32p' skills/MergeMill-dispatcher/scripts/MergeMill-review.sh
```
...
Run:
```bash
git add skills/MergeMill-dispatcher/scripts/MergeMill-review.sh tests/unit/test-lib-agent-per-side-launcher.sh
git commit -m "feat(MergeMill-review): rebind AGENT_LAUNCHER_ARGV to review-side after lib-agent source
...
## Task 4: Document `AGENT_DEV_LAUNCHER` / `AGENT_REVIEW_LAUNCHER` in `MergeMill.conf.example`

**Files:**
- Modify: `skills/MergeMill-dispatcher/scripts/MergeMill.conf.example` (insert ~25 lines after the existing AGENT_LAUNCHER block, before the AGENT_DEV_EXTRA_ARGS line)

- [ ] **Step 4.1: Locate the insertion point**

Run:
```bash
grep -nE "^AGENT_LAUNCHER=|^AGENT_DEV_EXTRA_ARGS=" skills/MergeMill-dispatcher/scripts/MergeMill.conf.example
```
...
Run:
```bash
grep -nE "^AGENT_DEV_EXTRA_ARGS=" skills/MergeMill-dispatcher/scripts/MergeMill.conf.example
```
...
**Consumer**: `MergeMill-dev.sh` and `MergeMill-review.sh` entry blocks rebind `AGENT_LAUNCHER_ARGV` to `AGENT_{DEV,REVIEW}_LAUNCHER_ARGV` immediately after sourcing `lib-agent.sh`. Downstream `_run_with_timeout` reads the rebound `AGENT_LAUNCHER_ARGV[@]` unchanged.
...
- [INV-31](#inv-31-operator-tunable-per-cli-flags-live-in-conf-not-in-lib-agentsh) — operator-tunable flags live in `MergeMill.conf`. The new vars follow that contract.
...
- `skills/MergeMill-dispatcher/scripts/lib-agent.sh` — init + tokenization for `AGENT_DEV_LAUNCHER` / `AGENT_REVIEW_LAUNCHER`; replace single guard with two per-side guards
- `skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh` — 1-line `AGENT_LAUNCHER_ARGV` rebind after `AGENT_CMD` rebind
- `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` — symmetric rebind for review side
...
- [`docs/pipeline/per-side-launcher.md`](https://github.com/panzi-hub/MergeMill/blob/feat/per-side-launcher/docs/pipeline/per-side-launcher.md)
- INV-38: per-side AGENT_LAUNCHER precedence
