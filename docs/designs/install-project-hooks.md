...
(`MergeMill.conf`, deploy helpers, validators), so directory replacement
isn't viable. Consumers fall back to per-file symlinks, e.g.:

```bash
for f in lib-agent.sh lib-auth.sh lib-config.sh MergeMill-{dev,review}.sh; do
  ln -s "$HOME/.claude/skills/MergeMill-dispatcher/scripts/$f" "scripts/$f"
done
```

When upstream adds a new file (e.g. `lib-review-verdict.sh`), per-file lists
**don't auto-sync**. `MergeMill-review.sh` `source`s the missing file from
the project-side `scripts/` (because `BASH_SOURCE[0]` resolves through the
project symlink, per `[INV-14]` in `MergeMill-review.sh:18-20`) and dies on
...
Ship `install-project-hooks.sh` inside `MergeMill-common` (already a hard
...
1. **Symlink dispatcher scripts.** For each `*.sh` in
   `<skills-root>/MergeMill-dispatcher/scripts/`, create a symlink
...
2. **Symlink the hooks dir.** `<project>/hooks` →
   `<skills-root>/MergeMill-common/hooks`. Existing real `hooks/`
...
- It does not write `.claude/settings.json` or any other IDE-specific
  ...
  `--no-git-hook` flag.
- It does not symlink files from `MergeMill-common/scripts/` itself.
  Those are agent-callable utilities (`mark-issue-checkbox.sh`, …) that
  consumers reach via `.agents/skills/MergeMill-common/scripts/...`
...
The script needs to find the on-disk MergeMill-dispatcher skill root.
Probe order (matches `lib-installer.sh::ensure_dispatcher_scripts_executable`):

1. `<project-root>/.agents/skills/MergeMill-dispatcher/scripts`
2. `<project-root>/.claude/skills/MergeMill-dispatcher/scripts`
3. `<project-root>/skills/MergeMill-dispatcher/scripts`

If none found, abort with a clear error pointing to `npx skills add`.

### Where the script lives

`skills/MergeMill-common/scripts/install-project-hooks.sh`. Materialized
into the consumer at `.agents/skills/MergeMill-common/scripts/...` via
`npx skills add`.
...
bash .agents/skills/MergeMill-common/scripts/install-claude-hooks.sh   # IDE-specific
```
...
- `[INV-14]` — `BASH_SOURCE[0]` is intentionally NOT `readlink -f`'d in
  `MergeMill-review.sh:18-20`, so a project-side symlink resolves
