...
**Issue**: #126
**Status**: Proposed
**Author**: zxkane (MergeMill-dev session)
...
`MergeMill-dev.sh` and `MergeMill-review.sh` are launched with the same
...
MergeMill projects in parallel (the operator's actual topology — see
...
`MergeMill-dev.sh` script-name is **not** unique across projects — every
...
- **No new argv** on the wrapper scripts (additive surface invites compat
  ...
  argv via `nohup "<path>/MergeMill-dev.sh" ...`.
...
1. **`PROJECT_DIR`** — the project's root directory, sourced from
   `MergeMill.conf`. Every wrapper invocation in `dispatch-local.sh:199-220`
   runs `nohup "${PROJECT_DIR}/scripts/MergeMill-(dev|review).sh" ...`, so
   `pgrep -f` sees the absolute path on argv0.
2. **`TYPE`** — selects which of the two wrapper scripts (`MergeMill-dev.sh`
   or `MergeMill-review.sh`) is the legal kill target.
...
- The wrappers use disjoint script names (`MergeMill-dev.sh` vs
  `MergeMill-review.sh`); combined with the project path anchor, the match
...
`PROJECT_DIR` is operator-supplied via `MergeMill.conf`. Most paths are
...
(`MergeMill-(dev|review)\.sh`) so we don't silently let escaped wrappers
...
`MergeMill-(dev|review).sh` AND the path to be under
`${PROJECT_DIR}/scripts/`.
...
- **TC-A**: `review` wrapper alive (PID file present + responsive),
  ...
  clean.
- **TC-B**: Mirror — `MergeMill-dev.sh` alive, dispatch `review`. Expect:
  dev wrapper untouched, review wrapper spawns.
- **TC-C**: Same-type orphan — real escaped `MergeMill-dev.sh --issue N`
...
- **TC-PGREP-001..004**: spawn fixture `sleep 60` processes whose `argv0`
  ...
  `MergeMill-review.sh --issue N`, decoy must remain alive.
...
| AC | Verification |
|---|---|
| `kill_stale_wrapper` from `dev-*` never SIGTERMs `MergeMill-review.sh`, and vice versa, even with same issue N | TC-PGREP-001/002 |
...
- Any change to `MergeMill-dev.sh` or `MergeMill-review.sh` argv.
