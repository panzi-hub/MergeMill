...
In scope:
- New `dispatcher.conf.example` declaring a `PROJECTS=()` array of paths to per-project `MergeMill.conf` files.
...
```bash
# dispatcher.conf
# Lookup priority: $DISPATCHER_CONF env, $HOME/.MergeMill/dispatcher.conf,
# $XDG_CONFIG_HOME/MergeMill/dispatcher.conf.

# Required: list of per-project MergeMill.conf paths.
# Each path must point to a project's existing MergeMill.conf; the dispatcher
# sources it (in a subshell) before running one tick.
PROJECTS=(
  "/data/git/myrepo-a/scripts/MergeMill.conf"
  "/data/git/myrepo-b/scripts/MergeMill.conf"
)
```

Per-project values (REPO, PROJECT_ID, MAX_CONCURRENT, DISPATCHER_APP_ID, DISPATCHER_APP_PEM, ...) all stay in each project's `MergeMill.conf`. The outer loop just iterates.
...
dispatcher-multi-tick.sh tick:
  ...
  for each conf_path in PROJECTS:
    in a subshell with MERGEMILL_CONF=$conf_path:
...
Key isolation property: each project gets its own subshell. Variables set by one project's `MergeMill.conf` (REPO, PROJECT_ID, MAX_CONCURRENT, etc.) cannot leak into the next project's tick. This matches the `lib-config.sh::load_MergeMill_conf` priority-1 path (`MERGEMILL_CONF` env override) introduced in PR-4 — no new mechanism needed.
...
The wrappers (`MergeMill-dev.sh`, `MergeMill-review.sh`) already read `DISPATCHER_APP_ID` / `DISPATCHER_APP_PEM` from each project's own `MergeMill.conf` via `lib-auth.sh::setup_github_auth`. This already works — the multi-project wrapper inherits it for free because each project's `MergeMill.conf` is sourced fresh in the subshell.

The cron prompt in SKILL.md updates from "set REPO, source MergeMill.conf, run dispatcher-tick.sh" to just "run dispatcher-multi-tick.sh" — the per-project auth happens inside each iteration.
...
for conf in "${PROJECTS[@]}"; do
  if ! ( MERGEMILL_CONF="$conf" bash "$SCRIPT_DIR/dispatcher-tick.sh" ); then
...
2. Each iteration runs `dispatcher-tick.sh` with `MERGEMILL_CONF` set to the right path (verified via a stub `dispatcher-tick.sh` that records its env).
...
These are the same trust checks `sudo` and `ssh` apply to their config files. `/tmp` (mode 1777) is rejected by the parent-dir check, so operators are nudged toward `$HOME/.MergeMill/dispatcher.conf` (mode 0700 home dir).

Escape hatch: `MERGEMILL_TRUST_CONF=1` disables the gate. Documented in `dispatcher.conf.example` for shared-VM cases where the conf is owned by a different uid by design.

The gate applies only to `dispatcher.conf` in this PR — per-project `MergeMill.conf` files are sourced inside the subshell via `lib-config.sh::load_MergeMill_conf` (existing pattern, not modified here). A follow-up PR could extend the trust gate to that path; out of scope for this change.
...
- **Not** changing how individual wrappers read auth — they already pull from per-project `MergeMill.conf`.
