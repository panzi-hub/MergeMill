...
(`mergeable == CONFLICTING`) can still be **approved** by the MergeMill-review
...
In `MergeMill-review.sh`, immediately **after** the `case "$AGGREGATE"` block
...
- **CONFLICTING (`block-substantive`)**: dev genuinely must rebase.
  ...
  - Post an `Auto-merge failed:`-prefixed marker on the **PR**. This reuses the
    existing dev-resume hook (`MergeMill-dev.sh` greps PR comments for a body
...
- Both: `−reviewing +pending-dev` (keep `MergeMill`), `RESULT_PARSED=true`,
  `exit 0`.

### Why `pending-dev` (not a new label)

`pending-dev` + `MergeMill` is the state the dispatcher's `list_pending_dev`
...
- `skills/MergeMill-review/references/decision-gate.md` — agent-side
  reinforcement (mergeable is a blocking finding).
- `skills/MergeMill-review/references/merge-conflict-resolution.md` — the
