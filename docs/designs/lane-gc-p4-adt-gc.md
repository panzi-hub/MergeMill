...
1. **`skills/MergeMill-dispatcher/scripts/adt-gc.sh`** (new entry-point
...
2. **`skills/MergeMill-dispatcher/scripts/install-gc-timer.sh`** (new
...
degree of freedom — see "Decision Making Guidelines" in the MergeMill-dev
...
- **Rule 2.4's "ancestry of any live wrapper" is scoped box-wide, not
  ...
  project's registry (`$ADT_STATE_ROOT/MergeMill-*/lanes/`); it has no
  central manifest of every project's `PROJECT_DIR` to scope the protective
  `pgrep -f 'MergeMill-(dev|review)\.sh'` ancestry walk to. Walking every
