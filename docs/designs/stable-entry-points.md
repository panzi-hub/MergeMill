...
1. **conf lookup** — `load_MergeMill_conf "$SCRIPT_DIR"` must resolve from the
   project's `scripts/` (where `MergeMill.conf` lives). The unresolved path is
   load-bearing here.
...
|---|---|---|
| `CONF_DIR` | `dirname` of the **unresolved** `${BASH_SOURCE[0]:-$0}` | `load_MergeMill_conf "$CONF_DIR"` (preserves [INV-14] conf lookup from the project's `scripts/`) AND the legacy `${CONF_DIR}/../../../scripts/MergeMill.conf` fallback |
...
at the project's `scripts/` (where `MergeMill.conf` lives). Result: **lib
...
- `_LIB_*_CONF_DIR` (unresolved) — passed to `load_MergeMill_conf`.
...
|---|---|---|
| `MergeMill-dev.sh` | (via lib-agent/lib-auth) | `lib-agent.sh`, `lib-auth.sh` |
| `MergeMill-review.sh` | (via lib-agent/lib-auth) | `lib-agent.sh`, `lib-auth.sh`, `lib-review-*.sh` (12) |
| `dispatch-local.sh` | `MergeMill.conf` + depth-3 fallback | `lib-config.sh` |
| `dispatcher-tick.sh` | `load_MergeMill_conf` | `lib-config.sh`, `lib-dispatch.sh`, `lib-review-bots.sh`, `gh-app-token.sh` |
| `setup-labels.sh` | `MergeMill.conf` + depth-3 fallback | (none) |
...
| `lib-agent.sh` | `load_MergeMill_conf` (CONF) | `lib-config.sh` (LIB) |
| `lib-auth.sh` | `load_MergeMill_conf` (CONF) | `lib-config.sh`, `gh-app-token.sh` (LIB) |
| `lib-dispatch.sh` | (none) | spawns `liveness-check-remote-aws-ssm.sh` via real-path `${_src%/*}` |
| MergeMill-common hooks | n/a (no conf) | whole-dir symlink → `dirname` already lands in real dir; unchanged |
...
**Note on MergeMill-common hooks**: `<project>/hooks` is a whole-directory
symlink into `MergeMill-common/hooks/`, so `dirname "${BASH_SOURCE[0]}"`
already resolves to the real hooks dir (the symlink is on the dir, not each
file). They don't read `MergeMill.conf`. No change needed.
...
1. **Symlink only the stable manifest** going forward — entry points + agent-
   ...
   ```
   MergeMill-dev.sh MergeMill-review.sh dispatch-local.sh dispatcher-tick.sh
