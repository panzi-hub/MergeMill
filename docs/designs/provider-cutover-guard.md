...
GitHub-rendered close". `MergeMill-review.sh` alone carries 15 raw `gh issue
...
- **Check 1 — no NEW raw-gh in the caller layer.** Scan the caller-layer files
  (`lib-dispatch.sh`, `MergeMill-dev.sh`, `MergeMill-review.sh`, every
...
- **Check 2 — provider files exist + are the only NON-baselined gh holders.**
  ...
  raw-gh site discovered anywhere under `skills/MergeMill-dispatcher/scripts/`
...
| `native_issue_pr_link` | MergeMill-dev.sh |
| `rest_request_changes` | lib-review-request-changes.sh |
| `review_bots` | lib-auth.sh, MergeMill-review.sh |
| `merge_closes_issue` | MergeMill-review.sh, MergeMill-dev.sh |
...
`ISSUE_PROVIDER=degraded` + `MERGEMILL_PROVIDERS_DIR` (per #280 TC-030/031).
...
`ci.yml` — run `bash skills/MergeMill-dispatcher/scripts/check-provider-cutover.sh`
