...
When `MergeMill-dev.sh` is invoked via `bash -c '...'` (SSM dispatch, subprocess
...
directory, failing to find `MergeMill.conf`.

## Fix

### 1. Add `MERGEMILL_CONF` env var as highest-priority config source

Callers can set `MERGEMILL_CONF=/path/to/MergeMill.conf` before invoking the
...
1. `MERGEMILL_CONF` env var (explicit path)
2. `${_LIB_DIR}/MergeMill.conf` (local to script)
3. `${_LIB_DIR}/../../../scripts/MergeMill.conf` (project root fallback)
