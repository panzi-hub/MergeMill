...
In `MergeMill-review.sh`'s INV-62 stdout-fallback block (already gated on rc 0): if the classifier returns
...
| File | Change |
|---|---|
| `skills/MergeMill-dispatcher/scripts/lib-review-codex.sh` | New `_codex_review_stdout_is_malformed` + signal sub-helpers; `_codex_review_classify_stdout` gains the `malformed` token (checked first); `_run_codex_review` re-runs a malformed rc-0 capture (bounded); `_classify_codex_drop_reason` gains the `malformed-output` bucket; `_codex_drop_reason_phrase` renders it. |
| `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` | The INV-62 stdout-fallback block treats `malformed` as no-verdict (no body composed, no post → unresolved → `unavailable`). |
...
| `tests/e2e/*` | Extend the stub-fleet test driving `MergeMill-review.sh` with a stubbed codex whose stdout is the prompt-echo shape; assert no phantom FAIL, the rendered drop reason names `malformed-output`, and the aggregate is decided by the surviving agents. |
...
This PR touches the review wrapper (`MergeMill-review.sh`) and its codex review lib (`lib-review-codex.sh`),
...
This PR EDITS existing files (`lib-review-codex.sh`, `MergeMill-review.sh`) — it does NOT add or remove a
