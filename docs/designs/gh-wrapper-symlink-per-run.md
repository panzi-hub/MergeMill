...
|---|---|---|---|
| **The wrapper itself** (`MergeMill-dev.sh`, `MergeMill-review.sh`) — bare `gh issue edit`, `gh pr comment`, … | bare `gh` | **`PATH`** lookup (`setup` prepends `${_LIB_AUTH_DIR}`) | *some* dir on `PATH` containing a `gh` → `gh-with-token-refresh.sh` |
| **The agent** — `bash scripts/gh issue comment …` | literal **relative path** `scripts/gh` | filesystem path relative to `cwd` (`MergeMill-dev.sh` does `cd "$PROJECT_DIR"`, so this is `${PROJECT_DIR}/scripts/gh`) | the physical file `${PROJECT_DIR}/scripts/gh` to exist |
...
`skills/MergeMill-dev/SKILL.md` Step 12 and
`skills/MergeMill-dev/references/MergeMill-mode.md`. **Removing
...
- `${PROJECT_DIR}/scripts/gh` is in `.gitignore` already (the `scripts/gh` and
  `skills/MergeMill-dispatcher/scripts/gh` entries), so leaving it on disk between
