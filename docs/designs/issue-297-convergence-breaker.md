...
`MergeMill-review.sh`'s own separate process and is NEVER set in the
...
re-add `MergeMill`.** `MergeMill` is never removed by this pipeline except on
...
sitting in `stalled` still carries `MergeMill`; the maintainer's actual re-arm
action (`stalled-rearm` in `transitions.json`) is to **remove the `stalled`
label**, which re-enters via dispatcher Step 2 (`MergeMill → in_progress`) and
resets the retry counter (INV-05). "Re-add `MergeMill`" is a no-op on this
...
1. Post ONE structured `reason=non-convergence` report carrying:
   ...
   - a **"To resume: fix per the checklist, then REMOVE the `stalled` label
     (`MergeMill` is retained; removal re-arms the pipeline via Step 2 and
...
2. Then the terminal transition via plain `label_swap`: `pending-dev → stalled`
   (the SAME declared movement `mark_stalled` uses — `MergeMill` is retained
...
removes `MergeMill` (expensive on an unattended pipeline); a missed trip is
...
- **CB-TRIP-001** — 3 frozen-head completed zero-commit rounds,
  ...
  `reason=non-convergence` report + marker, `stalled` added, `MergeMill` +
