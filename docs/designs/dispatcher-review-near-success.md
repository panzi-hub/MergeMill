...
Both wrappers (`MergeMill-dev.sh`, `MergeMill-review.sh`) install a
...
| File | Change |
|------|--------|
| `skills/MergeMill-dispatcher/scripts/lib-agent.sh` | + `install_agent_heartbeat` helper |
| `skills/MergeMill-dispatcher/scripts/lib-dispatch.sh` | + `pid_alive` mtime fallback, + `review_near_success` helper |
| `skills/MergeMill-dispatcher/scripts/dispatcher-tick.sh` | Step 5b review branch gates on `review_near_success` |
| `skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh` | call `install_agent_heartbeat` after PID-file write |
| `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` | call `install_agent_heartbeat` after PID-file write |
| `scripts/MergeMill.conf.example` | document `HEARTBEAT_INTERVAL_SECONDS`, `REVIEW_NEAR_SUCCESS_WINDOW_SECONDS` |
