...
(`MergeMill-review.sh:1835`, when codex produced review text but did not
self-post). There the wrapper already has the helper's rc directly — and on a
failed post its `else` branch (`MergeMill-review.sh:1859-1861`) logs a WARNING
...
- `post-verdict.sh` already sources `MergeMill.conf` (which sets `PROJECT_ID`);
...
### Wrapper wiring (`MergeMill-review.sh`)
...
- `skills/MergeMill-dispatcher/scripts/post-verdict.sh` — source `lib-config.sh`; write the session-keyed breadcrumb on a failed `gh` post (still exit 1)
- `skills/MergeMill-dispatcher/scripts/lib-review-postfail.sh` — NEW (path helper + classifier + phrase helper)
- `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` — source the lib; attach the post-failed reason (CLI-agnostic, ahead of the per-CLI scrapers)
