...
| Layer | Owns |
|---|---|
| Caller (broker in `lib-auth.sh`; wrapper in `MergeMill-review.sh`) | Positionals; pre-create existence check; `set +e` capture of `MERGE_OUT/MERGE_RC`; `${MERGE_OUT:0:500}` excerpt; metrics; `itp_transition_state` post-merge for `merge_closes_issue=0`; the manual-merge fallback on approve fail. |
