...
|---|---|---|
| #64 | `block-push-to-main.sh` regex false-pos / false-neg | existing `skills/MergeMill-common/hooks/block-push-to-main.sh` |
| #65 | Worktree creation should install repo-local git pre-push backstop | new `skills/MergeMill-common/hooks/install-git-pre-push.sh` |
| #68 | SKILL.md frontmatter hooks are skill-scoped, not project-scoped | new `skills/MergeMill-common/scripts/install-claude-hooks.sh` + docs |
...
**Key insight from #68**: the SKILL.md frontmatter hooks are bound to the skill activation context. As soon as the agent does any free-form bash work outside `/MergeMill-dev`, the frontmatter hooks don't fire. They are *not* automatically promoted to project-scoped `.claude/settings.json` entries.
...
### New `skills/MergeMill-common/hooks/lib-push.sh`
...
- Worktree-bootstrap callsites: this PR doesn't change those (they're in agent prompts / the MergeMill-dev SKILL.md skill). Documentation update only — the MergeMill-dev skill should call the installer right after `git worktree add`.

### #68: new `install-claude-hooks.sh` bootstrap

A one-shot installer for downstream consumers that materializes a project-scoped `.claude/settings.json` mirroring the MergeMill-dev SKILL.md frontmatter. Idempotent — re-running merges (doesn't clobber) hand-edits.

- Reads the canonical hook list from a curated source-of-truth (the MergeMill-common skill itself, not the MergeMill-dev SKILL.md frontmatter — we don't want the consumer's settings drift to be tied to whether they parse YAML).
- Writes / merges into `.claude/settings.json` with a `"managed_by": "MergeMill-common"` annotation so future `skills update` runs can re-merge cleanly.
- Calls `install-git-pre-push.sh` at the end (one-stop install).
- Documented in `MergeMill-common/SKILL.md`: "after `npx skills add`, run `bash .claude/skills/MergeMill-common/scripts/install-claude-hooks.sh`".
...
- `tests/unit/test-install-claude-hooks.sh` — covers:
  ...
  - Annotation present: `"managed_by": "MergeMill-common"` field is written
...
Touches `skills/MergeMill-common/hooks/*.sh` (watched). Also touches `docs/pipeline/invariants.md` (new INV-17). Gate passes via docs-touched.
...
- The MergeMill-dev workflow integration (calling `install-git-pre-push.sh` after `git worktree add`) is a SKILL.md prompt update, not a hook code change. PR-5 documents the call site but doesn't change the prompt body — that's a follow-up.
