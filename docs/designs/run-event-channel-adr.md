...
>
> MergeMill design (no interactive approval gate). Docs-only PR.
...
measure-event-channels.sh --repo panzi-hub/MergeMill --pr <N> --samples 5 --comment-only
# check-run latency + permission probe — produced §6.3:
measure-event-channels.sh --repo panzi-hub/MergeMill --sha <commit> --check-only
