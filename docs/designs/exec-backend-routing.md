...
│                                                                          │
│  $HOME/.MergeMill/dispatcher.conf  declares all projects:               │
│    - some are LOCAL  (MergeMill.conf path on disk here)                 │
│    - some are REMOTE (inline metadata + SSM_INSTANCE_ID)                 │
│                                                                          │
│  cron tick → dispatcher-multi-tick.sh                                    │
│    │                                                                     │
│    ├─→ tick(local-project)   sources MergeMill.conf, calls              │
...
Critical fact: **the dispatcher box does NOT have any per-project `MergeMill.conf` for remote projects.** It only knows what `dispatcher.conf` tells it (REPO, SSM_INSTANCE_ID, etc). The remote dev box has its own `MergeMill.conf` that's used by the wrapper after SSM kicks off `dispatch-local.sh`.

This is why PR-8's "every project has an MergeMill.conf path" pattern doesn't fit — that pattern assumes co-location of dispatcher and project source. Real deployment is mixed.
...
Everything else (`PROJECT_DIR`, `AGENT_CMD`, `AGENT_TIMEOUT`, etc.) is consumed by the wrapper, which reads its OWN local `MergeMill.conf` after SSM puts it on the right box.
...
# Format: a string ending in "MergeMill.conf" — multi-tick treats as a
# file path and sources it. The MergeMill.conf provides REPO etc.
PROJECTS+=( "/data/git/myrepo-local/scripts/MergeMill.conf" )
...
`lib-dispatch.sh` requires `REPO`, `REPO_OWNER`, and `REPO_NAME` separately (App-token scoping reads owner+name independently). In the local-project case, the operator's own `MergeMill.conf` declares all three (matches PR-8 / `MergeMill.conf.example`).
...
After `load_MergeMill_conf` returns (or after the multi-tick wrapper has sourced/eval'd the project's metadata into the subshell env), `dispatcher-tick.sh` defines a thin `dispatch()` helper:
...
1. **Iteration shape**: each `PROJECTS[i]` is now either a path or an inline bash block. The loop body decides per element:
   ...
   for entry in "${PROJECTS[@]}"; do
     if [[ "$entry" == *"/"* && -f "$entry" ]]; then
       # local project: PR-8 behavior, set MERGEMILL_CONF env override
       ( MERGEMILL_CONF="$entry" bash "$SCRIPT_DIR/dispatcher-tick.sh" )
     else
...
Hard architectural fact: `lib-dispatch.sh::pid_alive` does `kill -0 $(cat $PID_FILE)` against `${XDG_RUNTIME_DIR}/MergeMill-${PROJECT_ID}/issue-N.pid` (PR-7 path). For a remote project, the PID file is on the remote dev box, not on the dispatcher box. The dispatcher's `kill -0` always fails → the dispatcher always sees DEAD.
...
- PR-8's local-project entry shape (path string ending in `MergeMill.conf`) keeps working in `dispatcher-multi-tick.sh`.
...
New:
- `skills/MergeMill-dispatcher/scripts/dispatch-remote-aws-ssm.sh`
- `tests/unit/test-dispatch-remote-aws-ssm.sh`
- `tests/unit/test-dispatcher-tick-router.sh`
- `tests/unit/test-multi-tick-inline-projects.sh`
...
Modified:
- `skills/MergeMill-dispatcher/scripts/dispatcher-tick.sh` (3 callsites → `dispatch` helper)
- `skills/MergeMill-dispatcher/scripts/dispatcher-multi-tick.sh` (inline-block detection + eval path)
- `skills/MergeMill-dispatcher/scripts/dispatcher.conf.example` (mixed local + remote example)
- `skills/MergeMill-dispatcher/SKILL.md` (backend selection + remote topology note)
