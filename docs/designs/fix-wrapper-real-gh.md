...
before the PATH-stripping logic. `MergeMill.conf` is already sourced by
`dispatch-local.sh` and `MergeMill-dev.sh`, so a single conf line is
enough:
...
MergeMill.conf to override."

### Part 2 — startup-failure session report in `MergeMill-dev.sh`
...
- `skills/MergeMill-dispatcher/scripts/gh-with-token-refresh.sh` —
  REAL_GH override + improved error message.
- `skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh` — cleanup
  emits failure session report when AGENT_RAN=false but ISSUE_NUMBER
  is set; transitions labels.
- `skills/MergeMill-dispatcher/scripts/MergeMill.conf.example` —
  document `REAL_GH` with a commented-out example.
- `tests/unit/test-gh-with-token-refresh-real-gh.sh` — new.
- `tests/unit/test-MergeMill-dev-cleanup-startup-failure.sh` — new.

### Files deliberately not changed

- `MergeMill-review.sh` — the review wrapper has the same general
...
- `MergeMill-dev.sh` cleanup posts an `Agent Session Report (Dev)
