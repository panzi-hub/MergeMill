...
- model: `MergeMill-review.sh` calls `run_agent "$_agent_session_id" "$_agent_prompt" "${AGENT_REVIEW_MODEL:-sonnet}" ...` inside the per-agent subshell — every agent gets `${AGENT_REVIEW_MODEL:-sonnet}`.
...
(`MergeMill.conf.example`: "All listed agents currently share
...
calls `resume_agent`. (`MergeMill.conf.example`, kiro block: "kiro's
...
/ `lib-review-verdict.sh`): sourced by `MergeMill-review.sh`, unit-tested in
...
but that lib change has a second caller (`MergeMill-dev.sh` dev-resume) and
