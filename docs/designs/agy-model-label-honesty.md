...
1. `lib-review-resolve.sh::_review_fanout_model_label` — the `Fanning out …` log
   line (INV-58).
2. `MergeMill-review.sh::build_review_prompt` → `_agent_model` — the 6th
   `post-verdict.sh` arg folded into the verdict comment's `Review Agent: <name>
   (model: <model>)` line (INV-60).
3. `MergeMill-review.sh` `_REVIEW_HEAD_MODEL` — the `Reviewed HEAD: … model …`
...
| `MergeMill-review.sh::build_review_prompt` | `_agent_model` for the verdict trailer = `_resolve_review_agent_model_label "$_agent_name"`. |
| `MergeMill-review.sh` `_REVIEW_HEAD_MODEL` | = `_resolve_review_agent_model_label "$_REVIEW_HEAD_AGENT"`. |
