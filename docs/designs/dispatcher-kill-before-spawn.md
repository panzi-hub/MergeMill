...
`dispatch-local.sh` spawns a wrapper via `nohup` without checking whether a previous wrapper for the same issue is still alive. When a stale wrapper lingers (e.g. its inner agent CLI died but the outer `bash MergeMill-dev.sh` is still running), the new spawn happens anyway. Two wrappers now coexist for one issue.
...
If we changed the guard too (defense in depth), the kill-before-spawn would mask any guard logic the wrapper relies on for direct invocations (someone running `bash MergeMill-dev.sh` outside the dispatcher). Better to keep responsibilities separated.
