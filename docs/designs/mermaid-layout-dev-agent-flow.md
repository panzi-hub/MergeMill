...
1. **`<br/>` inside `sequenceDiagram` message text.** The Lifecycle block has one such case:
   ```
   D->>W: nohup MergeMill-dev.sh --issue N --mode new<br/>(or --mode resume --session ID)
   ```
...
2. View `https://github.com/panzi-hub/MergeMill/blob/<sha>/docs/pipeline/dev-agent-flow.md` via chrome-devtools MCP.
