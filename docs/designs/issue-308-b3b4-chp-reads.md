...
- `chp_pr_list` = `gh pr list --repo "$REPO" "$@"` (precedent: `MergeMill-dev.sh:388/767/1053`).
- `chp_pr_view` = `local pr="$1"; shift; gh pr view "$pr" --repo "$REPO" "$@"` (precedent: `MergeMill-review.sh:922/952/953`).
...
- The B3 sites pass a LOCAL `$repo` param; the verb supplies the GLOBAL `$REPO`.
  ...
  (`MergeMill-dev.sh:762/782`, `MergeMill-review.sh:775`). Dropping
...
- **B4 (`lib-review-e2e.sh`)**: in the review wrapper, `MergeMill-review.sh`
...
- The two existing isolation fixtures (`test-token-split-234.sh`,
  `test-MergeMill-review-sequential-e2e.sh`) updated to source the CHP seam and
