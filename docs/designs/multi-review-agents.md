...
Today `MergeMill-review.sh` runs exactly ONE review agent (`AGENT_REVIEW_CMD`,
...
the merge on their agreement would raise confidence before an MergeMill
...
1. **Unanimous PASS** — aggregated verdict is PASS iff **every available
   ...
   finding → FAIL".
2. **Single wrapper fans out internally** — `MergeMill-review.sh` spawns all
...
```
                        ┌──────────────────────────────────────────┐
                        │ MergeMill-review.sh (single wrapper)      │
