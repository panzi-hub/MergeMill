...
1. **The gate itself, `skills/MergeMill-dispatcher/scripts/dispatch-local.sh`**:
...
2. **`lib-lane.sh::lane_global_live_count`** (new function, this PR): the
   ...
   `MergeMill-*/lanes/`, joins on `lane_probe`'s own `live` verdict);
...
6. **Knobs**: `GATE_LOAD_PER_CORE`, `GATE_MIN_MEM_MB`, `GATE_SWAP_PCT`,
   `MAX_TOTAL_CONCURRENT` documented in `MergeMill.conf.example`;
