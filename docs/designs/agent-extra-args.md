...
versions MUST add these vars to their `MergeMill.conf` for `gemini` and `kiro` deployments
(documented in `MergeMill.conf.example` migration callout).
...
- docs/MergeMill-pipeline.md: new "Operator-tunable per-CLI flags" section with the
  EXTRA_ARGS contract.
...
The header callout in `MergeMill.conf.example` is the load-bearing operator-facing artifact.
