...
Today the MergeMill-review wrapper hard-codes Amazon Q Developer review:

- `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh:295-319` injects a "MANDATORY Q review" block into the review-agent prompt with `/q review` trigger and `amazon-q-developer[bot]` login filter.
- `skills/MergeMill-dev/SKILL.md` Step 10 + 11 list `/q review` and `/codex review` as the universe.
...
For users with bots outside the built-in registry, `MergeMill.conf` can set:
...
2. **`MergeMill-review.sh` startup validation** — defense in depth: the wrapper re-validates after sourcing `MergeMill.conf`, in case the wrapper is invoked outside the dispatcher (manual run, custom backend).
...
### `MergeMill-review.sh` review prompt
...
### `MergeMill-dev` SKILL.md / references

Step 10 / 11 in MergeMill-dev/SKILL.md and the per-bot tables in references/review-threads.md and references/review-commands.md become **examples** instead of enumerated truths. They still mention q/codex as the most common cases but with phrasing like "if your project configures Q, then ...".
...
|---|---|---|
| `REVIEW_BOTS` | MergeMill.conf (existing per-project file) | space-separated bot list, or empty |
| `REVIEW_BOTS_<NAME>_TRIGGER` | MergeMill.conf (optional) | custom bot trigger phrase |
| `REVIEW_BOTS_<NAME>_LOGIN` | MergeMill.conf (optional) | custom bot user.login filter |
...
- **Existing single-project deployments** that don't set `REVIEW_BOTS` get the default `q` from `MergeMill.conf.example`. Same as before.
...
- **No format change to `MergeMill.conf`** — purely additive variable.
...
`tests/unit/test-MergeMill-review-prompt.sh` (new — exercises the wrapper's prompt generation):
...
- `MergeMill-dev/SKILL.md` Step 10/11 don't currently invoke the wrapper — they're guidance for the dev agent during development, not the review agent's enforcement loop. We update both to talk in REVIEW_BOTS terms but the actual loop only runs in MergeMill-review.sh.
...
New:
- `skills/MergeMill-dispatcher/scripts/lib-review-bots.sh`
- `tests/unit/test-lib-review-bots.sh`
- `tests/unit/test-MergeMill-review-prompt.sh`
- `docs/designs/configurable-review-bots.md` (this file)

Modified:
- `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` (replace hardcoded Q block)
- `skills/MergeMill-dispatcher/scripts/MergeMill.conf.example` (add REVIEW_BOTS section)
- `skills/MergeMill-dev/SKILL.md` (Step 10 / 11 generalized)
- `skills/MergeMill-dev/references/review-threads.md` (per-bot table → REVIEW_BOTS-driven)
- `skills/MergeMill-dev/references/review-commands.md` (per-bot table → REVIEW_BOTS-driven)
- `skills/MergeMill-review/SKILL.md` (Triggering Bot Reviewers subsection updated)
