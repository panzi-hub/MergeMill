...
We deliberately do **not** follow the issue's literal "mktemp-d at install time + persist path in MergeMill.conf" path because:
...
`MergeMill.conf` is **not** modified. The dir path is computed lazily from `PROJECT_ID` + the user's runtime base, which both wrappers and the dispatcher already have.
...
#   1. MERGEMILL_PID_DIR (env override, used by tests)
#   2. $XDG_RUNTIME_DIR/MergeMill-${PROJECT_ID}   (the canonical Linux choice)
...
| `MergeMill-dev.sh:89` | `/tmp/agent-${PROJECT_ID}-issue-${N}.pid` | `$(pid_dir_for_project)/issue-${N}.pid` |
| `MergeMill-review.sh:71` | `/tmp/agent-${PROJECT_ID}-review-${N}.pid` | `$(pid_dir_for_project)/review-${N}.pid` |
...
|----------|--------|-------|
| Normal wrapper spawn | PID file written to /tmp | PID file written to $XDG_RUNTIME_DIR/MergeMill-${PROJECT_ID}/ |
...
| Cron without DBus session ($XDG_RUNTIME_DIR unset) | n/a | falls back to $HOME/.local/state/MergeMill-${PROJECT_ID}/ |
...
1. `tests/unit/test-pid-dir-helper.sh`:
   ...
   - MERGEMILL_PID_DIR override takes priority
   - Mode 0700 after creation
   - Idempotent on second call
   - Refuses symlink-target dir (rc != 0)
2. Existing `test-pid-guard.sh` and `test-kill-before-spawn.sh` continue to work because they construct PID file paths via the wrapper's own logic; they just need `MERGEMILL_PID_DIR=$tmpdir` exported in their setup.
