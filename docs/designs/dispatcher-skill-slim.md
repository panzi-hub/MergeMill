...
PR-3 of the pipeline-docs plan. Extracts the 224 lines of bash currently embedded in `skills/MergeMill-dispatcher/SKILL.md` into a single-entry-point script (`dispatcher-tick.sh`) backed by a helper library (`lib-dispatch.sh`). **Pure refactor — zero behavior change.**
...
- `skills/MergeMill-dispatcher/scripts/lib-dispatch.sh` — ~13 small composable functions (helpers).
- `skills/MergeMill-dispatcher/scripts/dispatcher-tick.sh` — single entry-point script. Sources `lib-dispatch.sh`, runs Steps 1–5 in order in one process, with `JUST_DISPATCHED` as a normal in-process bash array.
...
- `skills/MergeMill-dispatcher/SKILL.md` — slim from 438 → ~80 lines. Body becomes prose explaining what the dispatcher does, plus a single delegation: `bash "$PROJECT_DIR/scripts/dispatcher-tick.sh"`. Cross-links `docs/pipeline/` for the full state-machine spec.
...
E2E verification (manual): run `bash dispatcher-tick.sh` once locally against a test repo with one MergeMill issue, confirm the same labels and comments appear as before the refactor. Compare against an old SKILL.md run via git stash.

## Per CONTRIBUTING.md Rule 1

This PR touches `skills/MergeMill-dispatcher/scripts/*.sh` and `skills/MergeMill-dispatcher/SKILL.md` (both watched paths). It also touches `docs/pipeline/dispatcher-flow.md`. The CI gate passes via the docs-touched path.
