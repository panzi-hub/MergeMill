...
(`MergeMill-dev.sh` / `MergeMill-review.sh`) into PID_FILE. The actual
...
Both `MergeMill-dev.sh` and `MergeMill-review.sh` install a SIGTERM
...
2. **Defence: scan for orphans by command line.** After the PID-file
   path, additionally `pgrep -f "MergeMill-(dev|review).sh.*--issue ${ISSUE_NUM}\b"`
