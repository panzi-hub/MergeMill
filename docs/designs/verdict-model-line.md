...
Issue: #208 — `feat(MergeMill-review): show the review model on every verdict comment (INV-60)`
...
Resolve EXACTLY as the `Reviewed HEAD:` trailer (`MergeMill-review.sh` ~L1613)
...
### `MergeMill-review.sh::build_review_prompt` — interpolate the 6th arg
...
| File | Change |
|---|---|
| `skills/MergeMill-dispatcher/scripts/post-verdict.sh` | optional 6th `<model>` arg + loose validation + docs |
| `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` | `build_review_prompt` resolves model + passes 6th arg in all 3 examples |
...
| `skills/MergeMill-review/SKILL.md` | usage example shows 6th arg |
| `skills/MergeMill-review/references/decision-gate.md` | usage example shows 6th arg |
| `tests/unit/test-post-verdict.sh` | TC-PV-17.. (6th arg, newline reject, spaces/parens, discriminator) |
| `tests/unit/test-MergeMill-review-verdict-via-helper.sh` | TC-PVP-07.. (model 6th arg in all spots, equals resolved model, codex+non-codex) |
