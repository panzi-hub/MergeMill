...
`MergeMill-mode.md` "Resume Awareness" doc; the marker matching is **wrapper code**
in `MergeMill-dev.sh`. The fix touches both.

## Where the logic lives

| Layer | File | Role |
|---|---|---|
| Wrapper (resume prompt builder) | `skills/MergeMill-dispatcher/scripts/MergeMill-dev.sh` | Selects review feedback + injects prompt blocks |
| Agent reasoning contract | `skills/MergeMill-dev/references/MergeMill-mode.md` § Resume Awareness | Governs the done/not-done exit decision |
| Spec | `docs/pipeline/dev-agent-flow.md` § "Mode = resume", `docs/pipeline/invariants.md` (new INV-57) | Authoritative |

`MergeMill-dev.sh` sources only `lib-agent.sh` + `lib-auth.sh` (NOT `lib-dispatch.sh`),
so the new helper is **inline** in `MergeMill-dev.sh`. Adding it inline (an existing,
...
`MergeMill-mode.md` § Resume Awareness:
...
Plus a doc-contract assertion (MergeMill-mode.md mentions timestamp ordering + the
