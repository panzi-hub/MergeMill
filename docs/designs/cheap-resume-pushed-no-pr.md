...
`MergeMill-dev/SKILL.md` Step 7 runs `git push -u origin <branch>` **immediately before**
...
in `MergeMill-dev/SKILL.md`, executed by the agent with its own PR-body generation. The
...
PR-body generation (the `/MergeMill-dev` skill's Step 7).

Detection logic (`needs_open_pr_only` helper in `MergeMill-dev.sh`):
...
The helper + block are wired into **both** prompt builders in `MergeMill-dev.sh`:
