# Design Canvas — E2E_MODE: pluggable E2E channel for the MergeMill-review wrapper
...
`MergeMill-review`'s E2E channel is hardcoded to **browser-driven UI smoke testing** via Chrome DevTools MCP — login flow, navigate, screenshot upload, structured report. This shape only fits SaaS web apps with a per-PR preview URL.

Concrete trigger: a backend-pipeline consumer's PR required a 30–60 min run on a deployed PR-stage (DDB row state + S3 artifact assertions + visual confirmation of an LLM-cleaned transcript). The existing wrapper had no way to dispatch this — the review agent looped 3 times against operator-gated findings, the dev agent self-deescalated by removing the `MergeMill` label, the operator ran the E2E manually and pasted the evidence by hand. Next consumer in this shape should not need that ceremony.
...
- **A. New skill (e.g. `MergeMill-review-pipeline`).** Doubles the maintenance surface; project switching between web-app and pipeline shapes (e.g. a monorepo) would need two confs.
...
Full contract: `skills/MergeMill-review/references/e2e-command-mode.md`.
