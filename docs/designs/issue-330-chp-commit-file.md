...
`upload-screenshot.sh` REALLY lives at `skills/MergeMill-review/scripts/`
(symlinked into the dispatcher tree at `skills/MergeMill-dispatcher/scripts/` so
...
its OWN `BASH_SOURCE` then `../../MergeMill-dispatcher/scripts/lib-code-host.sh`
(NOT `$_SCRIPT_DIR`, which is deliberately the project-side symlink dir so the
existing `MergeMill.conf` lookup finds the project's conf). Guarded on the verb
...
`upload-screenshot.sh` resolves its own `$REPO` from `MergeMill.conf` and never
