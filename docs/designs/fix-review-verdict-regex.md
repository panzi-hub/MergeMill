...
`MergeMill-review.sh` polls for the agent's verdict comment with this jq
filter (line 474):
...
- `MergeMill-dev.sh` — separate wrapper, not implicated.

### Files changed

- `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` —
  broader regex; prompt clarification.
- `tests/unit/test-MergeMill-review-verdict-regex.sh` — new.
