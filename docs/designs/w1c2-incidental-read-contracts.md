...
|---|---|---|---|
| `MergeMill-dev.sh:604`     | approved-timestamp | `reviews`     | `[.reviews[]? \| select(.state=="APPROVED") \| .submittedAt] \| sort \| last // empty` |
| `MergeMill-review.sh:918`  | preview-URL scrape | `comments`    | `[.comments[].body \| select(contains("Preview"))] \| last`, then `grep -oP … \| head -1` |
| `MergeMill-review.sh:948`  | `headRefName`      | `headRefName` | `.headRefName` |
| `MergeMill-review.sh:949`  | `headRefOid`       | `headRefOid`  | `.headRefOid` |
| `MergeMill-review.sh:1591` | state (E2E gate)   | `state`       | `.state` → `_pr_open_gate` |
| `MergeMill-review.sh:3312` | state (PASS chain) | `state`       | `.state` → `_pr_open_gate` |
| `MergeMill-review.sh:3357` | bot-review-wait count | `comments` | `[.comments[] \| select(.body \| contains("bot-review-wait sha=\"…\""))] \| length` |
...
|---|---|---|
| `MergeMill-dev.sh:1091` | dev-resume `PR_REVIEW_COMMENTS` | `[.[] \| "- **\(.path):\(.line // "N/A")** — \(.body)"] \| join("\n")` |
...
- `skills/MergeMill-dispatcher/scripts/providers/chp-github.sh` — the two
  leaves.
- `skills/MergeMill-dispatcher/scripts/lib-code-host.sh` — shim posture
...
- `tests/unit/test-MergeMill-review-fail-branch-open-guard.sh` — line-order
  pins retargeted to `"state"`.
- `tests/unit/test-MergeMill-review-e2e-gate-open-guard.sh` — line-order
  pins retargeted to `"state"`.
- `tests/unit/test-dev-resume-post-approval-findings.sh` — isolation harness
  sources the REAL leaf.
- `tests/unit/test-MergeMill-review-sequential-e2e.sh` — TC-SE2E-FETCH
