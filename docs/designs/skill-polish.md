...
- Description doesn't trigger on "implement a GitHub issue end-to-end" / "MergeMill mode" — half the skill's purpose.
...
- Add to description: explicit "implement a GitHub issue", "MergeMill mode", "unattended" triggers.
- Rewrite the bold callout in infinitive form.
- Target ~300 lines.

### 3. `MergeMill-dispatcher`
...
- Replace `../../docs/pipeline/` links with absolute GitHub URLs (`https://github.com/panzi-hub/MergeMill/blob/main/docs/pipeline/...`).
...
- Missing "When to Use / When Not to Use" — competes with MergeMill-dev's PR-review step.
- The `gh-as-user.sh`-required rule for bot-reviewer triggers is buried in a blockquote.

**Fixes:**
- Description adds: "PR labeled for MergeMill review", "decide to approve or send back for fixes".
...
|---|---|---|
| MergeMill-dispatcher | `../../docs/pipeline/...` (5+ links) | Absolute GitHub URLs |
| MergeMill-dev | `references/...` (already in skill dir — OK) | unchanged |
| MergeMill-review | `references/...` (already in skill dir — OK) | unchanged |
| MergeMill-common | `hooks/README.md` (in skill dir — OK) | unchanged |

The hook-config blocks in `MergeMill-dev` and `MergeMill-review` frontmatter use `$CLAUDE_PROJECT_DIR/hooks/...`. After `npx skills add`, hooks live at `$CLAUDE_PROJECT_DIR/.claude/skills/MergeMill-common/hooks/...`. The MergeMill-common SKILL.md already documents the symlink workaround for this — keep that documentation but make it more prominent (move into the description triggers so it shows up when a user reports "hooks not running").
...
- Hook scripts in `skills/MergeMill-common/hooks/` — no behavior changes.
- Project-level `docs/pipeline/` — already accurate, just unreachable post-install for MergeMill-dispatcher.

## Risk

Low. Description rewrites change skill triggering — an agent might match different user phrasings than before. Mitigation: keep all old triggers + add new ones (don't remove). The structural extraction in MergeMill-dev preserves all content (just moves it).
