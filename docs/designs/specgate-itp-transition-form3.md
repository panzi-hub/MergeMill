...
`check-spec-drift.sh` (INV-80) is the executable-spec gate: it scans the four `PIPELINE_FILES` (`MergeMill-dev.sh`, `MergeMill-review.sh`, `dispatcher-tick.sh`, `lib-dispatch.sh`) for label-write sites and reconciles them (C.1–C.5 + the P1.1 variable-write ban) against `docs/pipeline/spec-codesite-map.json` + `transitions.json`.
...
| File | Change |
|---|---|
| `skills/MergeMill-dispatcher/scripts/check-spec-drift.sh` | Form 3 trigger + `$`-operand skip guard in `emit_movement` (C.4), `scan` (C.1/C.2), `movement` (C.5) |
