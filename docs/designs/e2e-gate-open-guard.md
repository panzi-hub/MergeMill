...
`MergeMill-review.sh` re-checks PR state before writing a `reviewing →
...
`MergeMill-review.sh:1060` `fail` / `:1084` `block-nonsubstantive`). This PR
...
- The verdict-`FAILED` `else` branch (`MergeMill-review.sh` ~`:1761`) and the
