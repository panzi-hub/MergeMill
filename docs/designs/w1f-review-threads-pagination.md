...
`skills/MergeMill-common/scripts/resolve-threads.sh:74-75`:
...
`skills/MergeMill-common/hooks/verify-completion.sh:71` and `:100` still
...
(hook layer, not provider layer) and not MergeMill-managed. The operator
files a non-`MergeMill` follow-up issue and this PR's body links to it
