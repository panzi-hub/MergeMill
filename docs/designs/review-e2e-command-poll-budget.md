...
1. **The verdict-poll budget is a fixed 30 s.** After the fan-out `wait`
   returns, `MergeMill-review.sh` polls issue comments `6 × 5 s = 30 s`
...
`MergeMill.conf.example` gains a cross-reference note on
...
smaller than the E2E it dispatched.* Producer: `MergeMill-review.sh`
...
- `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` — poll-budget
  resolver, `seq` change, reap step, multi-agent prompt signal + Step 4b
  strengthening.
- `skills/MergeMill-review/references/e2e-command-mode.md` — Fix D doc.
- `skills/MergeMill-dispatcher/scripts/MergeMill.conf.example` — cross-ref note.
