...
idiom) because `MergeMill-review.sh` does not source it.
...
`lib-dispatch.sh`, while `MergeMill-dev.sh`, `MergeMill-review.sh`,
...
- `MergeMill-dev.sh`: 5 sites (2 heredoc session-report bodies + 3 resume-fallback markers),
- `MergeMill-review.sh`: 17 sites (verdict / diagnostic / held / smoke comments + the Reviewed-HEAD trailer capture),
...
at wrapper startup (`load_MergeMill_conf`), so the verb's `$REPO` read is always in
