...
1. **`skills/MergeMill-dispatcher/scripts/lib-lane.sh`**: `_lane_backend()`
...
2. **`skills/MergeMill-dispatcher/scripts/lib-guardian.sh`**: `do_reap()`
   gains the identical `_lane_scope_kill()` call, in the identical position
   (before the pgid escalation), for the identical reason.
3. **`skills/MergeMill-dispatcher/scripts/adt-gc.sh`**: `--doctor` gains a
   ...
   inside the caller.
4. **`skills/MergeMill-dispatcher/scripts/MergeMill.conf.example`**: two
