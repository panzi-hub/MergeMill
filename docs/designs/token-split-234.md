...
> The dev agent's existing `gh pr create` call (MergeMill-dev SKILL Step 7) keeps
...
- `MergeMill-dev.sh` — call `setup_agent_token`; drain `$AGENT_PR_CREATE_FILE`.
- `MergeMill-review.sh` — call `setup_agent_token`; E2E broker post.
