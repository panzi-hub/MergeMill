...
No change is required in `lib-review-smoke.sh` or `MergeMill-review.sh`; the gate
already treats `unavailable` correctly (drop / all-unavailable fall-through).

## Files changed

- `skills/MergeMill-dispatcher/scripts/lib-agent-smoke.sh` — `_smoke_classify`
...
- `tests/unit/test-lib-agent-smoke.sh`, `tests/unit/test-MergeMill-review-smoke-gate.sh`
