...
2. **Wrapper threading** — `MergeMill-dev.sh` + `MergeMill-review.sh` mint
...
#233 already writes `…/MergeMill-<project>/runs/<review-session-uuid>/verdict-<agent>.json`,
...
```
${XDG_STATE_HOME:-$HOME/.local/state}/MergeMill-<project>/runs/
...
- New wrapper stdout **tees** into `run.log` (the existing `/tmp` redirect in
  ...
  `bash MergeMill-dev.sh` invocation also gets a run.log).
...
Sources `lib-config.sh` (`load_MergeMill_conf`) then `lib-dispatch.sh`, then
