...
the *dispatch* instant, which can lag the real `MergeMill`-label time by ticks
(concurrency cap, unresolved deps). For accurate TTHW the tick also fetches the
true `MergeMill`-label timestamp from the GitHub issue timeline and emits it as
...
if declare -F metrics_emit >/dev/null 2>&1; then
  _labeled_at="$(gh api "repos/${REPO}/issues/${issue_num}/timeline" \
    --jq 'map(select(.event == "labeled" and .label.name == "MergeMill")) | (.[0].created_at // empty)' \
    2>/dev/null || true)"
  ...
fi
```
...
| 3 | `dispatcher-tick.sh:304-314` | Call `itp_label_event_ts "$issue_num" "MergeMill"` guarded by the **bare** provider expression matching the shim |
...
on-box: label `MergeMill" or .label.name == "bug` widens the selector → returns
...
For `LABEL=MergeMill`, `lbl_json` is exactly `"MergeMill"` → **argv-equivalent**
...
re-run; Step-1 `npx skills update -g MergeMill-dispatcher` alone propagates it
