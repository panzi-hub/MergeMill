...
1. **`lib-agent.sh` startup guard** (INV-38): `AGENT_REVIEW_LAUNCHER` non-empty
   ...
   even starts.
2. **Fan-out subshell** (`MergeMill-review.sh`): force-clears
...
### 2. `MergeMill-review.sh` fan-out subshell
...
(`AGENT_LAUNCHER`, `AGENT_DEV_LAUNCHER`, `_parse_extra_args`) — `MergeMill.conf`
...
- `skills/MergeMill-dispatcher/scripts/MergeMill.conf.example` — commented
...
- Resolver unit (extend `test-MergeMill-review-per-agent-model.sh` or a new
