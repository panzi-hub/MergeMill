...
For each match: strip the transitional labels (single `gh issue edit --remove-label A --remove-label B ...` call). Keep `approved`/`stalled`/`MergeMill`/`no-auto-close` intact. Always log to dispatcher stdout.
