...
1. `dispatch-local.sh` cannot find `MergeMill.conf` because `SCRIPT_DIR` points
   to the installed skill location, not the project's `scripts/` directory.
2. `MergeMill-dev.sh` and `MergeMill-review.sh` use `readlink "$0"` which only
...
### 1. `MergeMill-dev.sh` and `MergeMill-review.sh` (line 17)
...
for the MergeMill pipeline scripts.
