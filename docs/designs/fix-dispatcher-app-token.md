...
sourced from the agent wrappers (`MergeMill-dev.sh`, `MergeMill-review.sh`).
...
2. **Path-entry local projects** — vars come from `load_MergeMill_conf`
   sourcing the per-project `MergeMill.conf`.
3. **Direct invocation** (single-project deployments) — vars come from
   `load_MergeMill_conf` finding `MergeMill.conf` via the script-local or
...
- `skills/MergeMill-dispatcher/scripts/dispatcher-tick.sh` — add token-gen
