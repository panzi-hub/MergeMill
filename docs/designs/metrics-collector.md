...
Add an **observe-only** metrics lane to the MergeMill pipeline so the stability
...
| File | Kind | Role |
|---|---|---|
| `skills/MergeMill-dispatcher/scripts/lib-metrics.sh` | sourced lib | `metrics_emit`, `metrics_dir`, `metrics_prune` — append-only JSONL writer + retention |
| `skills/MergeMill-dispatcher/scripts/metrics-report.sh` | executable | aggregator: incidents/month, cost-per-merged-PR, quota-failure rate, TTHW |
...
1. `${MERGEMILL_METRICS_DIR}` — test/override hook (mirrors `MERGEMILL_PID_DIR`).
2. `${XDG_STATE_HOME}/MergeMill-${PROJECT_ID}` — honors the issue's stated
   `XDG_STATE_HOME` convention when set.
3. `${HOME}/.local/state/MergeMill-${PROJECT_ID}` — the **durable** fallback
...
|---|---|---|
| dev start | `MergeMill-dev.sh` ~123 | after `LOG_FILE=`, capture `METRICS_START_TS` |
| dev end | `MergeMill-dev.sh:401` `cleanup()` | emit `wrapper_end` with `exit_code` + duration; token parse from `$LOG_FILE` |
| dev agent rc | `MergeMill-dev.sh:602/746/832` | `AGENT_EXIT` |
| review start | `MergeMill-review.sh:605` `WRAPPER_START_TS` | reuse as start ts |
| review end | `MergeMill-review.sh:448` `cleanup()` | emit `wrapper_end` |
| verdict | `MergeMill-review.sh:1902` `AGGREGATE=` | emit `verdict` + per-agent `agent_drop` |
| merge | `MergeMill-review.sh:2319` `MERGE_RC=` | emit `merge` success/failure |
