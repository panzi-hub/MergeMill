...
```bash
# MergeMill-review.sh, inside the per-agent verdict-poll loop (pre-#180):
else
...
- The separate `all-unavailable` crash-vs-no-verdict discriminator
  (~`MergeMill-review.sh:1297`). Its `AGENT_LAUNCH_RC[…] -ne 0` check only runs
...
2. **`MergeMill-review.sh`** — replace the inline loop with a call to
...
enforced. Producer: `MergeMill-review.sh` (poll-loop caller) +
...
- `skills/MergeMill-dispatcher/scripts/lib-review-poll.sh` —
  ...
  testable loop).
- `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` — call
