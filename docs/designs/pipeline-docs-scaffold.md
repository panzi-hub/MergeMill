...
- The label state machine for an `MergeMill` issue
...
3. **`.github/workflows/pipeline-docs-gate.yml`** — CI hard gate. Fails the PR check if it touches any of:
   ...
   - `skills/MergeMill-dev/scripts/` (if/when added)
   - `skills/MergeMill-review/scripts/` (if/when added)
   - `skills/MergeMill-common/hooks/`
   - `skills/MergeMill-common/scripts/`
...
### Why a separate `docs/pipeline/` directory and not extending `docs/MergeMill-pipeline.md`?

The existing `MergeMill-pipeline.md` is an overview / quick-start. The new docs are normative state-machine references that will grow as bugs surface invariants. Separate dir keeps overview vs. spec-grade docs distinct, and lets the CI gate watch a focused path glob.

The existing `docs/MergeMill-pipeline.md` stays — PR-2 will add a "see `docs/pipeline/` for the authoritative state machine" pointer at its top.
...
| Path | Why watched |
|---|---|
| `skills/MergeMill-dispatcher/scripts/*.sh` | All the wrapper / dispatch / lib-* scripts. Any change here is a state-machine change. |
| `skills/MergeMill-common/hooks/*.sh` | Workflow-enforcement hooks. Changes affect dev-agent flow. |
| `skills/MergeMill-common/scripts/*.sh` | mark-issue-checkbox / reply-to-comments / resolve-threads — all participate in the dev-agent's PR lifecycle. |
| `skills/MergeMill-{dispatcher,dev,review,common}/SKILL.md` | Until PR-3, SKILL.md *is* the state machine. |

Not watched (intentionally):

- `skills/create-issue/` — issue-authoring helper, not part of the dispatch loop.
- `skills/MergeMill-*/references/` — long-form prose only, no behavior.
...
- Open a quick test PR with a no-op edit to `skills/MergeMill-dispatcher/scripts/setup-labels.sh` and no docs change → confirm the gate fails. Then add the `pipeline-docs:none` label → confirm it passes. (Done in a throwaway branch, not landed.)
