...
`MergeMill-common/hooks/post-file-edit-reminder.sh` fires on every
...
- `skills/MergeMill-dev/SKILL.md` explicitly describes when to run tests,
...
1. **Delete** `skills/MergeMill-common/hooks/post-file-edit-reminder.sh`.
2. **Unregister** from hook configs:
   ...
   - `skills/MergeMill-dev/SKILL.md` — remove the two registration snippets
     in the hook configuration block.
3. **Documentation:**
   - `skills/MergeMill-common/SKILL.md` — drop the row from the hook table.
   - `skills/MergeMill-common/hooks/README.md` — drop the row from the hook
     table.
