...
| # | Title | Fix shape |
|---|---|---|
| #58 | `readlink -f` breaks MergeMill.conf lookup with symlinked vendoring | Refactor: extract `lib-config.sh::load_MergeMill_conf`, drop `readlink -f`, fix the `../../../` fallback depth |
...
Per project direction: any fix that touches code with duplicated patterns must consolidate before fixing. Three of the four issues touch dispatcher / lib-agent / lib-auth, where the `MergeMill.conf` loading block appears **byte-identical in three files**:

- `skills/MergeMill-dispatcher/scripts/lib-agent.sh` lines 6–17
- `skills/MergeMill-dispatcher/scripts/lib-auth.sh` lines 8–19
- `skills/MergeMill-dispatcher/scripts/dispatcher-tick.sh` lines 19–31
...
One function `load_MergeMill_conf` with the following contract:

- Tries 3 sources in priority order:
  1. `$MERGEMILL_CONF` env var (highest, explicit override)
  2. Same directory as the *invocation* path (NOT `readlink -f`, so symlink-vendor pattern works — #58 fix)
  3. `${PROJECT_DIR}/scripts/MergeMill.conf` if `PROJECT_DIR` is set (more robust than the broken `../../../scripts/` fallback)
...
- Function-scoped: writes `MERGEMILL_CONF_LOADED_FROM` global for diagnostics, doesn't pollute caller scope otherwise.
...
load_MergeMill_conf "${SCRIPT_DIR}"
```
...
Each file's other code uses its own `_LIB_*_DIR` variable. PR-4 will keep those for now — they're cheap, single-use, and consolidating them would mean editing every reference inside each file (out-of-scope creep). The `load_MergeMill_conf` helper is the only block that's truly identical across all three.
...
- New test: `tests/unit/test-lib-config.sh` (4 cases)
  1. `MERGEMILL_CONF` env var takes priority over filesystem search.
  2. Script-local `MergeMill.conf` is found (no `readlink -f` so symlinked path works).
  3. `${PROJECT_DIR}/scripts/MergeMill.conf` fallback is found when neither of the above is set.
...
- Update existing `test-bash-source-empty.sh`:
  - The test that simulates the symlinked-vendoring layout (#58 / #39 reproducer) now passes against the new `load_MergeMill_conf`.

## Per CONTRIBUTING.md Rule 1

This PR touches `skills/MergeMill-dispatcher/scripts/*.sh` (multiple watched files). It also touches `docs/pipeline/invariants.md` (INV-11, INV-14 status flips, new INV-16). The CI gate passes via the docs-touched path.
...
- Unit tests for `load_MergeMill_conf` covering all 3 priority paths.
