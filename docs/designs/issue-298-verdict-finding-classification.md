...
- **Protected-paths list** — new lib `skills/MergeMill-dispatcher/scripts/lib-review-classify.sh` (a **lib**, so no installer re-run):
  ```sh
  ...
  ```
  Documented as a conf override in `MergeMill.conf.example`. This is the primary, deterministic policy surface.
...
| `MergeMill-review.sh` | entry (edit) | prompt template/rules; aggregate recompute + emit at line 3373; hardcode `=true` at 754/1460/3121/3199 |
| `MergeMill.conf.example` | data | `REVIEW_PROTECTED_PATHS` doc |
