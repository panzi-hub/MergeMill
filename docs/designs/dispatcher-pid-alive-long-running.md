...
A long-running healthy `MergeMill-dev` wrapper (~70+ min, single Claude session, real progress, eventual successful PR) is repeatedly classified as crashed by the dispatcher's `pid_alive` check, exhausting `MAX_RETRIES` and triggering `mark_stalled` despite continuous progress. See issue #129 for the full timeline.
...
In `skills/MergeMill-dispatcher/scripts/dispatch-local.sh::kill_stale_wrapper`, the `rm -f "$pid_file"` at the bottom of the function runs even when the inner `kill -0 "$old_pid"` returned failure (i.e. nothing was actually killed). This is the bug the inline comment "Remove PID file regardless" identifies — "regardless" is precisely wrong.
