...
`reply-to-comments.sh:41` (an MergeMill-common util) is the **last** raw
...
The verb seam lives in **MergeMill-dispatcher**; the util in
**MergeMill-common**. The `readlink -f` self-source is the [INV-14]/[INV-65]
...
MergeMill.conf, while `readlink -f "$BASH_SOURCE"` resolves the real skill tree).
