...
(`MergeMill-dev.sh:69`, `MergeMill-review.sh:207`) and the exact verb form is
already in production (`MergeMill-dev.sh:1174`: `itp_read_task "$ISSUE_NUMBER" title,body -q '.'`).
...
|---|---|---|---|
| B5 | `MergeMill-dev.sh:887` | `ISSUE_BODY=$(gh issue view "$ISSUE_NUMBER" --repo "$REPO" --json title,body,comments -q '.')` | `ISSUE_BODY=$(itp_read_task "$ISSUE_NUMBER" title,body,comments -q '.')` |
| B7 | `MergeMill-review.sh:3439` | `HAS_NO_AUTO_CLOSE=$(gh issue view "$ISSUE_NUMBER" --repo "$REPO" --json labels \`<br>`  -q '[.labels[].name] \| any(. == "no-auto-close")' 2>/dev/null \|\| echo "false")` | `HAS_NO_AUTO_CLOSE=$(itp_read_task "$ISSUE_NUMBER" labels \`<br>`  -q '[.labels[].name] \| any(. == "no-auto-close")' 2>/dev/null \|\| echo "false")` |
...
3. **Live-wrapper edit is safe**: the MergeMill dev agent works in a worktree;
   ...
   batches (B1–B4).
4. **Precedent**: `itp_read_task … -q` already ships at `MergeMill-dev.sh:1174`.

## Baseline impact (AC3)

`scripts/providers/cutover-baseline.json` shrinks by **exactly 2** surviving
signatures — the MergeMill-dev.sh `gh issue view … title,body,comments`
survivor and the MergeMill-review.sh `gh issue view … labels` survivor.
...
- Comment-reads (`MergeMill-dev.sh:612/1048` `gh issue view --json comments` →
