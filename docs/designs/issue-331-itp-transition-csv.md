...
```
status: DESIGN — implementing this session (MergeMill, #296 second-tier)
issue: #331
...
|---|---|---|---|
| A1 | `MergeMill-dev.sh` (PR-found success) | `gh issue edit … --remove-label in-progress --remove-label pending-dev --add-label pending-review` | `… \|\| log "WARNING…"` |
| A2 | `MergeMill-review.sh` (post-merge approved-flip, INV-33) | `gh issue edit … --remove-label reviewing --remove-label MergeMill --add-label approved` | `… 2>/dev/null \|\| true` |
...
| B  | `MergeMill-review.sh` (auto-merge-fail re-queue) | `if ! _edit_err=$(gh issue edit … --remove-label reviewing --add-label pending-dev 2>&1 >/dev/null); then` | `if ! _err=$(… 2>&1 >/dev/null); then log` stderr-capture |
...
- **A1** `MergeMill-dev.sh`: `itp_transition_state "$ISSUE_NUMBER" "in-progress,pending-dev" "pending-review" || log "WARNING: Failed to update issue labels"` (preserve `|| log`).
- **A2** `MergeMill-review.sh`: `itp_transition_state "$ISSUE_NUMBER" "reviewing,MergeMill" "approved" 2>/dev/null || true` (preserve `2>/dev/null || true`).
...
- **B** `MergeMill-review.sh`: `if ! _edit_err=$(itp_transition_state "$ISSUE_NUMBER" "reviewing" "pending-dev" 2>&1 >/dev/null); then log …` (single-remove via the unchanged 3-positional verb; preserve the stderr-capture `if ! …` framing).
...
| `review-pass-merged` | `--remove-label "MergeMill"` (review:3467) | `never close the issue directly` (the INV-33 comment, already a `sites[]` anchor) |
...
`sites[]` entry (`MergeMill-dev.sh … "movement": "in-progress,pending-dev|pending-review"`).
A2 → `MergeMill,reviewing|approved` (sorted) matches the existing `MergeMill,reviewing|approved`
...
`MergeMill-dev.sh` (×1), `MergeMill-review.sh` (×2: the A2 + B `gh issue edit` lines),
...
| `MergeMill-dev.sh` | A1 → `itp_transition_state … "in-progress,pending-dev" "pending-review"` |
| `MergeMill-review.sh` | A2 → `… "reviewing,MergeMill" "approved"`; B → `… "reviewing" "pending-dev"` (stderr-capture) |
