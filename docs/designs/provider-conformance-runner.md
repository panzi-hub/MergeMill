...
`MERGEMILL_PROVIDERS_DIR` override at source time. To let `--itp X --chp Y`
...
| name | source dir |
|---|---|
| `github` | `skills/MergeMill-dispatcher/scripts/providers/` (skill tree, `readlink -f`) |
...
single `MERGEMILL_PROVIDERS_DIR` at the scratch dir. This is the load-bearing
...
`_render_close_keyword` fallback (`MergeMill-dev.sh`) renders the non-closing
...
real `_render_close_keyword` body from `MergeMill-dev.sh` (the same technique
