...
| #67 (INV-15) | Step 5a SIGTERM races wrapper trap; trap routes to `pending-dev`, not `pending-review` | wrapper `MergeMill-dev.sh::cleanup` |
...
`AGENT_TIMEOUT` is overridable via `MergeMill.conf` or env. Default `4h` (issue #60 recommendation). `--kill-after=30s` escalates to SIGKILL if the agent process group ignores SIGTERM. `--signal=TERM` lets the agent flush any final SSE bytes before dying.
