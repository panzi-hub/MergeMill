...
| `chp_github_mergeable PR` | `gh pr view $PR --repo $REPO --json mergeable -q '.mergeable'` | `MergeMill-review.sh:3202` (the ONLY leaf moved; [M2]) |
...
| `chp_github_approve PR` | `gh pr review $PR --repo $REPO --approve --body …` | `MergeMill-review.sh:3306` |
...
| `chp_github_merge PR` | `gh pr merge $PR --repo $REPO --squash --delete-branch` | `MergeMill-review.sh:3344` |
...
| `chp_github_close_keyword ISSUE` | renders `Closes #<ISSUE>` (no `gh`) | `MergeMill-dev.sh` `Closes #${ISSUE_NUMBER}` prompt literals |
...
- **Leave unnamed incidental PR reads RAW** (enumerated in the PR body): the
  ...
  lookups (`MergeMill-dev.sh:379/709/801/982`) and the PR-number-keyed
  `gh pr view $PR --json comments/state/headRefName/headRefOid/reviews`
  (`MergeMill-dev.sh:547`, `MergeMill-review.sh:873/903/904/1484/3119/3153`).
...
into `lib-pr-linkage.sh`, `lib-dispatch.sh`, `MergeMill-review.sh`,
