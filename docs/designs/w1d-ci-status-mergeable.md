...
- The mergeable poll (`MergeMill-review.sh:3491`) composes
...
`MergeMill-review.sh`, `coverage.conf`, `cap-map.conf`, three test files
(`test-chp-pr-lifecycle.sh`, `test-MergeMill-review-mergeable-gate.sh`,
...
- The 3 review-agent prompt heredocs instructing the agent to run `gh pr
  checks` itself (MergeMill-review.sh:1178/1238/1426) — prompt-prose,
...
- `verify-completion.sh`'s raw `gh pr checks` (MergeMill-common hook,
