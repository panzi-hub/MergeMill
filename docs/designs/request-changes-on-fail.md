...
1. **FAIL half (original #193 scope).** When `MergeMill-review.sh` reaches a
...
- `skills/MergeMill-review/SKILL.md` — overview (`:28`), When-to-Use (`:34`),
...
- `skills/MergeMill-review/references/decision-gate.md` — the PASS/FAIL
...
The wrapper-injected prompt's Decision section (`MergeMill-review.sh:773-814`)
...
- `skills/MergeMill-dispatcher/scripts/MergeMill-review.sh` — `submit_request_changes` helper + calls on the two substantive FAIL routes.
- `skills/MergeMill-review/SKILL.md` — framing fix + wrapper-owns-action rule.
- `skills/MergeMill-review/references/decision-gate.md` — action-pairing fix.
...
- `tests/unit/test-MergeMill-review-request-changes.sh` — new regression test.
