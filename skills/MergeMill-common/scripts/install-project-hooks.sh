...
#
# Why this exists:
#   Consumer projects that bootstrap against MergeMill-dev-team symlink the
...
#      NOT `*-aws-ssm.sh`) from the on-disk MergeMill-dispatcher/scripts/ into
...
#   3. Symlinks <project>/hooks → MergeMill-common/hooks.
...
#   - Doesn't symlink MergeMill-common/scripts/* (reached via .agents paths).
#
# Usage:
#   bash <project>/.agents/skills/MergeMill-common/scripts/install-project-hooks.sh
...
# MergeMill-common hooks dir. Mirrors lib-installer.sh's probe order so
...
for base in \
  ...
  "$ROOT/.claude/skills" \
  "$ROOT/skills"; do
  if [[ -d "$base/MergeMill-dispatcher/scripts" ]]; then
    DISPATCHER_DIR="$base/MergeMill-dispatcher/scripts"
    HOOKS_DIR="$base/MergeMill-common/hooks"
    break
  fi
done
...
if [[ -z "$DISPATCHER_DIR" ]]; then
  cat >&2 <<EOF
ERROR: cannot find MergeMill-dispatcher skill on disk under:
  $ROOT/.agents/skills/
  $ROOT/.claude/skills/
  $ROOT/skills/
...
if (( DOCTOR == 1 )); then
  ...
  conf="$ROOT/scripts/MergeMill.conf"
  if [[ -f "$conf" ]]; then
    mode="$(stat -c '%a' "$conf" 2>/dev/null || stat -f '%Lp' "$conf" 2>/dev/null || echo '???')"
    if [[ "$mode" == "600" ]]; then
      echo "  [ok] scripts/MergeMill.conf present (mode $mode)"
    else
      echo "  [warn] scripts/MergeMill.conf present but mode is $mode (expected 600)"
    fi
  else
    echo "  [FAIL] scripts/MergeMill.conf is MISSING — copy MergeMill.conf.example and fill it in"
    problems=$((problems + 1))
  fi
...
# so project-local files (MergeMill.conf, deploy.sh, validators, …) coexist
