...
#
# Usage:
#   bash skills/MergeMill-common/scripts/install-kiro-hooks.sh
#   bash skills/MergeMill-common/scripts/install-kiro-hooks.sh --agent MergeMill-dev
#
...
if [[ ! -f "$target" ]]; then
  ...
  jq -n --argjson meta "$managed_meta" --argjson hooks "$translated_hooks" \
    ...
    '$meta + {
       name: $name,
       description: "MergeMill-dev workflow agent",
  ...
  if ! jq -e '.hooks.preToolUse | length > 0' "$tmp" >/dev/null \
     || ! jq -e '._managed_by == "MergeMill-common"' "$tmp" >/dev/null; then
    mv "$backup" "$target"
