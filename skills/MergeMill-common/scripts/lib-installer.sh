...
#       Runs the MergeMill-common per-worktree git pre-push installer
...
# Path to the MergeMill-common scripts dir (where this lib lives).
...
merge_hooks_settings() {
  ...
  #   - We additionally hard-check `_managed_by == "MergeMill-common"` so
  ...
  if ! jq -e "($verify_filter) and (._managed_by == \"MergeMill-common\")" "$tmp" >/dev/null; then
    mv "$backup" "$target"
...
#
# Heal +x on the MergeMill-dispatcher's directly-invoked wrapper scripts
...
ensure_dispatcher_scripts_executable() {
  ...
  for candidate in \
    "$root/.agents/skills/MergeMill-dispatcher/scripts" \
    "$root/.claude/skills/MergeMill-dispatcher/scripts" \
    "$root/skills/MergeMill-dispatcher/scripts"; do
    if [[ -d "$candidate" ]]; then
  ...
  if [[ -z "$dispatcher_dir" ]]; then
    echo "WARN: ensure_dispatcher_scripts_executable: no MergeMill-dispatcher/scripts dir found under $root — skipping" >&2
    return 0
  fi
