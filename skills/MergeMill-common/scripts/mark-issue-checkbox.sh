...
for _conf_candidate in \
    "${SCRIPT_DIR}/MergeMill.conf" \
    "${SCRIPT_DIR}/../../MergeMill-dispatcher/scripts/MergeMill.conf" \
    "$(cd "${SCRIPT_DIR}/../../.." 2>/dev/null && pwd)/scripts/MergeMill.conf"; do
...
# The provider lib lives in the MergeMill-dispatcher skill tree; resolve it via
...
# above finds the project's MergeMill.conf. Guarded on EITHER verb being undefined
...
if ! declare -F itp_read_task >/dev/null 2>&1 || ! declare -F itp_mark_checkbox >/dev/null 2>&1; then
  ...
  _mic_lib="${_mic_real_dir}/../../MergeMill-dispatcher/scripts/lib-issue-provider.sh"
  if [[ -n "$_mic_real_dir" && -r "$_mic_lib" ]]; then
    # shellcheck source=../../MergeMill-dispatcher/scripts/lib-issue-provider.sh
    source "$_mic_lib"
  fi
