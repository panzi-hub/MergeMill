...
#     REPO      — owner/repo (default: owner/repo, override via MergeMill.conf)
...
for _conf_candidate in \
    "${_SCRIPT_DIR}/MergeMill.conf" \
    "${_SCRIPT_DIR}/../../MergeMill-dispatcher/scripts/MergeMill.conf" \
    "$(cd "${_SCRIPT_DIR}/../../.." 2>/dev/null && pwd)/scripts/MergeMill.conf"; do
...
# provider lib lives in the MergeMill-dispatcher skill tree; resolve it via
...
# conf-lookup above finds the project's MergeMill.conf. Guarded on the verb being
