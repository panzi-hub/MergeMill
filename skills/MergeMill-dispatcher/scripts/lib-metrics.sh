#!/bin/bash
# lib-metrics.sh — INV-70 observe-only metrics emitter for the MergeMill
# pipeline (issue #228).
#
...
#   1. ${MERGEMILL_METRICS_DIR}                   — test/override hook
#   2. ${XDG_STATE_HOME}/MergeMill-<project>      — when XDG_STATE_HOME is set
#   3. ${HOME}/.local/state/MergeMill-<project>   — DURABLE fallback
#
...
metrics_dir() {
  ...
  if [[ -n "${MERGEMILL_METRICS_DIR:-}" ]]; then
    dir="${MERGEMILL_METRICS_DIR}"
  elif [[ -z "$project_id" ]]; then
    return 1
  elif [[ -n "${XDG_STATE_HOME:-}" ]]; then
    dir="${XDG_STATE_HOME}/MergeMill-${project_id}"
  elif [[ -n "${HOME:-}" ]]; then
