...
# Source this file in MergeMill-dev.sh and MergeMill-review.sh.
...
# exported MERGEMILL_CONF_DIR, falling back to our own unresolved BASH_SOURCE
...
_LIB_AUTH_DIR="${MERGEMILL_CONF_DIR:-$_LIB_AUTH_OWN_DIR}"
...
load_MergeMill_conf "${_LIB_AUTH_DIR}" || true
...
#   4. MergeMill-dev.sh / MergeMill-review.sh — the wrapper-level
...
setup_github_auth() {
  ...
  # paths (issue #163):
  #
  #   1. The wrapper's OWN bare `gh` calls (MergeMill-dev.sh / -review.sh)
...
drain_agent_pr_create() {
  ...
  # MergeMill-dev.sh:397). `git ls-remote` is git-transport plumbing, not code-host
...
drain_agent_bot_triggers() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    # MERGEMILL_CONF_DIR) the broker resolved above, so the emitted
...
# Export REAL_GH if set in MergeMill.conf so gh-with-token-refresh.sh sees
