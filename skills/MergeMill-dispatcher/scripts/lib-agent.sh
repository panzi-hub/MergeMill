...
# fallback. Source this file in MergeMill-dev.sh and MergeMill-review.sh.
#
...
#   load-bearing flags via these vars (see MergeMill.conf.example
...
#              MergeMill.conf.example "gemini block".
...
#              see MergeMill.conf.example "kiro block".
...
# away from the project's scripts/ where MergeMill.conf lives. The entry
# therefore exports MERGEMILL_CONF_DIR (its UNRESOLVED dir) for us to use;
...
load_MergeMill_conf "${MERGEMILL_CONF_DIR:-$_LIB_AGENT_DIR}" || true

# Ensure PROJECT_DIR is an absolute path to the repo root.
# MergeMill.conf may use a relative BASH_SOURCE trick that can resolve
# incorrectly when sourced indirectly. Fall back to <conf-dir>/../../.. —
# anchored on the entry's project-side conf dir (MERGEMILL_CONF_DIR), not this
...
_LIB_AGENT_CONF_DIR="${MERGEMILL_CONF_DIR:-$_LIB_AGENT_DIR}"
PROJECT_DIR="${PROJECT_DIR:-$(cd "${_LIB_AGENT_CONF_DIR}/../../.." && pwd)}"

# Agent configuration (overridable via env or MergeMill.conf)
...
KIRO_AGENT_NAME="${KIRO_AGENT_NAME:-MergeMill-dev}"
...
# See MergeMill.conf.example for the operator-facing usage and rationale.
#
# We tokenize the launcher string into AGENT_LAUNCHER_ARGV[] once via `eval`
# at load time. This trusts MergeMill.conf (same trust level as the
  ...
  if ! eval "AGENT_LAUNCHER_ARGV=($AGENT_LAUNCHER)" 2>/dev/null; then
    ...
    if command -v error_surface >/dev/null 2>&1; then
      error_surface "$(error_peek_issue_arg "$@")" ADT_CFG_LAUNCHER_PARSE \
        ...
        "Fix AGENT_LAUNCHER in scripts/MergeMill.conf to a valid shell argv (e.g. 'cc --role dev'), then re-dispatch" \
  ...
  if ! eval "AGENT_DEV_LAUNCHER_ARGV=($AGENT_DEV_LAUNCHER)" 2>/dev/null; then
    ...
    if command -v error_surface >/dev/null 2>&1; then
      error_surface "$(error_peek_issue_arg "$@")" ADT_CFG_LAUNCHER_PARSE \
        ...
        "Fix AGENT_DEV_LAUNCHER in scripts/MergeMill.conf to a valid shell argv (e.g. 'cc --role dev'), then re-dispatch" \
        "docs/pipeline/errors.md#configuration-class-class-config" || true
    fi
  ...
  if ! eval "AGENT_REVIEW_LAUNCHER_ARGV=($AGENT_REVIEW_LAUNCHER)" 2>/dev/null; then
    ...
    if command -v error_surface >/dev/null 2>&1; then
      error_surface "$(error_peek_issue_arg "$@")" ADT_CFG_LAUNCHER_PARSE \
        ...
        "Fix AGENT_REVIEW_LAUNCHER in scripts/MergeMill.conf to a valid shell argv (e.g. 'cc --role review'), then re-dispatch" \
        "docs/pipeline/errors.md#configuration-class-class-config" || true
    fi
...
# subshell in MergeMill-review.sh (via _resolve_review_agent_launcher),
...
# `#!/bin/bash` plus PATH (MergeMill.conf documents the requirement).
_parse_extra_args() {
  local var_name="$1"
...
# the hang case sat for 8h+. Override per-deployment in MergeMill.conf.
#
...
preflight_agent_binary() {
  ...
  if command -v error_surface >/dev/null 2>&1; then
    error_surface "${ISSUE_NUMBER:--}" ADT_CFG_AGENT_BINARY_MISSING \
      ...
      "Install '${bin}' on the execution host (or fix PATH / AGENT_CMD in scripts/MergeMill.conf), then re-dispatch" \
      "docs/pipeline/errors.md#configuration-class-class-config" || true
  fi
...
# in the calling wrapper shell (closes #109). Used by MergeMill-dev.sh
# and MergeMill-review.sh so the two share the contract: forward TERM
...
# in their cleanup() trap (MergeMill-dev.sh does for INV-15 / #67); the
...
#
# Args: $1=pid_file, $2=label (e.g. "MergeMill-dev"), $3=issue_number
acquire_pid_guard() {
