#!/bin/bash
# lib-config.sh — single source of truth for loading MergeMill.conf.
#
...
#
# Usage (from a script that needs MergeMill.conf):
#
...
# SCRIPT_DIR to the project's `scripts/`, where MergeMill.conf actually
...
# supported via the legacy `../../../scripts/MergeMill.conf` fallback in
# dispatch-local.sh — kept for backward compat.

# load_MergeMill_conf <invoking_script_dir>
#
# Loads MergeMill.conf from the highest-priority source that exists:
#   1. ${MERGEMILL_CONF}                         (env-var override)
#   2. ${invoking_script_dir}/MergeMill.conf     (script-local)
#   3. ${PROJECT_DIR}/scripts/MergeMill.conf     (project-root fallback,
#                                                  only if PROJECT_DIR set)
#
# On success: sources the file, sets MERGEMILL_CONF_LOADED_FROM to the
...
load_MergeMill_conf() {
  ...
  if [[ -n "${MERGEMILL_CONF:-}" ]] && [[ -f "${MERGEMILL_CONF}" ]]; then
    candidate="${MERGEMILL_CONF}"
  elif [[ -n "$script_dir" ]] && [[ -f "${script_dir}/MergeMill.conf" ]]; then
    candidate="${script_dir}/MergeMill.conf"
  elif [[ -n "${PROJECT_DIR:-}" ]] && [[ -f "${PROJECT_DIR}/scripts/MergeMill.conf" ]]; then
    candidate="${PROJECT_DIR}/scripts/MergeMill.conf"
  fi
  ...
  source "$candidate"
  MERGEMILL_CONF_LOADED_FROM="$candidate"
  export MERGEMILL_CONF_LOADED_FROM
  return 0
}
...
#   1. ${MERGEMILL_PID_DIR}                         (override; used by tests)
#   2. ${XDG_RUNTIME_DIR}/MergeMill-${PROJECT_ID}   (preferred)
#   3. ${HOME}/.local/state/MergeMill-${PROJECT_ID} (fallback)
#
...
pid_dir_for_project() {
  ...
  local dir
  if [[ -n "${MERGEMILL_PID_DIR:-}" ]]; then
    dir="${MERGEMILL_PID_DIR}"
  elif [[ -n "${XDG_RUNTIME_DIR:-}" ]] && [[ -d "${XDG_RUNTIME_DIR}" ]]; then
    dir="${XDG_RUNTIME_DIR}/MergeMill-${PROJECT_ID}"
  else
    : "${HOME:?pid_dir_for_project: HOME must be set when XDG_RUNTIME_DIR is unset}"
    dir="${HOME}/.local/state/MergeMill-${PROJECT_ID}"
  fi
