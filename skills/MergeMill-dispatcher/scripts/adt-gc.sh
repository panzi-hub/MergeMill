...
# ${ADT_STATE_ROOT}/MergeMill-*/lanes/ in one invocation.
#
# Usage:
...
_gc_all_lane_dirs() {
  ...
  for lanes_dir in "$ADT_STATE_ROOT"/MergeMill-*/lanes; do
...
_gc_group_has_live_wrapper() {
  local pg="$1"
  pgrep -g "$pg" -f 'MergeMill-(dev|review)\.sh' 2>/dev/null | grep -vqw "$$"
}
...
# has no per-project manifest of which projects override MERGEMILL_PID_DIR,
...
_gc_pgid_is_live_pidfile_pgid() {
  ...
  for root in "$ADT_STATE_ROOT" "${XDG_RUNTIME_DIR:-}"; do
    ...
    for f in "$root"/MergeMill-*/*.pid; do
      [[ -f "$f" ]] || continue
...
_gc_pgid_in_live_wrapper_ancestry() {
  ...
  done < <(pgrep -f 'MergeMill-(dev|review)\.sh' 2>/dev/null | grep -vw "$$" || true)
  return 1
}
...
_gc_pass2() {
  ...
  while IFS= read -r pid; do
    ...
    # ADT_LANE_ID/MERGEMILL_CONF_LOADED_FROM/CC_USER directly — an
    ...
    if [[ -n "$lane_id" ]]; then
      ...
      conf_loaded="$(env_lookup "$pid" MERGEMILL_CONF_LOADED_FROM 2>/dev/null || echo "")"
      ...
      if [[ -n "$conf_loaded" ]] && [[ "$cc_user" =~ ^MergeMill-(dev|review)-bot$ ]] && [[ "$ppid" == "1" ]]; then
        eligible=true
        is_legacy=true
        age_floor=600
      fi
...
for _gc_lanes_dir in "$ADT_STATE_ROOT"/MergeMill-*/lanes; do
  [[ -d "$_gc_lanes_dir" ]] || continue
  _gc_proj="$(basename "$(dirname "$_gc_lanes_dir")")"
  _gc_proj="${_gc_proj#MergeMill-}"
