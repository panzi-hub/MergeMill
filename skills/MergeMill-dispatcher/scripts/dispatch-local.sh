#!/bin/bash
# dispatch-local.sh — Spawn MergeMill dev/review agent locally.
...
# scripts/, where MergeMill.conf lives [INV-14]. The legacy
# `../../../scripts/MergeMill.conf` fallback is preserved for callers that
...
PROJECT_DIR="${PROJECT_DIR:?Set PROJECT_DIR in MergeMill.conf}"
...
#
# Knobs (MergeMill.conf / dispatcher.conf, all optional — see
# MergeMill.conf.example / dispatcher.conf.example):
...
GATE_ATTEMPT_MARKER_ROOT="${ADT_STATE_ROOT:-$HOME/.local/state}/MergeMill-${PROJECT_ID}/lanes"
...
_admission_gate() {
  ...
  local marker_root
  marker_root="${ADT_STATE_ROOT:-$HOME/.local/state}/MergeMill-${PROJECT_ID}/lanes"
...
kill_stale_wrapper() {
  ...
  #   1. project — anchor on `${PROJECT_DIR}/scripts/`. Multiple MergeMill
  ...
  #      `MergeMill-dev.sh` orphans, `review` only `MergeMill-review.sh`
  #      (closes #126). Pre-fix the matcher was type-agnostic and a
  #      `dev-resume` for issue N would SIGTERM a live `MergeMill-review.sh
...
case "$TYPE" in
  dev-new)
    nohup "${PROJECT_DIR}/scripts/MergeMill-dev.sh" \
  ...
  dev-resume)
    ...
    # with no prior `Dev Session ID:` comment. MergeMill-dev.sh:257-260
    ...
    nohup "${PROJECT_DIR}/scripts/MergeMill-dev.sh" "${resume_args[@]}" \
      >> "/tmp/agent-${PROJECT_ID}-issue-${ISSUE_NUM}.log" 2>&1 &
    CHILD_PID=$!
    ;;
  review)
    nohup "${PROJECT_DIR}/scripts/MergeMill-review.sh" \
      --issue "$ISSUE_NUM" \
...
# dispatch.
rm -f "${ADT_STATE_ROOT:-$HOME/.local/state}/MergeMill-${PROJECT_ID}/lanes/.defer-${GATE_KIND}-${ISSUE_NUM}" 2>/dev/null || true
