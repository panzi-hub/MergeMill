#!/bin/bash
# dispatcher-tick.sh — single entry point for one MergeMill-dispatcher tick.
#
# Replaces the 224 lines of bash that used to live in
# `skills/MergeMill-dispatcher/SKILL.md` with a single script the dispatcher
...
#   Reads MergeMill.conf via lib-dispatch.sh (sourced).
...
# the project's scripts/ where MergeMill.conf lives [INV-14]; it also resolves
...
# `nohup .../MergeMill-{dev,review}.sh` fails with `Permission denied`
...
for _req in REPO REPO_OWNER PROJECT_ID PROJECT_DIR; do
  if [[ -z "${!_req:-}" ]]; then
    error_surface - ADT_CFG_MISSING_KEY \
      "Required MergeMill.conf key '${_req}' is unset (dispatcher tick)" \
      "${_req} is empty/unset in the project's scripts/MergeMill.conf" \
      "Set ${_req} in scripts/MergeMill.conf (see MergeMill.conf.example), then the next tick proceeds" \
      "docs/pipeline/errors.md#configuration-class-class-config"
    echo "[dispatcher-tick] FATAL: ${_req} must be set in MergeMill.conf" >&2
    exit 1
  fi
done
unset _req
...
# on. lib-auth.sh's top-level runs `load_MergeMill_conf` (fail-safe `|| true`)
...
case "${EXECUTION_BACKEND:-local}" in
  ...
  *)
    error_surface - ADT_CFG_EXECUTION_BACKEND_INVALID \
      ...
      "Set EXECUTION_BACKEND to local or remote-aws-ssm in dispatcher.conf/MergeMill.conf, then the next tick proceeds" \
      "docs/pipeline/errors.md#configuration-class-class-config"
...
if ! parse_review_bots "${REVIEW_BOTS:-}" >/dev/null; then
  error_surface - ADT_CFG_REVIEW_BOTS_INVALID \
    ...
    "Fix REVIEW_BOTS in scripts/MergeMill.conf to a space-separated list of known bot short-names (or empty), then the next tick proceeds" \
    "docs/pipeline/errors.md#configuration-class-class-config"
  echo "[dispatcher-tick] FATAL: REVIEW_BOTS validation failed (see error above). Fix MergeMill.conf before the next tick." >&2
  exit 1
fi
...
if ! _ift_err=$(issue_filter_validate "${ISSUE_FILTER:-}" 2>&1); then
  error_surface - ADT_CFG_ISSUE_FILTER_INVALID \
    ...
    "Fix ISSUE_FILTER in scripts/MergeMill.conf (or the project's dispatcher.conf block) per the grammar documented in MergeMill.conf.example, then the next tick proceeds" \
    "docs/pipeline/errors.md#configuration-class-class-config"
  echo "[dispatcher-tick] FATAL: ISSUE_FILTER validation failed: ${_ift_err:-see error above}. Fix MergeMill.conf before the next tick." >&2
  unset _ift_err
  exit 1
fi
unset _ift_err
...
if [[ "$_ift_bad_limit" -eq 1 ]]; then
  error_surface - ADT_CFG_ISSUE_SCAN_LIMIT_INVALID \
    ...
    "Set ISSUE_SCAN_LIMIT to a positive integer (or unset it for the default 100) in scripts/MergeMill.conf, then the next tick proceeds" \
    "docs/pipeline/errors.md#configuration-class-class-config"
  echo "[dispatcher-tick] FATAL: ISSUE_SCAN_LIMIT='${ISSUE_SCAN_LIMIT:-}' is not a positive integer. Fix MergeMill.conf before the next tick." >&2
...
if [[ "${GH_AUTH_MODE:-token}" == "app" ]] && github_seam_active; then
  if [[ -z "${DISPATCHER_APP_ID:-}" || -z "${DISPATCHER_APP_PEM:-}" ]]; then
    error_surface - ADT_AUTH_APP_CREDS_MISSING \
      ...
      "DISPATCHER_APP_ID and/or DISPATCHER_APP_PEM is empty in dispatcher.conf/MergeMill.conf" \
  ...
  # Auto-derive REPO_NAME from REPO when an older path-entry MergeMill.conf
...
for i in $(seq 0 $((new_count - 1))); do
  ...
  # [INV-70] Metrics: the issue is first picked up for MergeMill work — the
  ...
  #
  # The event `ts` is THIS dispatch instant, which can lag the real `MergeMill`
  # label time by ticks (concurrency cap, unresolved deps). For accurate TTHW we
  # also fetch the actual `MergeMill`-label timeline timestamp and emit it as
  ...
  if declare -F metrics_emit >/dev/null 2>&1; then
    ...
    if declare -F "itp_${ISSUE_PROVIDER}_label_event_ts" >/dev/null 2>&1; then
      _labeled_at="$(itp_label_event_ts "$issue_num" "MergeMill")"
    fi
