...
# MERGEMILL_CONF_LOADED_FROM, TERM_PROGRAM, GH_TOKEN_FILE) behind one
...
# across EVERY project's registry under `${ADT_STATE_ROOT}/MergeMill-*/lanes/`
...
# `MergeMill-*/lanes` directory exists under `ADT_STATE_ROOT` at all (a
...
# `*.pid` under every `MergeMill-*` project dir (mirrors
...
_lanes_root() {
  local project_id="$1"
  local root="${ADT_STATE_ROOT}/MergeMill-${project_id}/lanes"
...
# call site instead (see MergeMill-dev.sh/MergeMill-review.sh). A future
