...
PIPELINE_FILES=(MergeMill-dev.sh MergeMill-review.sh dispatcher-tick.sh lib-dispatch.sh)
...
# sorted in awk so {reviewing,MergeMill} compares equal to the table's
# {MergeMill,reviewing} regardless of source order.
collect_movements() {
  local f path
  for f in "${PIPELINE_FILES[@]}"; do
    path="$SCRIPTS_DIR/$f"
    ...
    awk -v file="$f" '
      ...
      # makes {reviewing,MergeMill} compare equal to {MergeMill,reviewing}.
