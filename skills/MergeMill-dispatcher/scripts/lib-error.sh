...
# token-refresh `gh` proxy at ${MERGEMILL_CONF_DIR}/gh (the project-side
...
# MERGEMILL_CONF_DIR). Used to locate the co-located `gh-with-token-refresh.sh`
# as a fallback when the project-side `${MERGEMILL_CONF_DIR}/gh` proxy symlink
...
error_surface() {
  ...
  #   1. The project-side `${MERGEMILL_CONF_DIR}/gh` symlink (the path
  ...
  if [[ -n "${MERGEMILL_CONF_DIR:-}" && -x "${MERGEMILL_CONF_DIR}/gh" ]]; then
    gh_proxy="${MERGEMILL_CONF_DIR}/gh"
  elif [[ -x "${_LIB_ERROR_DIR}/gh-with-token-refresh.sh" ]]; then
    gh_proxy="${_LIB_ERROR_DIR}/gh-with-token-refresh.sh"
  fi
  if [[ -z "$gh_proxy" ]]; then
    _error_log "token-refresh CLI proxy not resolvable (MERGEMILL_CONF_DIR='${MERGEMILL_CONF_DIR:-}', skill-tree fallback '${_LIB_ERROR_DIR}/gh-with-token-refresh.sh' missing); degrading envelope ${code} to log-only:"
    printf '%s\n' "$rendered" >&2
    return 0
  fi
