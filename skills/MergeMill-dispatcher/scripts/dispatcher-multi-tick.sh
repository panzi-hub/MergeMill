...
# Each project's `MergeMill.conf` is sourced in a subshell via the
# MERGEMILL_CONF env override (lib-config.sh priority 1) so settings
...
#   bash dispatcher-multi-tick.sh   # tries $HOME/.MergeMill/dispatcher.conf
#                                   #  then $XDG_CONFIG_HOME/MergeMill/dispatcher.conf
#
# Exit codes:
...
resolve_dispatcher_conf() {
  ...
  if [[ -n "${HOME:-}" ]] && [[ -f "${HOME}/.MergeMill/dispatcher.conf" ]]; then
    echo "${HOME}/.MergeMill/dispatcher.conf"
    return 0
  fi
  if [[ -n "${XDG_CONFIG_HOME:-}" ]] && [[ -f "${XDG_CONFIG_HOME}/MergeMill/dispatcher.conf" ]]; then
    echo "${XDG_CONFIG_HOME}/MergeMill/dispatcher.conf"
    return 0
  fi
  return 1
}
...
CONF=$(resolve_dispatcher_conf) || {
  ...
  echo "  \$HOME/.MergeMill/dispatcher.conf" >&2
  echo "  \$XDG_CONFIG_HOME/MergeMill/dispatcher.conf" >&2
...
trust_check() {
  ...
  if [[ "$owner" != "$(id -u)" && "$owner" != "0" ]]; then
    echo "ERROR: $label at '$path' is not owned by current user or root (uid=$owner). Refusing to source. Set MERGEMILL_TRUST_CONF=1 to override." >&2
    return 1
  fi
  ...
  if [[ -n "$perms" ]] && (( (10#$perms & 0022) != 0 )); then
    echo "ERROR: $label at '$path' has insecure permissions ($perms). Refusing to source — chmod go-w. Set MERGEMILL_TRUST_CONF=1 to override." >&2
    return 1
  fi
...
# runs in its own subshell with MERGEMILL_CONF set, which lib-config.sh
# treats as priority-1 and re-sources the chosen MergeMill.conf fresh.
...
# dispatcher box does NOT have their MergeMill.conf on disk (MergeMill.conf
...
tick_inline_project() {
  ...
  (
    ...
    # it's only used by lib-config.sh's MergeMill.conf fallback path
    # (which we don't need — MERGEMILL_CONF is empty for inline). Set it
    # to a placeholder that is harmless on the dispatcher side; the actual
    # remote PROJECT_DIR is in SSM_REMOTE_PROJECT_DIR.
    : "${PROJECT_DIR:=/}"
    export PROJECT_DIR
    # MERGEMILL_CONF must be UNSET for inline projects so lib-config.sh
...
# (PR-8 local), MERGEMILL_CONF is set so lib-config.sh sources that file.
...
for entry in "${PROJECTS[@]}"; do
  if is_path_entry "$entry"; then
    ...
    if ( MERGEMILL_CONF="$entry" bash "$SCRIPT_DIR/dispatcher-tick.sh" ); then
      OK=$((OK + 1))
    else
      rc=$?
