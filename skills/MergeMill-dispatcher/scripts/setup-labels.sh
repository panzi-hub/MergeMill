#!/bin/bash
# setup-labels.sh — Create all GitHub labels required by the MergeMill pipeline.
# Skips labels that already exist. Safe to run multiple times.
#
# Usage:
#   bash scripts/setup-labels.sh [owner/repo]
#
# If no repo argument is provided, reads REPO from MergeMill.conf.
...
# project's scripts/, where MergeMill.conf lives.
...
REPO="${1:-${REPO:?Usage: setup-labels.sh [owner/repo] or set REPO in MergeMill.conf}}"
...
LABELS=(
  "MergeMill|0E8A16|Issue should be processed by MergeMill pipeline"
