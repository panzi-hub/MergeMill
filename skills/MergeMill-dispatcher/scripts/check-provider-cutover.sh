...
# layer (lib-dispatch.sh, MergeMill-dev.sh, MergeMill-review.sh, every
...
# ---------------------------------------------------------------------------
# AC #41 requires that EVERY surviving raw-gh site in skills/MergeMill-dispatcher/
...
TRUSTED_BASELINE_PATH="${CUTOVER_TRUSTED_BASELINE_PATH:-skills/MergeMill-dispatcher/scripts/providers/cutover-baseline.json}"
...
CALLER_FILES=(lib-dispatch.sh MergeMill-dev.sh MergeMill-review.sh)
...
# ("MergeMill-dev.sh", "adapters/codex.sh", "providers/itp-github.sh", and any
...
tree_sh_files() {
  ...
  # symlinks into sibling skill dirs but still live in skills/MergeMill-dispatcher/
...
# tracked scripts (mark-issue-checkbox.sh -> ../../MergeMill-common/...) would
