...
# Reserved atoms (design §4.3): pipeline state labels + the `MergeMill`
...
# runs (the §7.2 stability corollary), and `MergeMill` is already implicit.
_ISSUE_FILTER_RESERVED_LABELS="in-progress reviewing pending-review pending-dev stalled approved MergeMill"
...
issue_filter_validate() {
  ...
  if reserved_hit=$(_issue_filter_uses_reserved_label "$filter"); then
    echo "issue_filter_validate: ISSUE_FILTER references reserved label '${reserved_hit}' (pipeline state labels and 'MergeMill' cannot be filter atoms)" >&2
    return 1
  fi
