#!/bin/bash
# metrics-report.sh — INV-70 baseline metrics aggregator for the MergeMill
# pipeline (issue #228).
#
...
if source "${LIB_DIR}/lib-config.sh" 2>/dev/null; then
  load_MergeMill_conf "${MERGEMILL_CONF_DIR}" 2>/dev/null || true
fi
...
# MergeMill-label time from the GitHub timeline — over ts, the dispatch instant,
