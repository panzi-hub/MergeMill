...
# readlink -f (the production path). Overridable via MERGEMILL_PROVIDERS_DIR so
...
_LIB_CHP_PROVIDERS_DIR="${MERGEMILL_PROVIDERS_DIR:-${_LIB_CHP_REAL_DIR}/providers}"
...
# in_reply_to=…` site (reply-to-comments.sh, an MergeMill-common util) routes
...
# builder's PR_REVIEW_COMMENTS read (MergeMill-dev.sh) routes through this so the
