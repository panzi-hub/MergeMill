...
#
# Usage:
#   scripts/MergeMill-review.sh --issue <number>
#
# Exit codes:
...
# <project>/scripts/MergeMill-review.sh keeps it pointed at the project's
# scripts/ — lib-agent.sh's load_MergeMill_conf then finds MergeMill.conf
...
export MERGEMILL_CONF_DIR="$SCRIPT_DIR"
...
# ~3h sooner. The dev side (MergeMill-dev.sh) is untouched and keeps 4h.
#
...
for _req in PROJECT_ID REPO REPO_OWNER REPO_NAME PROJECT_DIR; do
  if [[ -z "${!_req:-}" ]]; then
    error_surface "$ISSUE_NUMBER" ADT_CFG_MISSING_KEY \
      "Required MergeMill.conf key '${_req}' is unset (review wrapper)" \
      "${_req} is empty/unset in the project's scripts/MergeMill.conf" \
      "Set ${_req} in scripts/MergeMill.conf (see MergeMill.conf.example), then re-dispatch" \
      "docs/pipeline/errors.md#configuration-class-class-config"
    echo "Error: Set ${_req} in MergeMill.conf" >&2
    exit 1
  fi
done
...
if ! REVIEW_BOTS_VALIDATED=$(parse_review_bots "${REVIEW_BOTS:-}"); then
  ...
  error_surface "$ISSUE_NUMBER" ADT_CFG_REVIEW_BOTS_INVALID \
    ...
    "Fix REVIEW_BOTS in scripts/MergeMill.conf to a space-separated list of known bot short-names (or empty), then re-dispatch" \
    "docs/pipeline/errors.md#configuration-class-class-config"
  exit 1
fi
...
if [[ -n "$REVIEW_BOTS_VALIDATED" ]] && declare -F chp_caps >/dev/null 2>&1 \
  ...
  # abort the wrapper.
  echo "[MergeMill-review] Code host review_bots=0 — disabling bot-review enforcement (REVIEW_BOTS='${REVIEW_BOTS_VALIDATED}' has no effect: the backend has no slash-command registry, so the prompt's bot-review section, the trigger broker, and the post-run wait are all suppressed)." >&2
...
if declare -F lane_mint >/dev/null 2>&1; then
  ...
  if [[ -n "$ADT_LANE_DIR" ]]; then
    ...
    echo "[MergeMill-review] WARNING: lane_install failed — proceeding without a lane registry entry for this run (review-side reap/tagging degrades to the pre-Lane-GC behavior)." >&2
  fi
fi

# ---------------------------------------------------------------------------
# [Lane-GC PR-5 / INV-118] Guardian sidecar install (design §4-C3). Same
# contract as the dev side (MergeMill-dev.sh) — see that file's comment
...
if [[ -n "$ADT_LANE_DIR" ]]; then
  if ! command -v setsid >/dev/null 2>&1; then
    echo "[MergeMill-review] ERROR: setsid (util-linux) is missing — the guardian sidecar cannot be installed for this run. On macOS: brew install util-linux. Proceeding WITHOUT a guardian; the periodic GC (adt-gc.sh) remains the backstop reaper for this lane (degraded but not a hard abort)." >&2
  elif ! mkfifo "${ADT_LANE_DIR}/guard.fifo" 2>/dev/null; then
    echo "[MergeMill-review] WARNING: mkfifo failed for the guardian's FIFO — proceeding without a guardian for this run (GC remains the backstop)." >&2
  else
...
# `github_seam_active` predicate (lib-auth.sh). SAME shape as MergeMill-dev.sh
  ...
  if [[ "$GH_AUTH_MODE" == "app" ]]; then
    if [[ -z "${REVIEW_AGENT_APP_ID:-}" || -z "${REVIEW_AGENT_APP_PEM:-}" ]]; then
      ...
      error_surface "$ISSUE_NUMBER" ADT_AUTH_APP_CREDS_MISSING \
        ...
        "REVIEW_AGENT_APP_ID and/or REVIEW_AGENT_APP_PEM is empty in MergeMill.conf" \
...
validate_e2e_config() {
  ...
  if [[ "${E2E_ENABLED:-false}" == "true" ]] && [[ -z "${E2E_MODE:-}" ]]; then
    error_surface "$ISSUE_NUMBER" ADT_CFG_E2E_MODE_REQUIRED \
      ...
      "Set E2E_MODE to none, browser, or command in scripts/MergeMill.conf, then re-dispatch" \
      "docs/pipeline/errors.md#configuration-class-class-config"
    ...
    none|browser)
      ...
      if [[ -n "${E2E_COMMAND:-}" || -n "${E2E_COMMAND_EVIDENCE_PARSER:-}" || -n "${E2E_COMMAND_PRE_HOOKS:-}" ]]; then
        error_surface "$ISSUE_NUMBER" ADT_CFG_E2E_MODE_MISMATCH \
          ...
          "Set E2E_MODE=command (or clear the E2E_COMMAND* fields) in scripts/MergeMill.conf, then re-dispatch" \
          "docs/pipeline/errors.md#configuration-class-class-config"
    ...
    command)
      if [[ -z "${E2E_COMMAND:-}" ]]; then
        error_surface "$ISSUE_NUMBER" ADT_CFG_E2E_COMMAND_MISSING \
          ...
          "Set E2E_COMMAND in scripts/MergeMill.conf (e.g. 'bash scripts/e2e-pr-stage.sh \${PR_NUMBER}'), then re-dispatch" \
          "docs/pipeline/errors.md#configuration-class-class-config"
      ...
      if [[ -z "${E2E_COMMAND_EVIDENCE_PARSER:-}" ]]; then
        error_surface "$ISSUE_NUMBER" ADT_CFG_E2E_PARSER_MISSING \
          ...
          "Set E2E_COMMAND_EVIDENCE_PARSER in scripts/MergeMill.conf (see references/e2e-command-mode.md), then re-dispatch" \
          "docs/pipeline/errors.md#configuration-class-class-config"
      ...
      for _field in E2E_COMMAND E2E_COMMAND_PRE_HOOKS E2E_COMMAND_EVIDENCE_PARSER; do
        ...
        if [[ "$_value" =~ \$PR_NUMBER([^A-Za-z0-9_{]|$) ]]; then
          error_surface "$ISSUE_NUMBER" ADT_CFG_E2E_PR_NUMBER_UNBRACED \
            ...
            "Use \${PR_NUMBER} (with braces) in ${_field} in scripts/MergeMill.conf, then re-dispatch" \
            "docs/pipeline/errors.md#configuration-class-class-config"
...
validate_review_timeout_config() {
  if [[ -n "${AGENT_REVIEW_TIMEOUT:-}" ]] && ! _is_positive_timeout_value "$AGENT_REVIEW_TIMEOUT"; then
    error_surface "$ISSUE_NUMBER" ADT_CFG_REVIEW_TIMEOUT_INVALID \
      ...
      "Set AGENT_REVIEW_TIMEOUT to a positive coreutils-timeout value (e.g. 3600, 90m, 2h) in scripts/MergeMill.conf, then re-dispatch" \
      "docs/pipeline/errors.md#configuration-class-class-config"
  ...
  if [[ -n "${_E2E_BROWSER_TIMEOUT_RAW:-}" ]] && ! _is_positive_timeout_value "$_E2E_BROWSER_TIMEOUT_RAW"; then
    error_surface "$ISSUE_NUMBER" ADT_CFG_E2E_BROWSER_TIMEOUT_INVALID \
      ...
      "Set E2E_BROWSER_TIMEOUT_SECONDS to a positive value (e.g. 900) in scripts/MergeMill.conf, then re-dispatch" \
      "docs/pipeline/errors.md#configuration-class-class-config"
  ...
  if [[ -n "${REVIEW_SMOKE_TIMEOUT_SECONDS:-}" ]] && ! _is_positive_timeout_value "$REVIEW_SMOKE_TIMEOUT_SECONDS"; then
    error_surface "$ISSUE_NUMBER" ADT_CFG_SMOKE_TIMEOUT_INVALID \
      ...
      "Set REVIEW_SMOKE_TIMEOUT_SECONDS to a positive value in scripts/MergeMill.conf, then re-dispatch" \
      "docs/pipeline/errors.md#configuration-class-class-config"
...
# E2E_ENABLED toggle is preserved for back-compat in MergeMill.conf
...
if ! cd "$PROJECT_DIR"; then
  error_surface "$ISSUE_NUMBER" ADT_CFG_PROJECT_DIR_INVALID \
    ...
    "Fix PROJECT_DIR in scripts/MergeMill.conf so it points at the project checkout on the execution host, then re-dispatch" \
    "docs/pipeline/errors.md#configuration-class-class-config"
...
export CC_USER="${CC_USER:-MergeMill-review-bot}"
...
# this tee is additive and also covers a direct `bash MergeMill-review.sh` run.
...
# MergeMill-dev.sh so the trap is identical.
...
acquire_pid_guard "$PID_FILE" "MergeMill-review" "$ISSUE_NUMBER"
...
# Helpers
# ---------------------------------------------------------------------------
log() { echo "[MergeMill-review] $(date -u +%H:%M:%S) $*"; }
...
cleanup() {
  ...
  # as the dev side (MergeMill-dev.sh::cleanup()); see that file's comment
...
# ---------------------------------------------------------------------------
if [[ "${E2E_SCREENSHOT_UPLOAD:-false}" == "true" && -x "${PROJECT_DIR}/skills/MergeMill-review/scripts/upload-screenshot.sh" ]]; then
...
# string. Operators write the placeholder in MergeMill.conf with single
...
behavior):
- \`actionable_by_dev_agent\` (boolean) — \`true\` iff the MergeMill dev agent can
...
${_smoke_fail_clause}
The review was NOT run and the PR was NOT evaluated. The issue stays \`reviewing\` (it is NOT bounced to development — there is no PR problem for the dev agent to fix). Fix the agent configuration in \`MergeMill.conf\`; the next dispatch tick re-runs the review once the smoke passes.$(declare -F run_footer >/dev/null 2>&1 && run_footer || true)" 2>/dev/null || true
...
1. **[BLOCKING] Merge conflict with main** — PR #${PR_NUMBER} (\`${PR_BRANCH:-the PR branch}\`) is \`CONFLICTING\` with the base branch and cannot be merged. The review agent's PASS verdict is overridden by the wrapper-enforced mergeable gate (INV-44).
   - Dev agent must rebase before re-review:
    ...
    # Reuse the dev-resume rebase hook: MergeMill-dev.sh greps issue-level PR
...
if [[ "$PASSED_VERDICT" == "true" ]]; then
  ...
  if [[ "$HAS_NO_AUTO_CLOSE" == "true" ]]; then
    ...
    # Update labels: remove reviewing, add approved (keep no-auto-close and MergeMill)
    itp_transition_state "$ISSUE_NUMBER" "reviewing" "approved" 2>/dev/null || true
    ...
    if [[ $MERGE_RC -eq 0 ]]; then
      ...
      itp_transition_state "$ISSUE_NUMBER" "reviewing,MergeMill" "approved" 2>/dev/null || true
      ...
      # issue to pending-dev while keeping `MergeMill` so the dispatcher's
...
Re-dispatching dev agent to rebase onto main.$(declare -F run_footer >/dev/null 2>&1 && run_footer || true)" 2>&1 >/dev/null); then
      ...
      if ! _edit_err=$(itp_transition_state "$ISSUE_NUMBER" "reviewing" "pending-dev" 2>&1 >/dev/null); then
        ...
        log "Issue #${ISSUE_NUMBER} flipped to pending-dev for rebase re-dispatch (MergeMill label retained)."
      fi
    fi
  fi
else
