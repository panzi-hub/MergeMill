...
# run-id namespace):
#   ${XDG_STATE_HOME:-$HOME/.local/state}/MergeMill-<project>/runs/<run-id>/
...
_run_state_base() {
  ...
  if [[ -n "${MERGEMILL_RUN_DIR_BASE:-}" ]]; then
    # Test/override hook — the explicit base already includes MergeMill-<proj>.
    printf '%s\n' "${MERGEMILL_RUN_DIR_BASE}"
    return 0
  fi
  if [[ -n "${XDG_STATE_HOME:-}" ]]; then
    printf '%s/MergeMill-%s\n' "${XDG_STATE_HOME}" "$project_id"
  elif [[ -n "${HOME:-}" ]]; then
    printf '%s/.local/state/MergeMill-%s\n' "${HOME}" "$project_id"
  else
    return 1
  fi
}
