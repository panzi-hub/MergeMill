...
#
# Usage:
#   scripts/MergeMill-dev.sh --issue <number> --mode new
#   scripts/MergeMill-dev.sh --issue <number> --mode resume --session <session-id>
#
# Exit codes:
...
# <project>/scripts/MergeMill-dev.sh keeps it pointed at the project's
...
# overwrite this rebind. Same ordering is applied in MergeMill-review.sh.
...
for _req in PROJECT_ID REPO REPO_OWNER REPO_NAME PROJECT_DIR; do
  if [[ -z "${!_req:-}" ]]; then
    error_surface "$ISSUE_NUMBER" ADT_CFG_MISSING_KEY \
      "Required MergeMill.conf key '${_req}' is unset (dev wrapper)" \
...
if declare -F lane_mint >/dev/null 2>&1; then
  ...
  if [[ -n "$ADT_LANE_DIR" ]]; then
    ...
    echo "[MergeMill-dev] WARNING: lane_install failed — proceeding without a lane registry entry for this run (dev-side reap/tagging degrades to the pre-Lane-GC behavior)." >&2
  fi
fi
...
# sites (this wrapper, MergeMill-review.sh, dispatcher-tick.sh, and the
  ...
  if [[ "$GH_AUTH_MODE" == "app" ]]; then
    if [[ -z "${DEV_AGENT_APP_ID:-}" || -z "${DEV_AGENT_APP_PEM:-}" ]]; then
      ...
      error_surface "$ISSUE_NUMBER" ADT_AUTH_APP_CREDS_MISSING \
        ...
        "DEV_AGENT_APP_ID and/or DEV_AGENT_APP_PEM is empty in MergeMill.conf" \
...
export CC_USER="${CC_USER:-MergeMill-dev-bot}"
...
# `bash MergeMill-dev.sh` invocation has no such file — so guard here rather
...
# is additive and also gives a direct `bash MergeMill-dev.sh` invocation a
...
acquire_pid_guard "$PID_FILE" "MergeMill-dev" "$ISSUE_NUMBER"
...
# Helpers
# ---------------------------------------------------------------------------
log() { echo "[MergeMill-dev] $(date -u +%H:%M:%S) $*"; }
...
emit_open_pr_fast_path_block() {
  ...
  if [[ -n "${AGENT_GH_TOKEN_FILE:-}" ]]; then
   ...
   PR body (Step 7 of /MergeMill-dev), ensuring the body contains
   \"${_close_kw}\"."
  fi
...
not on PATH (set \`REAL_GH\` in MergeMill.conf — see #92), missing
...
EOF
  ...
  # tests/unit/test-MergeMill-dev-cleanup-startup-failure.sh uses the
...
EOF
  ...
  # `Dispatching MergeMill review`, `Moving to pending-review for
...
5. Continue following ${DEV_SKILL_CMD:-/MergeMill-dev} skill (fix -> test -> push -> wait CI)
...
5. Follow ${DEV_SKILL_CMD:-/MergeMill-dev} skill (Steps 1-12)
