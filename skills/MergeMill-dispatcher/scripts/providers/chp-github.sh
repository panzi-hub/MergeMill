...
#
# Sole caller: skills/MergeMill-common/scripts/resolve-threads.sh (selects
...
# scripts dir first (`_LIB_AUTH_DIR`, else `MERGEMILL_CONF_DIR`) — the same place
...
chp_github_trigger_bot() {
  ...
  for _d in "${_LIB_AUTH_DIR:-}" "${MERGEMILL_CONF_DIR:-}" "$_skill_dir"; do
...
# MergeMill-dev.sh / MergeMill-review.sh / lib-review-e2e.sh) become plain
...
# `PR_REVIEW_COMMENTS` read (MergeMill-dev.sh) — the comments the dev agent
...
#     standalone util that resolves its own $REPO from MergeMill.conf and never
...
# (MergeMill-review.sh, lib-review-e2e.sh) route their auto-merge markers,
