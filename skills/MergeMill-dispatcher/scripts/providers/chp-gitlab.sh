...
_chp_gitlab_require_project() {
  if [[ -z "${GITLAB_PROJECT:-}" ]]; then
    echo "ERROR: chp_gitlab_${1:-<verb>}: GITLAB_PROJECT env var required (unset or empty). Set GITLAB_PROJECT=<url-encoded project path> in MergeMill.conf — see §3.4 / docs/gitlab-token-setup.md." >&2
    return 1
  fi
}
...
# + MergeMill-review.sh:3604/3813) pass `--body <string>`; no --body-file / no
...
chp_gitlab_commit_file() {
  ...
  # MergeMill.conf's REPO). Detect: a `/` in the value means raw → encode.
...
chp_gitlab_count_reviews_by_login() {
  ...
  # (missing_bot_reviews, lib-review-bots.sh) threads MergeMill.conf's RAW
