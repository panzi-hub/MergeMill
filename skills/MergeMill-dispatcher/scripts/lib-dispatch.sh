#!/bin/bash
# lib-dispatch.sh — composable helpers for the MergeMill-dispatcher tick.
#
# All gh / jq / regex logic that used to live in MergeMill-dispatcher/SKILL.md
...
: "${REPO:?REPO must be set in MergeMill.conf}"
: "${REPO_OWNER:?REPO_OWNER must be set in MergeMill.conf}"
: "${PROJECT_ID:?PROJECT_ID must be set in MergeMill.conf}"
...
# state=open, labels-AND="MergeMill", limit=ISSUE_SCAN_LIMIT,
...
count_active() {
  ...
  if _issue_filter_is_unset "${ISSUE_FILTER:-}"; then
    itp_count_by_state open "MergeMill" "$limit" "in-progress,reviewing"
  else
    itp_list_by_state open "MergeMill" "$limit" "number,labels,assignees" \
      | issue_filter_apply \
...
# Step 2: issues with `MergeMill` label and NO state label.
# Echoes JSON array of {number, labels, title}.
#
# [INV-87]/[W1a, #371] The enumeration routes through the ABSTRACT
# itp_list_by_state contract (state=open, labels-AND="MergeMill",
...
list_new_issues() {
  itp_list_by_state open "MergeMill" "${ISSUE_SCAN_LIMIT:-100}" "$(issue_filter_fields "number,labels,title")" | jq '[.[] | select(
    (.labels | any(. == "in-progress" or . == "pending-review" or . == "reviewing"
                   or . == "pending-dev" or . == "stalled" or . == "approved")) | not
  )]' | issue_filter_apply
}

# Step 3: issues with `MergeMill` + `pending-review` AND NOT `reviewing`.
...
list_pending_review() {
  ...
  itp_list_by_state open "MergeMill,pending-review" "${ISSUE_SCAN_LIMIT:-100}" "$(issue_filter_fields "number,labels")" | jq '[.[] | select(
...
# Step 4: issues with `MergeMill` + `pending-dev`.
...
list_stale_candidates() {
  ...
  # selection.
  itp_list_by_state open "MergeMill" "${ISSUE_SCAN_LIMIT:-100}" "$(issue_filter_fields "number,labels")" | jq '[.[] | select(
...
# List MergeMill issues whose label set is in violation of the
...
# Bug 5.
#
# Why exclude `Mode: startup-failure`: MergeMill-dev.sh's startup-failure
...
classify_recent_review_verdict() {
  ...
  # review wrapper already emits per MergeMill-review.sh:588-590).
  ...
  # process (it is resolved only inside MergeMill-review.sh's SEPARATE
...
# inside `MergeMill-review.sh`'s SEPARATE process via `gh api user`, and never
...
#     (MergeMill-review.sh, posted right after the agent's findings comment).
...
# `MergeMill-review.sh`'s SEPARATE process) and `FALLBACK_SESSION_ID` is
...
handle_completed_session_routing() {
  ...
  case "$_verdict" in
    ...
    failed-substantive)
      ...
      # removes `MergeMill` (expensive on an unattended pipeline).
      ...
      if [ "$_cb_threshold" -gt 0 ] && [ "$_cb_rounds" -ge "$_cb_threshold" ]; then
        ...
        # the SAME movement `mark_stalled` uses (no undeclared `MergeMill →
        # stalled` edge; `MergeMill` is retained throughout, never part of this
...
The MergeMill dev↔review loop is **not converging**: the review keeps failing
...
- Transitioned the issue to \`stalled\` (autonomy halted; \`pending-dev\` removed; \`MergeMill\` is retained) — REMOVING the \`stalled\` label is the operator's explicit opt-in to resume (re-enters via Step 2; retry counter resets, INV-05).
...
**To resume: fix per the checklist above, then REMOVE the \`stalled\` label (the \`MergeMill\` label is retained; removal re-arms the pipeline and resets the retry counter, INV-05).**
@${REPO_OWNER}
CBREPORT
)"
        return 0
...
#   Dispatching MergeMill development...
#
...
post_dispatch_token() {
  ...
  case "$mode" in
    dev-new)     human="Dispatching MergeMill development..." ;;
    dev-resume)  human="Resuming MergeMill development..." ;;
    review)      human="Dispatching MergeMill review..." ;;
...
# in MergeMill-review.sh. Empty on parse failure / no match (caller
...
handle_pending_dev_pr_exists() {
  ...
  if [ -n "$last_head" ] && [ -n "$current_head" ] && [ "$current_head" = "$last_head" ]; then
    ...
    # see [INV-111] in MergeMill-dev.sh's cleanup()). Without a session id,
