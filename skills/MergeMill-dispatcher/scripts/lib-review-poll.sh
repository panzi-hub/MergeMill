...
# set by MergeMill.conf). Pure: no side effects, no I/O.
#
...
# _classify_unresolved_agent. (Moved here from MergeMill-review.sh in #180.)
...
# issue #180). Extracted from MergeMill-review.sh so the loop itself — not just
...
# MergeMill-review.sh survives verdict resolution even on a path where the
...
# (MergeMill-review.sh, `_fanout_pids`) — NOT inside the `codex review`
# process itself.
