...
How to write acceptance criteria (ACs) that the MergeMill dev/review loop can
...
2. **Do NOT add the `MergeMill` label to the follow-up.** An MergeMill follow-up
...
So: follow-up created first → no `MergeMill` label → referenced under Out of
...
### Example 2 — a genuine post-merge AC split into a non-blocking, non-MergeMill follow-up
...
**Do not leave this as a blocking AC** on the MergeMill feature issue. Instead:

1. Create the follow-up first — issue titled `Post-deploy verify: new latency
   panel populates from prod telemetry` — and **omit the `MergeMill` label**
...
MergeMill loop; the prod-only check lives in a non-MergeMill follow-up that a
